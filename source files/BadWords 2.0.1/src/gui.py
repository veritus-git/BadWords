...
class BadWordsGUI:
    def __init__(self, root, engine, resolve_handler):
        try:
            ...
            def_auto_f = True
            def_auto_d = False
            def_show_typos = True # New default
            ...
            if prefs:
                ...
                # Model handling (Stored as technical name sometimes, but var stores display name usually)
                # Here we assume var_model stores Display Name.
                # If prefs stored "medium", we need "Medium (Best balance...)"
                # This reverse mapping is tricky if not saved properly.
                # Let's trust what was saved in save_preferences (which saves var_model.get() directly)
                ...
                def_compound = p_set.get("compound", def_compound)
                ...
                def_show_typos = p_set.get("show_typos", def_show_typos) # Load new pref
            ...
            self.var_compound = tk.BooleanVar(value=def_compound)
            ...
            self.var_show_typos = tk.BooleanVar(value=def_show_typos) # New Var
    ...
    def load_project(self):
        try:
            ...
            self.var_show_typos.set(s.get("show_typos", True)) # Load project pref for typos
    ...
    def _add_gear_button(self, parent, bg_color):
        settings_btn = tk.Label(parent, text="≡", font=(config.UI_FONT_NAME, 20), bg=bg_color, fg=config.GEAR_COLOR, cursor="hand2", bd=0)
    ...
    def run_analysis_pipeline(self):
        ...
        def run_thread():
            words, segments = self.engine.run_analysis_pipeline(settings, callback_status=self.set_status, callback_progress=self.set_progress)
            if words:
                for w in words:
    ...
    def show_reviewer_stage(self):
        ...
        if is_reviewer_mode:
            ...
            # --- CLEAR TRANSCRIPT BUTTON (Localized) ---
            def clear_transcript_action():
                ...
                if confirm.result:
                    for w in self.words_data:
                        if w.get('status') in ['bad', 'repeat', 'typo', 'eraser'] or (w.get('selected') and w.get('status') is not None):
                            if w.get('type') not in ['silence', 'inaudible']:
                                 w['status'] = None
                                 w['selected'] = False
                            elif w.get('type') == 'inaudible':
                                 w['status'] = 'inaudible'
                                 w['selected'] = True
                    self.populate_text_area()
            # Style: Dark Gray (#4a4a4a) with White text
            # ADDED: activebackground="#4a4a4a", activeforeground="white" to disable hover highlight
    ...
    def run_generation_logic(self):
        ...
        # --- FORCE RESOLVE FOCUS ---
        # Wywołanie API OpenPage("edit") zazwyczaj wymusza na systemie operacyjnym
        # wyciągnięcie okna Resolve na wierzch (Focus), ponieważ aplikacja zgłasza aktywność.
    ...
    def _on_generation_success(self, warning_code=None):
        # --- MODIFIED: Do not restore GUI (Keep it in background/hidden) ---
        # We REMOVED self.root.deiconify() here.
        # Instead, we minimize it so it's accessible via taskbar ("w tle").
        ...
        # Show popup directly. Parent is mostly hidden/minimized, so CustomMessage handles it.
        self.root.after(100, lambda: CustomMessage(self.root, "Success", self.txt(msg_key)))
        self.root.after(2000, lambda: self.set_progress(0))

    def _on_generation_error(self, error_msg):
        # --- SHOW GUI START ---
        # For errors, we likely want to show the app again to fix config.
    ...
    def _configure_text_tags(self):
        ...
        self.text_area.tag_configure("rtl", justify='right') # New Tag for RTL alignment
    ...
    def populate_text_area(self):
        ...
        while i < batch_len:
            ...
            if w_obj.get('is_segment_start'):
                ...
                # Insert a zero-width space that has the alignment tag ('rtl') but 
                # NO background color tag. This prevents the background color of the 
                # first character from flooding the left-side justification padding.
                if is_rtl_content:
            ...
            if w_obj.get('is_inaudible'):
                ...
                # --- NEW LOGIC: Hide Typos if disabled ---
                if state == "typo" and not show_typos:
                    state_tag = "normal" # Render as normal text
                else:
                    state_tag = state if state else "normal"
                # ----------------------------------------
                
                # --- RTL FIX ---
                # Visually reverse text for display in Tkinter if it is RTL
                raw_text = w_obj['text']
                if self.is_text_rtl(raw_text):
                    # Reverse string for visual correctness in LTR widget
                    display_text = raw_text[::-1]
                else:
                    display_text = raw_text
                # ---------------

                self.text_area.insert(tk.END, display_text, (tag_name, "normal", state_tag) + base_tags)
                if state and (state != "typo" or show_typos): # Only tag if shown
                    self.text_area.tag_add(state, f"{tag_name}.first", f"{tag_name}.last")
                
                space_tag = "normal"
                
                # Check next word for contiguous highlighting
                if state and (state != "typo" or show_typos): 
                    if i + 1 < batch_len:
                        next_idx = i + 1
                        # Skip invisibles
                        ...
                        if next_idx < batch_len:
                            ...
                            # Hide next typo if disabled
                            if next_state == "typo" and not show_typos:
                                next_state = None 
    ...
    def update_word_status(self, word_id, status):
        ...
        show_typos = self.var_show_typos.get() # Check flag
        ...
        def apply_tag_to_word(w_id, new_stat):
            ...
            # Apply tag only if it's not a hidden typo
            if new_stat and new_stat != "normal":
                if new_stat == "typo" and not show_typos:
                    pass # Do not apply green tag
                else:
                    try: self.text_area.tag_add(new_stat, f"{tag_name}.first", f"{tag_name}.last")
                    except: pass
                    
        for wid, stat in updates: apply_tag_to_word(wid, stat)
        self.text_area.configure(state="disabled")