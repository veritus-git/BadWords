...
- UPDATED: Advanced Silence Processing (Micro-Gap Bridging & Safety Padding) KEPT.
- UPDATED: Inaudible threshold lowered to 1.2s.
"""
...
class AudioEngine:
    ...
    def save_preferences(self, settings_dict):
        ...
        """
        try:
            # Fallback if os_doc wasn't updated in runtime context yet (safeguard)
...
"""
    ...
    def _build_data_structure(self, json_data, silence_ranges, filler_words, fps, 
        ...
        # --- NEW LOGIC START: MICRO-GAP BRIDGING & PADDING ---
        # 1. Bridging Micro-Gaps (Noise Islands < 0.15s)
        if scaled_silence:
            bridged = []
            curr = scaled_silence[0]
            for next_s in scaled_silence[1:]:
                # If gap between current silence end and next silence start is small (noise island)
                if next_s['s'] - curr['e'] < 0.15:
                    curr['e'] = next_s['e'] # Merge
                else:
                    bridged.append(curr)
                    curr = next_s
            bridged.append(curr)
        ...
        # 2. Safety Padding (-0.05s on both sides => Shrink Silence)
        # This keeps more audio around words and ensures cut isn't too aggressive
        if scaled_silence:
            padded = []
        ...
        # Store GLOBAL (raw) silence to persist across GUI->Engine cycle.
        # This will be used for "Check Whole Timeline" logic during assembly.
        raw_global_silence = scaled_silence
        
        # Use valid silence for visual filtering (Gap-Filling in GUI)
        ...
        if temp_words:
            ...
            for i in range(1, len(temp_words)):
                ...
                if not relevant:
                    if (gap_end - gap_start) >= 1.2: # CHANGED from 1.7 to 1.2
                        final_words.append({
                    ...
                    for s in relevant:
                        ...
                        if valid_start - current_pos > 1.2: # CHANGED from 1.7 to 1.2
                             final_words.append({
                    ...
                    if gap_end - current_pos > 1.2: # CHANGED from 1.7 to 1.2
                        final_words.append({
        ...
        # INJECT RAW SILENCE METADATA (FIX)
        # We attach the full silence list to the first word object.
        # This allows us to retrieve it later in 'calculate_timeline_structure'
        # even after the data has traveled through the GUI layer.
        if final_words:
    ...
    def calculate_timeline_structure(self, words_data, fps, settings):
        ...
        # --- RETRIEVE GLOBAL SILENCE IF AVAILABLE (NEW LOGIC) ---
        # If the project contains the raw metadata (new version), use it for aggressive cutting.
        # This solves the "Check Whole Timeline" requirement.
        raw_silence = words_data[0].get('_meta_global_silence', None)
        
        # Determine silence blocks for SNAP logic (visual fallback is fine for snapping)
        ...
        for i, chunk in enumerate(chunks):
            ...
            if i < len(chunks) - 1:
                ...
                # Snapping logic (Align cuts to nearest VISUAL silence edge)
        ...
        if do_silence_cut or do_silence_mark:
            ...
            # If we have raw global silence data, use it!
            # Otherwise fallback to visual blocks (Legacy projects).
            if raw_silence is not None:
                ...
                # Use visual word blocks {'start', 'end'}
                for s in silence_blocks_for_snap:
