...
class ResolveHandler:
    ...
    def get_timeline_source_info(self):
        """
        Retrieves the MediaPoolItem used as source for the current operation.
        Smart detection: Checks Video track first, then Audio track.
        
        Returns:
            (MediaPoolItem, context_type)
            context_type is 'video' or 'audio' or None.
        """
        if not self.timeline: return None, None
        
        # 1. Try Video Track 1
        v_clips = self.timeline.GetItemListInTrack("video", 1)
        if v_clips:
            # Idea: Try to get clip under playhead, fallback to first
            try:
                current_time = self.timeline.GetCurrentTimecode()
                # Resolve API for GetCurrentTimecode usually returns string HH:MM:SS:FF
                # We won't parse it here to avoid complexity/bugs with FPS.
                # Just take the first clip for robustness as per legacy logic
                return v_clips[0].GetMediaPoolItem(), 'video'
            except:
                return v_clips[0].GetMediaPoolItem(), 'video'
        
        # 2. Try Audio Track 1 (If Video is empty)
        a_clips = self.timeline.GetItemListInTrack("audio", 1)
        if a_clips:
            return a_clips[0].GetMediaPoolItem(), 'audio'
            
        return None, None

    def detect_unsynced_video_items(self):
        """
        Scans Video Track 1 to see if clips are standalone (not linked to audio).
        This detects Adjustment Clips, independent B-Roll, or unlinked video,
        which requires the 'Compound Clip Fix' (Wrapper) mode to process correctly.
        """
        if not self.timeline: return False
        
        # Get all video items
        v_items = self.timeline.GetItemListInTrack("video", 1) or []
        
        # If no video, it's Audio Only mode, so not unsynced video logic.
        if not v_items: return False
        
        for item in v_items:
            try:
                # GetLinkedItems returns a dict or list of linked items (usually audio)
                # If empty, the video clip is standalone (Adjustment layer, etc.)
                linked = item.GetLinkedItems()
                if not linked:
                    log_info("Detected unsynced video item (e.g. Adjustment Clip/Unlinked).")
                    return True
            except:
                pass
    ...
    def generate_timeline_from_ops(self, ops, source_item, new_tl_name, audio_only_mode=False):
        """
