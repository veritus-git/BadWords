...
:: ==========================================
:: 3. INSTALL DEPENDENCIES (Force Upgrade)
:: ==========================================
...
if "%GPU_MODE%"=="1" (
    echo [INFO] Mode: NVIDIA GPU
    "%VENV_PYTHON%" -m pip install faster-whisper pypdf --no-warn-script-location --force-reinstall --ignore-installed
) else (
    echo [INFO] Mode: CPU
    "%VENV_PYTHON%" -m pip install faster-whisper pypdf --no-warn-script-location --force-reinstall --ignore-installed
)
