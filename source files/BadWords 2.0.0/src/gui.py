...
UPDATED v9.8: 
- Smart Device Selection (Greyed out GPU if missing libs).
- Language Mapping (Display Full Name -> Send Code to Whisper).
- Enhanced ScrollableMenu with disabled states.
- FIXED: Dark Title Bar consistency (removed flawed logic, added persistent Map binding).
- REMOVED: Manual Model Download button (Unified UI for Linux/Windows).
- HOTFIX: Startup Freeze Prevention (Safe HWND handling & Overrideredirect check).
"""
...
# Maps display names to Whisper ISO codes
WHISPER_LANG_MAP = {
    "Auto": "Auto",
    "English": "en",
    "Polish": "pl",
...
# Reverse map for loading saved settings (Code -> Display Name)
WHISPER_CODE_MAP = {v: k for k, v in WHISPER_LANG_MAP.items()}
...
def apply_title_bar_style(window, event=None):
    """
    Forces Windows 10/11 title bar to use dark mode (DWM).
    Enhanced for stability, persistence, and SAFETY (Anti-Freeze).
    """
    if platform.system() != "Windows": return

    # SAFETY CHECK 1: If window has no title bar (overrideredirect), skip DWM logic.
    # Accessing/Setting attributes on a frameless window can be problematic or useless.
    try:
        if window.overrideredirect(): return
    except: 
        return

    try:
        # SAFETY CHECK 2: Do NOT call update_idletasks here if triggered by an event.
        # This prevents infinite recursion loops in the event loop.
        # We rely on the caller to ensure HWND exists (usually true for mapped windows).
        ...
        # Force Redraw ONLY on manual calls (event is None).
        # Doing this inside a <Map> handler (event is not None) can cause flicker loops or freezes.
        if event is None:
...
class SplashScreen(tk.Toplevel):
    def __init__(self, parent):
        ...
        # NOTE: Splash screen has NO title bar (overrideredirect), so we skip styling it.
        # Calling apply_title_bar_style on it is unnecessary and risky.
...
class ScrollableMenu(tk.Toplevel):
    def __init__(self, parent, options, callback, x_anchor, y_anchor, width=150, font_size=10, on_destroy_cb=None):
        ...
        super().__init__(parent)
        self.withdraw()
        self.overrideredirect(True) # Frameless -> No title bar style needed
        ...
        for item in options:
            ...
            lbl = tk.Label(inner_frame, text=f"  {label}", bg=config.MENU_BG, fg=fg_color, 
                           font=self.ui_font, anchor="w", cursor=cursor_style)
...
class CustomMessage(tk.Toplevel):
    def __init__(self, parent, title, message, btn_text="OK", is_error=False):
        ...
        # === FORCE DARK MODE (Safe Method) ===
        self.update_idletasks() # Ensure HWND exists
        apply_title_bar_style(self) # First manual application
        self.bind("<Map>", lambda e: apply_title_bar_style(self, e)) # Persist
        self.deiconify()
        self.focus_set()
...
class CustomConfirm(tk.Toplevel):
    def __init__(self, parent, title, message, yes_text="Yes", no_text="No"):
        ...
        self.bind("<Map>", lambda e: apply_title_bar_style(self, e))
        self.deiconify()
        self.focus_set()
        self.wait_window()
...
class BadWordsGUI:
    def __init__(self, root, engine, resolve_handler):
        ...
        # 1. Update idletasks to ensure HWND exists (Safe update)
        self.root.update_idletasks()
        
        # 2. Apply the dark style manually ONCE (with force redraw)
        apply_title_bar_style(self.root)
        
        # 3. Bind to <Map> event to persist dark mode on restore/show.
        # Pass event argument to signal "Event Triggered" -> No forced redraw loop
        self.root.bind("<Map>", lambda e: apply_title_bar_style(self.root, e))
    ...
    def set_progress(self, value):
        ...
        # ANIMATION STATE LOGIC (Indeterminate)
        if value == -1:
            if prev_val != -1: # Just switched to indeterminate
            ...
            if prev_val == -1: # Just switched from indeterminate
                if self.indeterminate_anim_job:
                    self.root.after_cancel(self.indeterminate_anim_job)
                    self.indeterminate_anim_job = None
            
            # Static update triggers
            self.root.after(0, self._update_status_ui)
            self.root.after(0, self._update_sidebar_status)
    ...
    def _animate_indeterminate(self):
        ...
        # Update position (0.0 to 1.0) with loop
        self.indeterminate_pos += 0.02
        if self.indeterminate_pos > 1.0:
            self.indeterminate_pos = -0.2 # Reset to start (loop)
    ...
    def _update_status_ui(self):
        if hasattr(self, 'status_canvas') and self.status_canvas.winfo_exists(): 
            try:
                ...
                # If indeterminate, skip static rect drawing (animation loop handles it)
                if self.current_progress_val == -1:
                    return
    ...
    def _update_sidebar_status(self):
        if hasattr(self, 'sidebar_status_canvas') and self.sidebar_status_canvas.winfo_exists():
            try:
                ...
                # If indeterminate, skip static rect drawing
                if self.current_progress_val == -1:
                    return
    ...
    def save_project(self):
        try:
            ...
            lang_display = self.var_lang.get()
            lang_code = WHISPER_LANG_MAP.get(lang_display, "Auto")
    ...
    def load_project(self):
        try:
            ...
            self.var_lang.set(WHISPER_CODE_MAP.get(saved_lang_code, "Auto"))
            
            self.var_model.set(s.get("model", ""))
            self.var_device.set(s.get("device", "GPU")) # Note: cleaned up later by smart logic
    ...
    def _start_download_sequence(self, tech_name, on_success):
        # Deprecated logic
        pass
    ...
    def show_config_stage(self):
        ...
        def create_input_row(parent, label, var, values=None, hint=""):
            ...
            if values:
                ...
                def open_menu(event):
                    ...
                    # Values can be strings or tuples (Label, Value, Enabled)
                    menu_options = []
                    for v in values:
                        if isinstance(v, tuple):
                            menu_options.append(v)
                        else:
                            menu_options.append((v, v, True))
        ...
        # USE MAPPING KEYS
        whisper_langs = list(WHISPER_LANG_MAP.keys())
        ...
        def open_menu_model(event):
            ...
            # Map standard strings to tuples (Label, Value, Enabled)
        ...
        # Logic: If NVIDIA libs are missing, disable "GPU" option
        has_nvidia = self.engine.os_doc.has_nvidia_support()
        
        # Options format: (Label, Value, Enabled)
        device_opts = [
            ("Auto", "Auto", True),
            ("GPU" if has_nvidia else "GPU (Not installed)", "GPU", has_nvidia),
            ("CPU", "CPU", True)
        ]
        
        # If GPU was previously selected but now invalid (e.g. driver removed), reset to Auto
        if not has_nvidia and self.var_device.get() == "GPU":
        ...
        # Re-apply dark mode after building UI to ensure title bar wasn't reset
        apply_title_bar_style(self.root)
    ...
    def open_filler_editor(self):
        ...
        editor.bind("<Map>", lambda e: apply_title_bar_style(editor, e))
        editor.deiconify()
    ...
    def run_analysis_pipeline(self):
        ...
        # TRANSLATE DISPLAY LANG -> CODE
        # We need to map the UI display language (e.g., "English") to the Whisper code ("en")
        # WHISPER_LANG_MAP handles this. If not found, default to "Auto".
        display_lang = self.var_lang.get()
        whisper_lang_code = WHISPER_LANG_MAP.get(display_lang, "Auto")
    ...
    def show_reviewer_stage(self):
        ...
        # === FORCE DARK MODE CHECK ===
        # Re-apply dark mode after rebuilding UI
        apply_title_bar_style(self.root)
