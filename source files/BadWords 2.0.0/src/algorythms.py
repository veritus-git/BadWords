...
ROLE: Tool Layer
DESCRIPTION:
CORE LOGIC v6.6 (HEAD SYNC & PDF FIX) - Script vs Transcript Alignment.
Features:
...
class CompareEngineV5:
    ...
    def run(self):
        ...
        print(f"--- STARTING COMPARE v6.6 (Script: {self.s_len}, Trans: {self.t_len}) ---")
    ...
    if target_w.get('is_inaudible'):
        ...
        if new_status is None:
            if w_obj.get('is_inaudible'):
                new_status = 'inaudible'
        
        w_obj['status'] = new_status
        w_obj['selected'] = (new_status == 'bad' or new_status == 'inaudible')
