...
class OSDoctor:
    ...
    def set_always_on_top(self, window_id: int, top: bool) -> bool:
        """
        Sets the window to always on top using native OS API.
        - Windows: WS_EX_TOPMOST via GWL_EXSTYLE and SetWindowPos(HWND_TOPMOST)
        - macOS: NSWindow setLevel: NSFloatingWindowLevel (3)
        ...
        if self.is_win:
            try:
                ...
                # SWP_NOMOVE = 0x0002, SWP_NOSIZE = 0x0001, SWP_NOACTIVATE = 0x0010, SWP_FRAMECHANGED = 0x0020
                hwnd_insert_after = -1 if top else -2
                flags = 0x0002 | 0x0001 | 0x0010 | 0x0020
            ...
            try:
                ...
                event.xclient.data[3] = 1  # normal application
                event.xclient.data[4] = 0

                # SubstructureRedirectMask (0x00100000) | SubstructureNotifyMask (0x00080000)
                mask = 0x00180000
