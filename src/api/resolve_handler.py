...
class ResolveHandler:
    ...
    def get_optimal_source_item(self, original_timeline_name):
        """
        AUTO-SOURCING LOGIC:
        Condition A: Exactly 1 uncut clip on the timeline -> returns the MediaPoolItem of that clip.
        Condition B: Timeline is cut or multi-clip -> returns the entire Timeline from Media Pool as a clip.
        """
        if not self.project or not self.media_pool: 
            return None, None

        # 1. Find the timeline object to count its contents
        target_tl = None
        count = self.project.GetTimelineCount()
        for i in range(1, count + 1):
            tl = self.project.GetTimelineByIndex(i)
        ...
        if total_v_items == 1 and total_a_items <= 1:
            ...
            if v_pool_item is None:
                ...
                if a_pool_item is None:
                    ...
                    # Critical condition: Ensure video and audio originate from the same physical file
                    # Guards against errors when the user manually replaced the audio track
        ...
        # CONDITION B (Entire Timeline as a clip)
        ...
        try:
            ...
            def op_to_clipitems(op, clip_map):
                ...
                for clip in clip_map:
                    ...
                    # falls inside an op (prevents marker / timeline desync).
