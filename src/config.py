#!/usr/bin/env python3
# -*- coding: utf-8 -*-

#Copyright (c) 2026 Szymon Wolarz
#Licensed under the MIT License. See LICENSE file in the project root for full license information.

"""
MODULE: config.py
ROLE: Data Layer / Configuration
DESCRIPTION:
Stores constants, default settings, color palette,
translations (i18n), algorithm parameters, and supported languages data.
This is a pure data store, independent of GUI libraries.
"""

import platform

# ==========================================
# APPLICATION INFO
# ==========================================
APP_NAME = "BadWords"
VERSION = "3.0.1"
POSTHOG_API_KEY = "phc_NRFaTKhPJJE8cBa3o9gXYo2mlktR5qup5tn7PdxuRsr"
POSTHOG_HOST = "https://eu.i.posthog.com"

# ==========================================
# WINDOW & GUI SETTINGS
# ==========================================
# Base dimensions for 100% DPI (96 PPI)
CFG_WINDOW_W_BASE = 400
CFG_WINDOW_H_BASE = 740

def get_system_font_name():
    """
    Returns preferred font depending on the operating system.
    Does not require GUI library (tkinter).
    """
    system = platform.system()
    if system == "Windows":
        return "Segoe UI"
    if system == "Darwin": # macOS
        return "Helvetica Neue"
    # Linux / Fallback
    return "Noto Sans"

UI_FONT_NAME = get_system_font_name()
BASE_FONT_PT = 12 if platform.system() == "Darwin" else 10

def FS(size):
    """Dynamically scales font sizes based on OS."""
    return size + 2 if platform.system() == "Darwin" else size

# ==========================================
# ANALYSIS PARAMETERS
# ==========================================
DEFAULT_BAD_WORDS = ["yyy", "eee", "aaa", "umm", "uh", "ah", "mhm"]
SIMILARITY_THRESHOLD = 0.45

# Default filler/hesitation initial prompt for Whisper transcription
DEFAULT_WHISPER_PROMPT = "yyy, eee, uuu, yhm, yyyy, eeee, aaaa, mhm, aha, umm, uh, ah"

# GOLDEN Verbatim Initial Prompt — DO NOT CHANGE.
# This is the exact prompt from the validated src_old that forces Whisper into
# raw acoustic capture mode: stutters, withdrawals, and broken words are transcribed
# as-is instead of being smoothed into grammatical sentences.
# Used as the default for ai_initial_prompt.
GOLDEN_INITIAL_PROMPT = (
    "Umm, yyy, eh, mmm, tsk, h-h-hello, i... i will check. "
    "I went... I went to the st... store, tak tak."
)

# ==========================================
# PER-LANGUAGE VERBATIM INITIAL PROMPTS
# ==========================================
# Each prompt is tailored to its language's native hesitation/filler phonemes.
# This prevents Whisper from hallucinating foreign filler sounds when operating
# in a specific language mode, and forces raw acoustic capture (stutters, withdrawals,
# broken words) instead of grammatically smoothed output.
# Follows the same philosophy as GOLDEN_INITIAL_PROMPT.
# Key: Whisper ISO language code. "Auto" → falls back to GOLDEN_INITIAL_PROMPT.
WHISPER_PROMPTS = {
    # Afrikaans — generated native fillers
    "af": "uh, uhm... uh, uhm, h-hello, i... i will check. I went... I went to the store, uh.",
    # Amharic — generated native fillers
    "am": "እህ, እም... እህ, እም, h-hello, i... i will check. I went... I went to the store, እህ.",
    # Arabic
    "ar": "يعني، آه، إيه... إيه، صح، أ-أنا، ذهبت... ذهبت إلى، يعني.",
    # Assamese — generated native fillers
    "as": "উম, এহ... উম, এহ, h-hello, i... i will check. I went... I went to the store, উম.",
    # Azerbaijani — generated native fillers
    "az": "ıı, eee... ıı, eee, h-hello, i... i will check. I went... I went to the store, ıı.",
    # Bashkir — generated native fillers
    "ba": "эээ, ммм... эээ, ммм, h-hello, i... i will check. I went... I went to the store, эээ.",
    # Belarusian — generated native fillers
    "be": "эээ, ммм... эээ, ммм, h-hello, i... i will check. I went... I went to the store, эээ.",
    # Bulgarian — generated native fillers
    "bg": "ааа, ъъъ... ааа, ъъъ, h-hello, i... i will check. I went... I went to the store, ааа.",
    # Bengali — generated native fillers
    "bn": "উম, এহ... উম, এহ, h-hello, i... i will check. I went... I went to the store, উম.",
    # Tibetan — generated native fillers
    "bo": "ཨ་མ... ཨ་མ, h-hello, i... i will check. I went... I went to the store, ཨ་མ.",
    # Breton — generated native fillers
    "br": "euh, bah... euh, bah, h-hello, i... i will check. I went... I went to the store, euh.",
    # Bosnian — generated native fillers
    "bs": "ehm, pa... ehm, pa, h-hello, i... i will check. I went... I went to the store, ehm.",
    # Catalan — generated native fillers
    "ca": "eh, hmm... eh, hmm, h-hello, i... i will check. I went... I went to the store, eh.",
    # Czech
    "cs": "Ehm, no, prostě... prostě vlastně, já... já šel, š-šel do, ehm, obchodu.",
    # Welsh — generated native fillers
    "cy": "ym, ych... ym, ych, h-hello, i... i will check. I went... I went to the store, ym.",
    # Danish
    "da": "Øh, altså... altså ligesom, jeg... jeg gik, g-gik til, øh, butikken.",
    # German
    "de": "Ähm, äh, hm, also... also ich, ne, ich woll... wollte sagen, ich bin ge-gegangen, äh, genau.",
    # Greek
    "el": "Εμ, δηλαδή... δηλαδή, ε, εγώ... εγώ πήγα, πή-πήγα στο, εμ, μαγαζί.",
    # English
    "en": "Umm, yyy, eh, mmm, tsk, h-h-hello, i... i will check. I went... I went to the st... store, you know.",
    # Spanish
    "es": "Eh, este... este, o sea, mmm, bueno, yo... yo fui, fu-fui al, al al-almacén, ¿no?",
    # Estonian — generated native fillers
    "et": "ee, mm... ee, mm, h-hello, i... i will check. I went... I went to the store, ee.",
    # Basque — generated native fillers
    "eu": "eh, ba... eh, ba, h-hello, i... i will check. I went... I went to the store, eh.",
    # Persian — generated native fillers
    "fa": "um, eh... um, eh, h-hello, i... i will check. I went... I went to the store, um.",
    # Finnish
    "fi": "Öö, tota... tota niinku, mä... mä menin, me-menin sinne, öö, kauppaan.",
    # Faroese — generated native fillers
    "fo": "øh, hmm... øh, hmm, h-hello, i... i will check. I went... I went to the store, øh.",
    # French
    "fr": "Euh, ben, voilà... voilà donc, hm, j-j'ai, je vou... voulais dire, je suis al-allé, euh, voilà.",
    # Galician — generated native fillers
    "gl": "eh, hmm... eh, hmm, h-hello, i... i will check. I went... I went to the store, eh.",
    # Gujarati — generated native fillers
    "gu": "ઉમ, એહ... ઉમ, એહ, h-hello, i... i will check. I went... I went to the store, ઉમ.",
    # Hausa — generated native fillers
    "ha": "uhm, toh... uhm, toh, h-hello, i... i will check. I went... I went to the store, uhm.",
    # Hawaiian — generated native fillers
    "haw": "ʻō, um... ʻō, um, h-hello, i... i will check. I went... I went to the store, ʻō.",
    # Hebrew
    "he": "אמ, כלומר... כלומר, נו, אני... אני הלכתי, ה-הלכתי ל, אמ, חנות.",
    # Hindi
    "hi": "मतलब, हां, अच्छा, वो... वो, म-मैं गया, म-मतलब दुकान को, हां।",
    # Croatian — generated native fillers
    "hr": "ehm, pa... ehm, pa, h-hello, i... i will check. I went... I went to the store, ehm.",
    # Haitian Creole — generated native fillers
    "ht": "en, um... en, um, h-hello, i... i will check. I went... I went to the store, en.",
    # Hungarian
    "hu": "Hm, tehát... tehát szóval, izé, én... én mentem, me-mentem a, hm, boltba.",
    # Armenian — generated native fillers
    "hy": "ըըը, մմմ... ըըը, մմմ, h-hello, i... i will check. I went... I went to the store, ըըը.",
    # Indonesian — generated native fillers
    "id": "anu, em... anu, em, h-hello, i... i will check. I went... I went to the store, anu.",
    # Icelandic — generated native fillers
    "is": "öh, hmm... öh, hmm, h-hello, i... i will check. I went... I went to the store, öh.",
    # Italian
    "it": "Ehm, cioè... cioè, beh, allora, io... io vol-volevo dire, sono an-andato, ehm, ecco.",
    # Japanese
    "ja": "えーと、あの、んー、わ-わたしは、い...いきました、そのー。",
    # Javanese — generated native fillers
    "jw": "anu, em... anu, em, h-hello, i... i will check. I went... I went to the store, anu.",
    # Georgian — generated native fillers
    "ka": "აა, მმ... აა, მმ, h-hello, i... i will check. I went... I went to the store, აა.",
    # Kazakh — generated native fillers
    "kk": "эээ, ммм... эээ, ммм, h-hello, i... i will check. I went... I went to the store, эээ.",
    # Khmer — generated native fillers
    "km": "អឺ, អឹម... អឺ, អឹម, h-hello, i... i will check. I went... I went to the store, អឺ.",
    # Kannada — generated native fillers
    "kn": "ಉಮ್, ಎಹ್... ಉಮ್, ಎಹ್, h-hello, i... i will check. I went... I went to the store, ಉಮ್.",
    # Korean
    "ko": "음, 어, 그... 그러니까, 뭐, 나-나는, 갔...갔어요, 음.",
    # Latin — generated native fillers
    "la": "ehem, um... ehem, um, h-hello, i... i will check. I went... I went to the store, ehem.",
    # Luxembourgish — generated native fillers
    "lb": "ehm, majo... ehm, majo, h-hello, i... i will check. I went... I went to the store, ehm.",
    # Lingala — generated native fillers
    "ln": "euh, um... euh, um, h-hello, i... i will check. I went... I went to the store, euh.",
    # Lao — generated native fillers
    "lo": "ເອີ, ອືມ... ເອີ, ອືມ, h-hello, i... i will check. I went... I went to the store, ເອີ.",
    # Lithuanian — generated native fillers
    "lt": "ėė, mm... ėė, mm, h-hello, i... i will check. I went... I went to the store, ėė.",
    # Latvian — generated native fillers
    "lv": "ēē, mm... ēē, mm, h-hello, i... i will check. I went... I went to the store, ēē.",
    # Malagasy — generated native fillers
    "mg": "euh, um... euh, um, h-hello, i... i will check. I went... I went to the store, euh.",
    # Maori — generated native fillers
    "mi": "ā, um... ā, um, h-hello, i... i will check. I went... I went to the store, ā.",
    # Macedonian — generated native fillers
    "mk": "ааа, еее... ааа, еее, h-hello, i... i will check. I went... I went to the store, ааа.",
    # Malayalam — generated native fillers
    "ml": "ഉം, ഏഹ്... ഉം, ഏഹ്, h-hello, i... i will check. I went... I went to the store, ഉം.",
    # Mongolian — generated native fillers
    "mn": "ээ, мм... ээ, мм, h-hello, i... i will check. I went... I went to the store, ээ.",
    # Marathi — generated native fillers
    "mr": "उम, एह... उम, एह, h-hello, i... i will check. I went... I went to the store, उम.",
    # Malay — generated native fillers
    "ms": "anu, em... anu, em, h-hello, i... i will check. I went... I went to the store, anu.",
    # Maltese — generated native fillers
    "mt": "ehm, mela... ehm, mela, h-hello, i... i will check. I went... I went to the store, ehm.",
    # Myanmar — generated native fillers
    "my": "အင်း, အဲ... အင်း, အဲ, h-hello, i... i will check. I went... I went to the store, အင်း.",
    # Nepali — generated native fillers
    "ne": "उम, एह... उम, एह, h-hello, i... i will check. I went... I went to the store, उम.",
    # Dutch
    "nl": "Eh, uhm, nou... nou eigenlijk, ik... ik wilde zeggen, ik ben ge-gegaan, uhm, ja.",
    # Norwegian Nynorsk — generated native fillers
    "nn": "øhm, liksom... øhm, liksom, h-hello, i... i will check. I went... I went to the store, øhm.",
    # Norwegian
    "no": "Øhm, eh, liksom... liksom altså, jeg... jeg gikk, g-gikk til, øhm, butikken.",
    # Occitan — generated native fillers
    "oc": "euh, ben... euh, ben, h-hello, i... i will check. I went... I went to the store, euh.",
    # Punjabi — generated native fillers
    "pa": "ਉਮ, ਏਹ... ਉਮ, ਏਹ, h-hello, i... i will check. I went... I went to the store, ਉਮ.",
    # Polish
    "pl": "Yyy, eee, eeee, yyyy, no tak, znaczy... znaczy to jest, wiesz, no... Ja po-pójdę, id... idę do sk... sklepu, no właśnie.",
    # Pashto — generated native fillers
    "ps": "امم, اې... امم, اې, h-hello, i... i will check. I went... I went to the store, امم.",
    # Portuguese
    "pt": "É, né, ahn... ahn, então, eu... eu fui, fu-fui ao, ao ar-armazém, né?",
    # Romanian
    "ro": "Ăă, deci, adică... adică, păi, eu... eu am mers, am-am mers la, ăă, magazin.",
    # Russian
    "ru": "Эм, ну, значит... значит это, вот, я-я пошёл, по-пошёл в, в ма-магазин, ну вот.",
    # Sanskrit — generated native fillers
    "sa": "उम्, एह्... उम्, एह्, h-hello, i... i will check. I went... I went to the store, उम्.",
    # Sindhi — generated native fillers
    "sd": "امم, اې... امم, اې, h-hello, i... i will check. I went... I went to the store, امم.",
    # Sinhala — generated native fillers
    "si": "උම්, එහ්... උම්, එහ්, h-hello, i... i will check. I went... I went to the store, උම්.",
    # Slovak
    "sk": "Ehm, no, teda... teda vlastne, ja... ja som išiel, i-išiel do, ehm, obchodu.",
    # Slovenian — generated native fillers
    "sl": "um, eh... um, eh, h-hello, i... i will check. I went... I went to the store, um.",
    # Shona — generated native fillers
    "sn": "ehm, um... ehm, um, h-hello, i... i will check. I went... I went to the store, ehm.",
    # Somali — generated native fillers
    "so": "ee, um... ee, um, h-hello, i... i will check. I went... I went to the store, ee.",
    # Albanian — generated native fillers
    "sq": "ëëë, hmm... ëëë, hmm, h-hello, i... i will check. I went... I went to the store, ëëë.",
    # Serbian — generated native fillers
    "sr": "ehm, pa... ehm, pa, h-hello, i... i will check. I went... I went to the store, ehm.",
    # Sundanese — generated native fillers
    "su": "euh, em... euh, em, h-hello, i... i will check. I went... I went to the store, euh.",
    # Swedish
    "sv": "Öh, eh, alltså... alltså liksom, jag... jag gick, g-gick till, öh, butiken.",
    # Swahili — generated native fillers
    "sw": "eh, um... eh, um, h-hello, i... i will check. I went... I went to the store, eh.",
    # Tamil — generated native fillers
    "ta": "உம், ஏஹ்... உம், ஏஹ், h-hello, i... i will check. I went... I went to the store, உம்.",
    # Telugu — generated native fillers
    "te": "ఉమ్, ఏహ్... ఉమ్, ఏహ్, h-hello, i... i will check. I went... I went to the store, ఉమ్.",
    # Tajik — generated native fillers
    "tg": "эээ, ммм... эээ, ммм, h-hello, i... i will check. I went... I went to the store, эээ.",
    # Thai — generated native fillers
    "th": "เอ้อ, อืม... เอ้อ, อืม, h-hello, i... i will check. I went... I went to the store, เอ้อ.",
    # Turkmen — generated native fillers
    "tk": "eee, mmm... eee, mmm, h-hello, i... i will check. I went... I went to the store, eee.",
    # Tagalog — generated native fillers
    "tl": "ano, um... ano, um, h-hello, i... i will check. I went... I went to the store, ano.",
    # Turkish
    "tr": "Şey, yani... yani, hm, ben... ben git-gittim, şey, markete, yani.",
    # Tatar — generated native fillers
    "tt": "эээ, ммм... эээ, ммм, h-hello, i... i will check. I went... I went to the store, эээ.",
    # Ukrainian
    "uk": "Ееее, ммм, ну, значить... значить це, тобто, я-я пішов, пі-пішов до, ну, магазину.",
    # Urdu — generated native fillers
    "ur": "اُم، اے... اُم، اے, h-hello, i... i will check. I went... I went to the store, اُم، اے.",
    # Uzbek — generated native fillers
    "uz": "eee, mmm... eee, mmm, h-hello, i... i will check. I went... I went to the store, eee.",
    # Vietnamese — generated native fillers
    "vi": "ờ, ừm... ờ, ừm, h-hello, i... i will check. I went... I went to the store, ờ.",
    # Yiddish — generated native fillers
    "yi": "אהם, עה... אהם, עה, h-hello, i... i will check. I went... I went to the store, אהם.",
    # Yoruba — generated native fillers
    "yo": "ẹn, um... ẹn, um, h-hello, i... i will check. I went... I went to the store, ẹn.",
    # Chinese
    "zh": "那个，就是，嗯，啊，我...我去了，去-去了那个，那个商店，就是。",
    # Cantonese — generated native fillers
    "yue": "嗰個, 呢... 嗰個, 呢, h-hello, i... i will check. I went... I went to the store, 嗰個.",
}

def get_whisper_prompt_for_lang(lang, user_custom_prompt=None):
    """
    Returns the appropriate Whisper initial prompt for a given transcription language.

    Priority:
      1. User's custom prompt (ai_initial_prompt from settings) — if set and non-empty.
      2. Per-language prompt from WHISPER_PROMPTS — if lang is a known ISO code.
      3. GOLDEN_INITIAL_PROMPT — universal English-based fallback (for Auto or unknown langs).

    Args:
        lang: Whisper ISO language code (e.g. 'pl', 'en', 'de') or None/'Auto' for auto-detect.
        user_custom_prompt: The user's custom ai_initial_prompt value from settings.

    Returns:
        str: The resolved initial prompt string.
    """
    # 1. User has set a non-empty custom prompt — always respect it
    if user_custom_prompt and user_custom_prompt.strip():
        return user_custom_prompt.strip()

    # 2. Specific language selected — look up per-language prompt
    if lang and lang not in (None, "Auto", "auto", "None", ""):
        lang_code = str(lang).lower().strip()
        if lang_code in WHISPER_PROMPTS:
            return WHISPER_PROMPTS[lang_code]

    # 3. Auto-detect or unknown language — use the universal GOLDEN baseline
    return GOLDEN_INITIAL_PROMPT


# Official DaVinci Resolve marker colors
RESOLVE_COLORS = [
    "Orange", "Apricot", "Yellow", "Lime", "Green", "Olive",
    "Teal", "Navy", "Blue", "Purple", "Violet",
    "Pink", "Tan", "Beige", "Brown", "Chocolate",
]

# Colors blocked from custom markers (reserved for silence/inaudible/system markers)
RESOLVE_COLORS_BLOCKED = {"Olive", "Violet", "Chocolate", "Navy", "Tan"}

# HEX codes for DaVinci Resolve marker colors
RESOLVE_COLORS_HEX = {
    "Orange":    "#eb6e00",
    "Apricot":   "#ffa833",
    "Yellow":    "#e2a91c",
    "Lime":      "#9fc615",
    "Green":     "#448f64",
    "Olive":     "#808000",
    "Teal":      "#009899",
    "Navy":      "#001F5B",
    "Blue":      "#4376a1",
    "Purple":    "#9973a0",
    "Violet":    "#EE82EE",
    "Pink":      "#e98cb5",
    "Tan":       "#D2B48C",
    "Beige":     "#c6a077",
    "Brown":     "#996600",
    "Chocolate": "#7B3F00",
}

# ==========================================
# USER DATA DEFAULTS (user.json)
# Stores user identity and consent state only.
# ==========================================
DEFAULT_USER_DATA = {
    "uuid": None,               # Hashed machine UUID for telemetry (anonymous)
    "telemetry_opt_in": None,   # None = not yet asked, True/False = user decision
    "telemetry_geo": True,      # Whether to include country/city in telemetry ping
    "last_pinged_version": "",  # Version string of the last successful telemetry ping
}

# ==========================================
# APPLICATION SETTINGS DEFAULTS (settings.json)
# Stores persistent UI and engine preferences.
# DO NOT include track/timeline/model selections here — those belong to main UI state.
# ==========================================
DEFAULT_SETTINGS = {
    # ── Transcript / Editor ──────────────────────────────────────────────────
    "editor_font_family":   get_system_font_name(),
    "editor_font_size":     12,
    "editor_line_height":   12,
    "transcript_font_size":   12,       # legacy alias kept for compatibility
    "transcript_line_height": 12,       # legacy alias kept for compatibility
    "view_mode":            "segmented",
    "transcript_layout":    "segmented",
    "settings_view_mode":   "basic",
    # ── Chunking ────────────────────────────────────────────────────────────
    "chunk_punct_count":    1,
    "chunk_max_words":      15,
    "chunk_lookahead":      3,
    "chunk_min_chars":      7,
    "chunk_min_words":      7,
    "inaud_min_dur":        0.02,
    # ── Audio sync ──────────────────────────────────────────────────────────
    "offset":               0.0,
    "pad":                  0.0,
    "snap_max":             0.0,
    # ── Silence detection ───────────────────────────────────────────────────
    "silence_min_dur":      0.2,    # Minimum silence duration (seconds) for FFmpeg silencedetect
    "silence_threshold_db": -42.0,  # Silence threshold in dB (both standalone and post-transcript)
    "ui_spin_thresh":       -42.0,
    "ui_spin_pad":          0.1,
    "fs_cut_mode":          True,
    "fs_mark_mode":         False,
    "sync_davinci_chapter": True,
    # ── AI / Whisper ────────────────────────────────────────────────────────
    "compute_type":         "int8",
    "ai_compute_type":      "Auto",
    "device":               "auto",
    "ai_initial_prompt":    GOLDEN_INITIAL_PROMPT,
    "ai_ultra_precise":     True,
    "ai_vad_filter":        False,
    "ai_beam_size":             1,
    "ai_temperature":           0.0,
    "ai_condition_on_prev":     False,
    "ai_logprob_threshold":     -1.0,
    "ai_no_speech_threshold":   0.2,
    "ai_patience":              1.0,
    "ai_compression_ratio_threshold": 10.0,
    "ai_no_repeat_ngram_size":  0,
    "ai_regroup":               False,
    "ai_suppress_silence":      False,
    "ai_q_levels":              20,
    "ai_k_size":                5,
    # ── App / UI ────────────────────────────────────────────────────────────
    "gui_lang":             "en",
    "always_on_top":        True,
    "hidden_panels":        [],
    "accent_color":         "green",
    "app_icon":             "default",
    # ── Algorithm tuning ────────────────────────────────────────────────────
    "algo_fuzzy_threshold":   75,
    "algo_retake_lookahead":  80,
    "algo_distance_penalty":  2.0,
    "algo_anchor_depth":      3,
    # ── Keyboard shortcuts ───────────────────────────────────────────────────
    "shortcuts": {
        "mark_red":        "1",
        "mark_blue":       "2",
        "mark_green":      "3",
        "mark_eraser":     "4",
        "jump_to_word":    "Ctrl+RMB",
        "play_stop":       "Space",
        "search":          "Ctrl+F",
        "open_settings":   "Escape",
    },
    # ── Custom markers ───────────────────────────────────────────────────────
    "custom_markers":       [],
    # ── Telemetry ────────────────────────────────────────────────────────────
    "telemetry_opt_in":     False,
    "telemetry_geo":        False,
    # ── Assembly / XML Pipeline ──────────────────────────────────────────────
    # False (default): audio tracks are re-mapped sequentially (A1, A2, A3...)
    # True: original source track indices are preserved in the output timeline
    "xml_preserve_track_order": False,
    # ── Transcript timestamps ───────────────────────────────────────────────
    # False (default): timestamps rounded to nearest second, e.g. [01:08]
    # True: full millisecond precision, e.g. [01:07.986]
    "timestamp_precise": False,
    # ── Auto Updates ─────────────────────────────────────────────────────────
    # True (default): check for new releases on GitHub/GitLab at every startup.
    "auto_check_updates": True,
    # True: silently download and apply updates BEFORE loading BadWords (blocking).
    "auto_update_on_start": False,
    # ── Model Run History ─────────────────────────────────────────────────────
    # Dict mapping model name → True if it was ever successfully run this install.
    # Used to show "first run" hints when loading a model for the first time.
    "models_run_history": {},
}


# Keys in DEFAULT_SETTINGS whose change requires an application restart.
RESTART_REQUIRED_KEYS = ["compute_type", "device", "gui_lang", "ai_compute_type", "app_icon"]

# ==========================================
# COLOR PALETTE
# ==========================================

DARK_PALETTE = {
    "BG_COLOR": "#1c1c1c",
    "SIDEBAR_BG": "#262626",
    "INPUT_BG": "#333333",
    "INPUT_FG": "#ffffff",
    "FG_COLOR": "#d9d9d9",
    "NOTE_COL": "#808080",
    "SEPARATOR_COL": "#404040",
    "FOOTER_COLOR": "#1c1c1c",
    "DISCLAIMER_FG": "#555555",
    "SCROLL_BG": "#2b2b2b",
    "SCROLL_FG": "#555555",
    "SCROLL_ACTIVE": "#777777",
    "MENU_BG": "#2b2b2b",
    "MENU_FG": "#ffffff",
    "GEAR_COLOR": "#a0a0a0",
    "BTN_BG": "#23a559",
    "BTN_FG": "#ffffff",
    "BTN_ACTIVE": "#1e8f4c",
    "BTN_GHOST_BG": "#404040",
    "BTN_GHOST_ACTIVE": "#505050",
    "CANCEL_BG": "#b33a3a",
    "CANCEL_ACTIVE": "#8f2e2e",
    "CHECKBOX_BG": "white",
    "PROGRESS_HEIGHT": 24,
    "PROGRESS_TRACK_COLOR": "#333333",
    "PROGRESS_FILL_COLOR": "#23a559",
    "STATUS_TEXT_COLOR": "#eeeeee"
}

# Module-level defaults populated right away
for _k, _v in DARK_PALETTE.items():
    globals()[_k] = _v

# ── Title Bar (CSD) ────────────────────────────────────────────────────────
COLOR_TITLEBAR_BG    = "#191919"   # default title-bar background
COLOR_TITLEBAR_HOVER = "#2b2b2b"   # button hover (non-close)

# --- Word Marking Colors ---
WORD_NORMAL_FG    = "#dcddde"
WORD_BAD_BG       = "#ed4245" # Red (Filler/Error)
WORD_BAD_FG       = "#ffffff"
WORD_REPEAT_BG    = "#2980b9" # Blue (Repeat)
WORD_REPEAT_FG    = "#ffffff"
WORD_TYPO_BG      = "#27ae60" # Green (Typo)
WORD_TYPO_FG      = "#ffffff"
WORD_HOVER_BG     = "#4f545c"
WORD_MISSING_BG   = "#f1c40f" # Yellow (Missing in audio)
WORD_MISSING_FG   = "#000000"
WORD_INAUDIBLE_BG = "#8B4513" # Brown (Inaudible)
WORD_INAUDIBLE_FG = "#ffffff"

# ==========================================
# SUPPORTED LANGUAGES (WHISPER)
# ==========================================
# Languages that naturally read from Right to Left
RTL_LANGUAGES = {'ar', 'he', 'fa', 'ur', 'yi', 'ps', 'sd'}

# Key: Whisper Code (ISO), Value: Native Name
SUPPORTED_LANGUAGES = {
    "Auto": "Auto",
    "af": "Afrikaans",
    "am": "አማርኛ", # Amharic
    "ar": "العربية", # Arabic
    "as": "অসমীয়া", # Assamese
    "az": "Azərbaycan", # Azerbaijani
    "ba": "Башҡортса", # Bashkir
    "be": "Беларуская", # Belarusian
    "bg": "Български", # Bulgarian
    "bn": "বাংলা", # Bengali
    "bo": "བོད་སྐད་", # Tibetan
    "br": "Brezhoneg", # Breton
    "bs": "Bosanski", # Bosnian
    "ca": "Català", # Catalan
    "cs": "Čeština", # Czech
    "cy": "Cymraeg", # Welsh
    "da": "Dansk", # Danish
    "de": "Deutsch", # German
    "el": "Ελληνικά", # Greek
    "en": "English",
    "es": "Español", # Spanish
    "et": "Eesti", # Estonian
    "eu": "Euskara", # Basque
    "fa": "فارسی", # Persian
    "fi": "Suomi", # Finnish
    "fo": "Føroyskt", # Faroese
    "fr": "Français", # French
    "gl": "Galego", # Galician
    "gu": "ગુજરાતી", # Gujarati
    "ha": "Hausa",
    "haw": "ʻŌlelo Hawaiʻi", # Hawaiian
    "he": "עברית", # Hebrew
    "hi": "हिन्दी", # Hindi
    "hr": "Hrvatski", # Croatian
    "ht": "Kreyòl Ayisyen", # Haitian Creole
    "hu": "Magyar", # Hungarian
    "hy": "Հայերեն", # Armenian
    "id": "Bahasa Indonesia", # Indonesian
    "is": "Íslenska", # Icelandic
    "it": "Italiano", # Italian
    "ja": "日本語", # Japanese
    "jw": "Basa Jawa", # Javanese
    "ka": "ქართული", # Georgian
    "kk": "Қазақша", # Kazakh
    "km": "ភាសាខ្មែរ", # Khmer
    "kn": "ಕನ್ನಡ", # Kannada
    "ko": "한국어", # Korean
    "la": "Latina", # Latin
    "lb": "Lëtzebuergesch", # Luxembourgish
    "ln": "Lingála", # Lingala
    "lo": "ພາສາລາວ", # Lao
    "lt": "Lietuvių", # Lithuanian
    "lv": "Latviešu", # Latvian
    "mg": "Malagasy",
    "mi": "Te Reo Māori", # Maori
    "mk": "Македонски", # Macedonian
    "ml": "മലയാളം", # Malayalam
    "mn": "Монгол", # Mongolian
    "mr": "मराठी", # Marathi
    "ms": "Bahasa Melayu", # Malay
    "mt": "Malti", # Maltese
    "my": "ဗမာစာ", # Myanmar
    "ne": "नेपाली", # Nepali
    "nl": "Nederlands", # Dutch
    "nn": "Norsk nynorsk", # Norwegian Nynorsk
    "no": "Norsk", # Norwegian
    "oc": "Occitan",
    "pa": "ਪੰਜਾਬੀ", # Punjabi
    "pl": "Polski", # Polish
    "ps": "پښتو", # Pashto
    "pt": "Português", # Portuguese
    "ro": "Română", # Romanian
    "ru": "Русский", # Russian
    "sa": "संस्कृतम्", # Sanskrit
    "sd": "سنڌي", # Sindhi
    "si": "සිංහල", # Sinhala
    "sk": "Slovenčina", # Slovak
    "sl": "Slovenščina", # Slovenian
    "sn": "ChiShona", # Shona
    "so": "Soomaali", # Somali
    "sq": "Shqip", # Albanian
    "sr": "Српски", # Serbian
    "su": "Basa Sunda", # Sundanese
    "sv": "Svenska", # Swedish
    "sw": "Kiswahili", # Swahili
    "ta": "தமிழ்", # Tamil
    "te": "తెలుగు", # Telugu
    "tg": "Тоҷикӣ", # Tajik
    "th": "ไทย", # Thai
    "tk": "Türkmen", # Turkmen
    "tl": "Tagalog",
    "tr": "Türkçe", # Turkish
    "tt": "Tatarça", # Tatar
    "uk": "Українська", # Ukrainian
    "ur": "اردو", # Urdu
    "uz": "Oʻzbek", # Uzbek
    "vi": "Tiếng Việt", # Vietnamese
    "yi": "ייִדיש", # Yiddish
    "yo": "Yorùbá", # Yoruba
    "zh": "中文", # Chinese
    "yue": "粵語", # Cantonese
}

# ==========================================
# TRANSLATIONS (I18N)
# ==========================================
# Supported Languages: English (en), Polish (pl), German (de), Spanish (es),
# French (fr), Italian (it), Portuguese (pt), Ukrainian (uk), Dutch (nl), Russian (ru)

SUPPORTED_LANGS = {
    'en': 'English',
    'pl': 'Polski',
    'de': 'Deutsch',
    'es': 'Español',
    'fr': 'Français',
    'it': 'Italiano',
    'pt': 'Português',
    'uk': 'Українська',
    'nl': 'Nederlands',
    'ru': 'Русский'
}
TRANS = {
    'en': {
        "lbl_hidden_start": "Start of audio detected as inaudible, skipped.",
        "btn_show_anyway": "show it anyway",
        "chk_sync_davinci": "Sync DaVinci Timeline on Chapter Switch",
        "tt_refresh_timelines": "Refresh Timelines",
        "tt_reset_to_default": "Reset to Default",
        "tt_advanced_locked": "Available in upcoming subversions of BadWords 3.0",
        "btn_analyze": "Analyze",
        "btn_analyze_compare": "Analyze (Compare)",
        "btn_analyze_standalone": "Analyze (Standalone)",
        "btn_apply": "Apply",
        "btn_assemble": "Assemble",
        "btn_clear": "Clear",
        "btn_close": "Close",
        "btn_export_project": "Export Project",
        "btn_fast_silence_detection": "Fast Silence Detection",
        "btn_import_project": "Import Project",
        "btn_import_script": "Import Script",
        "btn_restart_later": "Restart Later",
        "btn_restart_now": "Restart Now",
        "btn_restore_defaults": "Reset All Settings",
        "btn_run_fast_silence": "Run Fast Silence",
        "btn_save": "Save",
        "btn_back_to_transcription": "Back to Transcription",
        "lbl_font_preview": "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
        "lbl_add_custom_marker": "+ add custom marker...",
        "lbl_cut_silence_directly": "Cut silence directly",
        "lbl_detect_and_cut_silence": "Detect and cut silence",
        "lbl_detect_and_mark_silence": "Detect and mark silence",
        "lbl_device": "Device",
        "lbl_display_mode": "Display Mode",
        "lbl_fast_silence_workspace": "Fast Silence Workspace",
        "lbl_font_size_pt": "Font size (pt)",
        "lbl_initializing": "Initializing...",
        "lbl_lang": "Language",
        "lbl_language": "Language",
        "lbl_view_mode": "Settings View (Basic/Advanced)",
        "lbl_line_spacing_px": "Line spacing (px)",
        "lbl_loading": "Loading...",
        "lbl_mark_filler_words_automat": "Mark filler words automatically",
        "lbl_mark_inaudible_fragments": "Mark inaudible fragments",
        "lbl_mark_silence_with_color": "Mark silence with color",
        "lbl_marking_mode": "Marking Mode",
        "lbl_model": "Model",
        "lbl_offset_s": "Offset (s)",
        "lbl_padding_s": "Padding (s)",
        "lbl_pinned_favorites": "Pinned Favorites",
        "lbl_ripple_delete_red_clips": "<span style='color: #ed4245;'>Ripple Delete</span> red clips",
        "lbl_show_detected_typos": "Show detected typos",
        "lbl_show_inaudible_fragments": "Show inaudible fragments",
        "lbl_silence_threshold_db": "Silence threshold (dB)",
        "lbl_snap_max_s": "Snap max (s)",
        "lbl_threshold_db": "Threshold (dB)",
        "lbl_min_silence_dur": "Min silence duration (s)",
        "lbl_timeline_selection": "Timeline Selection",
        "lbl_tracks_selection": "Track/s Selection",
        "lbl_transcript_font": "Transcript font",
        "lbl_transcription_workspace": "Transcription Workspace",
        "lbl_words": "words",
        "msg_analysis_failed": "Analysis failed.",
        "msg_fast_silence": "Fast Silence",
        "msg_fast_silence_processing_c": "Fast silence processing complete.",
        "msg_no_active_transcription_t": "No active transcription timeline detected.",
        "msg_no_silence_segments_detec": "No silence segments detected.",
        "msg_please_import_or_paste_a": "Please import or paste a script first.",
        "msg_restart_lang": "Language changed. Please restart BadWords.",
        "msg_success": "Success",
        "msg_the_transcription_process": "The transcription process has finished.",
        "msg_timeline_assembled_succes": "Timeline assembled successfully.",
        "msg_title_language_changed": "Language Changed",
        "msg_title_settings": "Settings",
        "msg_title_restore_defaults": "Restore Defaults",
        "msg_restore_defaults_confirm": "Are you sure you want to reset all settings to their default values?",
        "opt_continuous_flow": "Continuous Flow",
        "lbl_error": "Error",
        "txt_detecting_silence": "Detecting silence...",
        "txt_building_data": "Building data structure...",
        "txt_fast_silence_complete": "Fast Silence complete.",
        "txt_slowing_audio": "Slowing audio...",
        "txt_enhancing_audio": "Enhancing audio...",
        "txt_transcribing_pct": "Transcribing...",
        "txt_detecting_source": "Detecting optimal source...",
        "txt_calculating_cuts": "Calculating cuts...",
        "msg_warning": "Warning",
        "opt_segmented_blocks": "Segmented blocks",
        "ph_paste_script_here": "Paste script here...",
        "ph_search": "Search...",
        "rad_blue_retake": "Blue (Retake)",
        "rad_eraser_clear": "Eraser (Clear)",
        "rad_green_typo": "Green (Typo)",
        "rad_red_cut_filler": "Red (Cut/Filler)",
        "tab_appearance": "Appearance",
        "tab_audio_sync": "Audio Sync",
        "tab_general": "General",
        "tool_assembly": "Assembly",
        "tool_filler_words": "Filler Words",
        "tool_main_panel": "Main Panel",
        "tool_mark_inaudible": "Mark Inaudible",
        "tool_quit": "Quit",
        "tool_ripple_delete": "Ripple Delete",
        "tool_script_analysis": "Script Analysis",
        "tool_settings": "Settings",
        "tool_show_inaudible": "Show Inaudible",
        "tool_show_typos": "Show Typos",
        "tool_silence": "Silence",
        "tooltip_clear_all_markings": "Clear all markings",
        "tooltip_dev": "Feature in development",
        "tt_revert_to_default": "Revert to default",
        "txt_assembling_timeline": "Assembling timeline...",
        "txt_finishing": "Finishing...",
        "txt_initializing_analysis": "Initializing analysis...",
        "txt_initializing_assembly": "Initializing assembly...",
        "txt_initializing_fast_silence": "Initializing Fast Silence...",
        "txt_save": "Save",
        "txt_saved": "Saved!",
        "txt_select": "Select...",
        "txt_select_tracks": "Select tracks...",
        "msg_engine_render": "Rendering audio...",
        "msg_engine_check_model": "Checking model...",
        "msg_engine_whisper_run": "Starting Whisper...",
        "msg_engine_whisper_init": "Initializing transcription...",
        "msg_engine_silence": "Silence detection...",
        "msg_engine_processing": "Processing...",
        "msg_engine_init_analysis": "Finalizing data...",
        "msg_engine_fast_silence_detect": "Detecting silence (FFmpeg)...",
        "msg_engine_fast_silence_build": "Building data structure...",
        "msg_engine_fast_silence_done": "Fast Silence complete.",
        "msg_engine_assembly_init": "Initializing Assembly...",
        "msg_engine_assembly_source": "Detecting optimal source...",
        "msg_engine_slow_audio": "Slowing audio...",
        "btn_ok": "OK",
        "btn_view_basic": "Basic View",
        "btn_view_advanced": "Advanced View",
        "lbl_chunk_punct_count": "Punctuation marks per block",
        "msg_restart_lang": "Changes saved. To see the results, please restart BadWords.",
        "msg_restore_title": "Reset All Settings",
        "msg_restore_desc": "This will reset ALL settings to their factory defaults. Are you sure?",
        "msg_clear_title": "Clear Transcript",
        "msg_clear_desc": "Are you sure you want to clear all markings?",
        "opt_continuous_flow": "Continuous Flow",
        "status_render": "Rendering audio...",
        "status_direct_source": "Extracting audio directly from source...",
        "status_slow": "Slowing audio...",
        "status_check_model": "Checking model...",
        "status_downloading_model": "Downloading model...",
        "status_downloading_pct": "Downloading model: {pct}%",
        "first_run_hint_1": "First launch takes a little longer — the AI is caching its weights locally.",
        "first_run_hint_2": "Warming up the local AI engine. Future runs will be much faster!",
        "first_run_hint_3": "The AI is unpacking its virtual bags on your drive. Just a moment.",
        "first_run_hint_4": "Optimizing the AI model for your hardware. This is a one-time process.",
        "first_run_hint_5": "First run always takes longer. Promise the next one will be a breeze!",
        "first_run_hint_6": "Compiling local AI layers. Almost there...",
        "first_run_hint_7": "The AI is getting familiar with your local system.",
        "first_run_hint_8": "Setting things up locally. Perfect time to grab a quick drink.",
        "first_run_hint_9": "Allocating memory for the AI... Initial processing takes some time.",
        "first_run_hint_10": "The AI is adjusting its local parameters for the very first time.",
        "analysis_hint_1": "The AI is listening intensely. Shhh...",
        "analysis_hint_2": "Turning your audio into text at warp speed.",
        "analysis_hint_3": "The AI is mercilessly analyzing every single sigh and pause.",
        "analysis_hint_4": "Extracting words from the sound waves...",
        "analysis_hint_5": "The AI is speed-reading your audio file.",
        "analysis_hint_6": "Translating human noises into actual words.",
        "analysis_hint_7": "Poring over the audio track. No words will escape.",
        "analysis_hint_8": "The AI is sculpting your audio like digital clay.",
        "analysis_hint_9": "Slicing and dicing the audio for perfect synchronization.",
        "analysis_hint_10": "Polishing the transcript. Hang tight!",
        "status_whisper_init": "Initializing transcription...",
        "status_transcribing": "Transcribing...",
        "status_silence": "Silence detection...",
        "status_process": "Processing...",
        "status_finalize": "Finalizing data...",
        "status_assembly_init": "Initializing Assembly...",
        "status_assembly_source": "Detecting optimal source...",
        "status_calc_cuts": "Calculating Cuts...",
        "status_assembly_resolve": "Assembling in Resolve...",
        "status_assembly_clips": "Assembling clips...",
        "msg_quit_title": "Quit BadWords",
        "msg_quit_desc": "Are you sure you want to quit?",
        "btn_yes": "Yes",
        "btn_no": "No",
        "title_telemetry": "BadWords Analytics",
        "msg_telemetry": "Hi! 👋\n\nTo help me improve BadWords, do you agree to send a one-time, 100% anonymous ping after installation / update?\n\nOnly the app version and OS type (e.g., Windows) are sent. Audio files, scripts, or personal data are NEVER collected.\n\nGeolocation (country/city) is optional — uncheck the box below to disable it.\n\nYou can change these preferences anytime in Settings → Telemetry.",
        "btn_telemetry_yes": "I Agree",
        "btn_telemetry_no": "No thanks",
        "chk_telemetry_geo": "Share your general location (country/city)",
        "tab_interface": "Interface",
        "lbl_always_on_top": "Always on top",
        "lbl_hidden_panels": "Hidden panels",
        "btn_export_settings": "Export Settings",
        "btn_import_settings": "Import Settings",
        "tab_algorithms": "Algorithms",
        "tab_shortcuts": "Shortcuts",
        "lbl_algo_fuzzy": "Fuzzy match threshold (%)",
        "lbl_algo_lookahead": "Retake lookahead (words)",
        "lbl_algo_penalty": "Distance penalty",
        "lbl_algo_anchor": "Anchor depth",
        "shortcut_mark_red": "Switch to Red Marker",
        "shortcut_mark_blue": "Switch to Blue Marker",
        "shortcut_mark_green": "Switch to Green Marker",
        "shortcut_mark_eraser": "Switch to Eraser",
        "shortcut_jump_to_word": "Jump to word",
        "shortcut_play_stop": "Play / Stop",
        "shortcut_search": "Search",
        "shortcut_open_settings": "Open Settings",
        "shortcut_none": "(none)",
        "shortcut_custom_marker_fmt": "Switch to \"{name}\" Marker",
        "tab_custom_markers": "Custom Markers",
        "tab_telemetry": "Telemetry",
        "btn_add_marker": "Add Marker",
        "btn_export_markers": "Export",
        "btn_import_markers": "Import",
        "msg_no_markers_to_export": "No custom markers to export.",
        "msg_import_invalid_format": "Invalid file format. Expected a JSON file with a \"custom_markers\" list.",
        "msg_import_no_valid_markers": "No valid markers found in the file.",
        "btn_remove_marker": "Remove Marker",
        "btn_edit_marker": "Edit",
        "resolve_color_orange": "Orange",
        "resolve_color_apricot": "Apricot",
        "resolve_color_yellow": "Yellow",
        "resolve_color_lime": "Lime",
        "resolve_color_green": "Green",

        "resolve_color_teal": "Teal",
        "resolve_color_blue": "Blue",
        "resolve_color_purple": "Purple",
        "resolve_color_pink": "Pink",
        "resolve_color_beige": "Beige",
        "resolve_color_brown": "Brown",
        "placeholder_marker_name": "e.g. My Marker",
        "lbl_marker_name": "Marker name",
        "lbl_marker_color": "Color",
        "chk_telemetry_opt_in": "Enable Anonymous Telemetry",
        "chk_telemetry_geo": "Include Geolocation",
        "msg_telemetry_settings": "We collect minimal anonymous data (OS version, app version) to improve BadWords. No audio or personal files are ever sent.",
        "link_repo_label": "BadWords on GitHub",
        "link_repo_desc": "Browse the source code, report issues, and follow the project",
        "link_coffee_label": "Buy Me a Coffee",
        "link_coffee_desc": "Support the development of BadWords",
        "msg_compute_fail_title": "Hardware Unsupported",
        "msg_compute_fail_desc": "Your CPU/GPU does not support the selected compute type. Please select a different type or use 'Auto'.",

        "lbl_accent_color": "Accent Color",
        "lbl_app_icon": "App Icon",
        "lbl_vad_filter": "VAD Filter",
        "lbl_beam_size": "Beam Size",
        "lbl_temperature": "Temperature",
        "lbl_condition_prev": "Condition on Previous Text",
        "lbl_logprob": "Log-Prob Threshold",
        "lbl_no_speech": "No-Speech Threshold",
        "tab_transcript": "Transcript",
        "tab_ai_engine": "AI Engine",
        "lbl_chunk_max_words": "Max chunk words",
        "lbl_chunk_lookahead": "Lookahead words",
        "lbl_chunk_min_chars": "Min chunk chars",
        "lbl_compute_type": "Compute type",
        "lbl_initial_prompt": "Initial prompt",
        "lbl_patience": "Patience",
        "lbl_compression_ratio": "Compression Ratio",
        "lbl_no_repeat_ngram": "No-Repeat N-Gram Size",
        "lbl_regroup": "Regroup",
        "lbl_suppress_silence": "Suppress Silence",
        "lbl_q_levels": "Q Levels",
        "lbl_k_size": "K Size",
        "msg_restart_required": "Changes applied. Full effect will be visible on the next launch.",
        "msg_restart_lang_pending": "Language changed. Full changes will apply on restart.",
        "msg_unsaved_title": "Unsaved Changes",
        "msg_unsaved_desc": "You have unsaved changes. What would you like to do?\n\n",
        "btn_discard": "Discard",
        "btn_save": "Save",
        "btn_save_all": "Save All",
        "btn_discard_all": "Discard All",
        "btn_cancel": "Cancel",
        "txt_saved": "Saved!",
        # R6: Keys previously hardcoded in gui.py
        "msg_assembly_failed": "Assembly failed. Check the log file.",
        "msg_load_project_failed": "Failed to load project",
        "msg_fs_error": "Standalone Silence Error",
        "search_placeholder": "Search...",
        "search_results_counter_empty": "0/0",
        "search_tooltip_prev": "Previous (Shift+Enter)",
        "search_tooltip_next": "Next (Enter)",
        "search_tooltip_close": "Close (Esc)",
        # ── Standalone Silence Detection (renamed from Fast Silence) ──────────
        "btn_standalone_silence_detection": "Standalone Silence Detection",
        "lbl_standalone_silence_workspace": "Silence Detection Workspace",
        "msg_standalone_silence": "Silence Detection",
        "msg_standalone_silence_processing_c": "Silence detection complete.",
        "btn_run_standalone_silence": "Run Detection",
        "txt_initializing_standalone_silence": "Initializing Silence Detection...",
        # ── Timeline / Track selection ──────────────────────────────────────────
        "msg_no_timelines_detected": "No timelines detected",
        "msg_no_audio_tracks_detected": "No audio tracks detected",
        "txt_all_tracks": "All tracks",
        "msg_transcription_source": "BadWords - Source Timeline: <i>{tl}</i> - Tracks: <i>{tr}</i>",
        "txt_all": "All",
        "lbl_xml_preserve_track_order": "Preserve original audio track order",
        "tt_xml_preserve_track_order": "When enabled, audio tracks keep their source indices in the output timeline (e.g. A3 stays A3). When disabled (default), tracks are re-mapped sequentially starting from A1.",
        "status_assembly_xml_build": "Building XML assembly...",
        "status_assembly_xml_import": "Importing XML timeline...",
        "status_assembly_colors": "Applying clip colors...",
        "lbl_timestamp_precise": "Precise timestamps (ms)",
        "tt_timestamp_precise": "Show full millisecond precision in segment timestamps (e.g. [01:07.986] instead of [01:08]).",
    },
    'pl': {
        "lbl_hidden_start": "Początek audio oznaczony jako niezrozumiały, pominięto.",
        "btn_show_anyway": "pokaż mimo to",
        "chk_sync_davinci": "Synchronizuj oś czasu DaVinci przy zmianie edycji",
        "tt_refresh_timelines": "Odśwież Oś Czasu",
        "tt_reset_to_default": "Resetuj do Domyślnych",
        "tt_advanced_locked": "Dostępne w kolejnych podwersjach BadWords 3.0",
        "btn_analyze": "Analizuj",
        "btn_analyze_compare": "Analizuj (Porównaj)",
        "btn_analyze_standalone": "Analizuj (Bez Skryptu)",
        "btn_apply": "Zastosuj",
        "btn_assemble": "Montuj",
        "btn_clear": "Wyczyść",
        "btn_close": "Zamknij",
        "btn_export_project": "Eksportuj Projekt",
        "btn_fast_silence_detection": "Szybkie Wykrywanie Ciszy",
        "btn_import_project": "Importuj Projekt",
        "btn_import_script": "Importuj Scenariusz",
        "btn_restart_later": "Uruchom Ponownie Później",
        "btn_restart_now": "Uruchom Ponownie Teraz",
        "btn_restore_defaults": "Resetuj Wszystkie Ustawienia",
        "btn_run_fast_silence": "Uruchom Szybką Ciszę",
        "btn_save": "Zapisz",
        "btn_back_to_transcription": "Powrót do Transkrypcji",
        "lbl_font_preview": "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
        "lbl_add_custom_marker": "+ dodaj własny znacznik...",
        "lbl_cut_silence_directly": "Wytnij ciszę bezpośrednio",
        "lbl_detect_and_cut_silence": "Wykryj i wytnij ciszę",
        "lbl_detect_and_mark_silence": "Wykryj i oznacz ciszę",
        "lbl_device": "Urządzenie",
        "lbl_display_mode": "Tryb Wyświetlania",
        "lbl_fast_silence_workspace": "Przestrzeń Szybkiej Ciszy",
        "lbl_font_size_pt": "Rozmiar czcionki (pt)",
        "lbl_initializing": "Inicjalizowanie...",
        "lbl_lang": "Język",
        "lbl_language": "Język",
        "lbl_view_mode": "Widok Ustawień (Podstawowy/Zaawansowany)",
        "lbl_line_spacing_px": "Odstępy linii (px)",
        "lbl_loading": "Ładowanie...",
        "lbl_mark_filler_words_automat": "Oznaczaj wypełniacze automatycznie",
        "lbl_mark_inaudible_fragments": "Oznaczaj niezrozumiałe fragmenty",
        "lbl_mark_silence_with_color": "Oznacz ciszę kolorem",
        "lbl_marking_mode": "Tryb Zaznaczania",
        "lbl_model": "Model",
        "lbl_offset_s": "Przesunięcie (s)",
        "lbl_padding_s": "Margines (s)",
        "lbl_pinned_favorites": "Przypięte Narzędzia",
        "lbl_ripple_delete_red_clips": "Usuń ze zsuwaniem (<span style='color: #ed4245;'>Ripple Delete</span>)",
        "lbl_show_detected_typos": "Pokaż wykryte literówki",
        "lbl_show_inaudible_fragments": "Pokaż niezrozumiałe fragmenty",
        "lbl_silence_threshold_db": "Próg ciszy (dB)",
        "lbl_snap_max_s": "Przyciąganie maks. (s)",
        "lbl_threshold_db": "Próg (dB)",
        "lbl_min_silence_dur": "Min. czas ciszy (s)",
        "lbl_timeline_selection": "Wybór Osi Czasu",
        "lbl_tracks_selection": "Wybór Ścieżek",
        "lbl_transcript_font": "Czcionka transkryptu",
        "lbl_transcription_workspace": "Przestrzeń Transkrypcji",
        "lbl_words": "słów",
        "msg_analysis_failed": "Analiza nie powiodła się.",
        "msg_fast_silence": "Szybka Cisza",
        "msg_fast_silence_processing_c": "Przetwarzanie ciszy zakończone.",
        "msg_no_active_transcription_t": "Nie wykryto aktywnej osi czasu.",
        "msg_no_silence_segments_detec": "Nie wykryto segmentów ciszy.",
        "msg_please_import_or_paste_a": "Najpierw zaimportuj lub wklej scenariusz.",
        "msg_restart_lang": "Zmieniono język. Uruchom BadWords ponownie.",
        "msg_success": "Sukces",
        "msg_the_transcription_process": "Proces transkrypcji został zakończony.",
        "msg_timeline_assembled_succes": "Timeline zmontowany pomyślnie.",
        "msg_title_language_changed": "Zmieniono Język",
        "msg_title_settings": "Ustawienia",
        "msg_title_restore_defaults": "Przywróć Domyślne",
        "msg_restore_defaults_confirm": "Czy na pewno chcesz zresetować wszystkie ustawienia do wartości domyślnych?",
        "opt_continuous_flow": "Ciągły tekst",
        "lbl_error": "Błąd",
        "txt_detecting_silence": "Wykrywanie ciszy...",
        "txt_building_data": "Budowanie struktury danych...",
        "txt_fast_silence_complete": "Szybka Cisza zakończona.",
        "txt_slowing_audio": "Spowalnianie audio...",
        "txt_enhancing_audio": "Ulepszanie audio...",
        "txt_transcribing_pct": "Transkrybowanie...",
        "txt_detecting_source": "Wykrywanie optymalnego źródła...",
        "txt_calculating_cuts": "Obliczanie cięć...",
        "msg_warning": "Ostrzeżenie",
        "opt_segmented_blocks": "Podzielone bloki",
        "ph_paste_script_here": "Wklej scenariusz tutaj...",
        "ph_search": "Szukaj...",
        "rad_blue_retake": "Niebieski (Dubel)",
        "rad_eraser_clear": "Gumka (Wyczyść)",
        "rad_green_typo": "Zielony (Literówka)",
        "rad_red_cut_filler": "Czerwony (Cięcie/Wypełniacz)",
        "tab_appearance": "Wygląd",
        "tab_audio_sync": "Synchronizacja",
        "tab_general": "Ogólne",
        "tool_assembly": "Montaż",
        "tool_filler_words": "Wypełniacze",
        "tool_main_panel": "Panel Główny",
        "tool_mark_inaudible": "Oznacz Niezrozumiałe",
        "tool_quit": "Wyjdź",
        "tool_ripple_delete": "Ripple Delete",
        "tool_script_analysis": "Analiza Skryptu",
        "tool_settings": "Ustawienia",
        "tool_show_inaudible": "Pokaż Niezrozumiałe",
        "tool_show_typos": "Pokaż Literówki",
        "tool_silence": "Cisza",
        "tooltip_clear_all_markings": "Wyczyść wszystkie oznaczenia",
        "tooltip_dev": "Funkcja wciąż w produkcji",
        "tt_revert_to_default": "Przywróć domyślne",
        "txt_assembling_timeline": "Montowanie timeline...",
        "txt_finishing": "Kończenie...",
        "txt_initializing_analysis": "Inicjalizacja analizy...",
        "txt_initializing_assembly": "Inicjalizacja montażu...",
        "txt_initializing_fast_silence": "Inicjalizacja Szybkiej Ciszy...",
        "txt_save": "Zapisz",
        "txt_saved": "Zapisano!",
        "txt_select": "Wybierz...",
        "txt_select_tracks": "Wybierz ścieżki...",
        "msg_engine_render": "Renderowanie audio...",
        "msg_engine_check_model": "Sprawdzanie modelu...",
        "msg_engine_whisper_run": "Uruchamianie Whisper...",
        "msg_engine_whisper_init": "Inicjalizacja transkrypcji...",
        "msg_engine_silence": "Wykrywanie ciszy...",
        "msg_engine_processing": "Przetwarzanie...",
        "msg_engine_init_analysis": "Finalizowanie danych...",
        "msg_engine_fast_silence_detect": "Wykrywanie ciszy (FFmpeg)...",
        "msg_engine_fast_silence_build": "Budowanie struktury danych...",
        "msg_engine_fast_silence_done": "Szybka Cisza zakończona.",
        "msg_engine_assembly_init": "Inicjalizacja montażu...",
        "msg_engine_assembly_source": "Wykrywanie optymalnego źródła...",
        "msg_engine_slow_audio": "Spowalnianie audio...",
        "btn_ok": "OK",
        "btn_view_basic": "Widok Podstawowy",
        "btn_view_advanced": "Widok Zaawansowany",
        "lbl_chunk_punct_count": "Znaki interpunkcyjne na blok",
        "msg_restore_title": "Przywróć Domyślne",
        "msg_restore_desc": "Czy na pewno chcesz zresetować wszystkie ustawienia do wartości domyślnych?",
        "msg_clear_title": "Wyczyść Transkrypt",
        "msg_clear_desc": "Czy na pewno chcesz usunąć wszystkie oznaczenia?",
        "status_render": "Renderowanie audio...",
        "status_direct_source": "Wyodrębnianie audio bezpośrednio ze źródła...",
        "status_slow": "Spowalnianie audio...",
        "status_check_model": "Sprawdzanie modelu...",
        "status_downloading_model": "Pobieranie modelu...",
        "status_downloading_pct": "Pobieranie modelu: {pct}%",
        "first_run_hint_1": "Pierwsze uruchomienie wymaga cierpliwości — AI układa sobie wagi na dysku.",
        "first_run_hint_2": "Rozgrzewamy lokalne obwody AI. Następnym razem pójdzie błyskawicznie!",
        "first_run_hint_3": "AI rozpakowuje swoje wirtualne walizki. Daj mu chwilę.",
        "first_run_hint_4": "AI optymalizuje się pod Twój sprzęt. To proces jednorazowy.",
        "first_run_hint_5": "Pierwszy raz zawsze trwa dłużej. Potem będzie już tylko z górki!",
        "first_run_hint_6": "Kompilujemy lokalne warstwy modelu. Już prawie...",
        "first_run_hint_7": "AI właśnie zapoznaje się z Twoim systemem operacyjnym.",
        "first_run_hint_8": "Zapisujemy pliki tymczasowe. Dobry moment na łyk wody.",
        "first_run_hint_9": "Pierwsza inicjalizacja trwa długo, ale Twoje dane nie opuszczają komputera.",
        "first_run_hint_10": "AI kalibruje swoje lokalne parametry po raz pierwszy.",
        "analysis_hint_1": "AI właśnie wsłuchuje się w Twoje audio. Ciii...",
        "analysis_hint_2": "Przelewamy słowa na papier... to znaczy, na twardy dysk.",
        "analysis_hint_3": "AI bezlitośnie analizuje każde Twoje 'yyy' i 'eee'.",
        "analysis_hint_4": "Zamieniamy fale dźwiękowe na tekst z prędkością światła.",
        "analysis_hint_5": "AI intensywnie myśli, co autor miał na myśli.",
        "analysis_hint_6": "Wyciągamy sens z ludzkiego bełkotu... żartuję, mówisz bardzo wyraźnie.",
        "analysis_hint_7": "Transkrypcja w toku. Żadne słowo nie ucieknie przed AI.",
        "analysis_hint_8": "AI rzeźbi w dźwięku jak Michał Anioł w marmurze.",
        "analysis_hint_9": "Tniemy i dopasowujemy audio prosto pod oś czasu DaVinci.",
        "analysis_hint_10": "Polerujemy transkrypcję na błysk. Zaraz kończymy!",
        "status_whisper_init": "Inicjalizacja transkrypcji...",
        "status_transcribing": "Transkrybowanie...",
        "status_silence": "Wykrywanie ciszy...",
        "status_process": "Przetwarzanie...",
        "status_finalize": "Finalizowanie danych...",
        "status_assembly_init": "Inicjalizacja montażu...",
        "status_assembly_source": "Wykrywanie optymalnego źródła...",
        "status_calc_cuts": "Obliczanie cięć...",
        "status_assembly_resolve": "Montowanie w Resolve...",
        "status_assembly_clips": "Montowanie klipów...",
        "msg_quit_title": "Zamykanie BadWords",
        "msg_quit_desc": "Czy na pewno chcesz wyjść?",
        "btn_yes": "Tak",
        "btn_no": "Nie",
        "title_telemetry": "Statystyki BadWords",
        "msg_telemetry": "Cześć! 👋\n\nAby pomóc mi w rozwoju BadWords, czy zgadzasz się na wysłanie jednorazowego, w 100% anonimowego pingu po instalacji / aktualizacji?\n\nWysyłane są tylko wersja aplikacji i typ systemu (np. Windows). Żadne pliki audio, scenariusze ani dane osobiste NIGDY nie są zbierane.\n\nGeolokalizacja (państwo/miasto) jest opcjonalna — odznacz pole poniżej, aby ją wyłączyć.\n\nMożesz zmienić te ustawienia w dowolnym momencie w Ustawienia → Telemetria.",
        "btn_telemetry_yes": "Zgadzam się",
        "btn_telemetry_no": "Nie, dzięki",
        "chk_telemetry_geo": "Udostępnij swoją ogólną lokalizację (państwo/miasto)",
        "tab_interface": "Interfejs",
        "lbl_always_on_top": "Zawsze na wierzchu",
        "lbl_hidden_panels": "Ukryte panele",
        "btn_export_settings": "Eksportuj Ustawienia",
        "btn_import_settings": "Importuj Ustawienia",
        "tab_algorithms": "Algorytmy",
        "tab_shortcuts": "Skróty klawiszowe",
        "lbl_algo_fuzzy": "Próg dopasowania rozmytego (%)",
        "lbl_algo_lookahead": "Wyprzedzenie retake (słowa)",
        "lbl_algo_penalty": "Kara dystansu",
        "lbl_algo_anchor": "Głębokość kotwicy",
        "shortcut_mark_red": "Czerwony marker",
        "shortcut_mark_blue": "Niebieski marker",
        "shortcut_mark_green": "Zielony marker",
        "shortcut_mark_eraser": "Gumka",
        "shortcut_jump_to_word": "Skocz do słowa",
        "shortcut_play_stop": "Odtwórz / Zatrzymaj",
        "shortcut_search": "Szukaj",
        "shortcut_open_settings": "Otwórz ustawienia",
        "shortcut_none": "(brak)",
        "shortcut_custom_marker_fmt": "Przełącz na marker \"{name}\"",
        "tab_custom_markers": "Własne markery",
        "tab_telemetry": "Telemetria",
        "btn_add_marker": "Dodaj marker",
        "btn_export_markers": "Eksportuj",
        "btn_import_markers": "Importuj",
        "msg_no_markers_to_export": "Brak niestandardowych markerów do eksportu.",
        "msg_import_invalid_format": "Nieprawidłowy format pliku. Oczekiwano pliku JSON z listą \"custom_markers\".",
        "msg_import_no_valid_markers": "W pliku nie znaleziono żadnych prawidłowych markerów.",
        "btn_remove_marker": "Usuń marker",
        "btn_edit_marker": "Edytuj",
        "resolve_color_orange": "Pomarańczowy",
        "resolve_color_apricot": "Morelowy",
        "resolve_color_yellow": "Żółty",
        "resolve_color_lime": "Limonkowy",
        "resolve_color_green": "Zielony",

        "resolve_color_teal": "Morski",
        "resolve_color_blue": "Niebieski",
        "resolve_color_purple": "Fioletowy",
        "resolve_color_pink": "Różowy",
        "resolve_color_beige": "Beżowy",
        "resolve_color_brown": "Brązowy",
        "placeholder_marker_name": "np. Mój marker",
        "lbl_marker_name": "Nazwa markera",
        "lbl_marker_color": "Kolor",
        "chk_telemetry_opt_in": "Włącz anonimową telemetrię",
        "msg_telemetry_settings": "Zbieramy minimalne anonimowe dane (wersja systemu, wersja aplikacji) w celu ulepszenia BadWords. Żadne pliki audio ani osobiste nie są nigdy wysyłane.",
        "link_repo_label": "BadWords na GitHubie",
        "link_repo_desc": "Przeglądaj kod źródłowy, zgłaszaj błędy i śledź projekt",
        "link_coffee_label": "Postaw mi kawę",
        "link_coffee_desc": "Wesprzyj rozwój BadWords",
        "msg_compute_fail_title": "Sprzęt nieobsługiwany",
        "msg_compute_fail_desc": "Twoje CPU/GPU nie obsługuje wybranego typu obliczeń. Wybierz inny typ lub użyj 'Auto'.",
        "lbl_accent_color": "Kolor akcentu",
        "lbl_app_icon": "Ikona aplikacji",
        "lbl_vad_filter": "Filtr VAD",
        "lbl_beam_size": "Rozmiar wiązki",
        "lbl_temperature": "Temperatura",
        "lbl_condition_prev": "Warunkuj na poprzednim tekście",
        "lbl_logprob": "Próg log-prawdopodobieństwa",
        "lbl_no_speech": "Próg braku mowy",
        "tab_transcript": "Transkrypcja",
        "tab_ai_engine": "Silnik AI",
        "lbl_chunk_max_words": "Maks. słów w bloku",
        "lbl_chunk_lookahead": "Słowa wyprzedzenia",
        "lbl_chunk_min_chars": "Min. znaków w bloku",
        "lbl_compute_type": "Tryb obliczeń",
        "lbl_initial_prompt": "Początkowy prompt",
        "lbl_patience": "Cierpliwość",
        "lbl_compression_ratio": "Współczynnik kompresji",
        "lbl_no_repeat_ngram": "Brak powtórzeń N-Gram",
        "lbl_regroup": "Przegrupowanie",
        "lbl_suppress_silence": "Tłumienie ciszy",
        "lbl_q_levels": "Poziomy Q",
        "lbl_k_size": "Rozmiar K",
        "msg_restart_required": "Zastosowano zmiany. Ich pełne zastosowanie będzie widoczne przy następnym uruchomieniu.",
        "msg_restart_lang_pending": "Zmieniono język. Pełne zmiany zostaną zastosowane przy następnym uruchomieniu.",
        "msg_unsaved_title": "Niezapisane zmiany",
        "msg_unsaved_desc": "Masz niezapisane zmiany. Co chcesz zrobić?\n\n",
        "btn_discard": "Odrzuć",
        "btn_save_all": "Zapisz wszystkie",
        "btn_discard_all": "Odrzuć wszystkie",
        "btn_cancel": "Anuluj",
        "msg_assembly_failed": "Montaż nie powiódł się. Sprawdź plik dziennika.",
        "msg_load_project_failed": "Nie udało się załadować projektu",
        "msg_fs_error": "Błąd Detekcji Standalone",
        "search_placeholder": "Szukaj...",
        "search_results_counter_empty": "0/0",
        "search_tooltip_prev": "Poprzedni (Shift+Enter)",
        "search_tooltip_next": "Następny (Enter)",
        "search_tooltip_close": "Zamknij (Esc)",
        # ── Standalone Silence Detection ───────────────────────────────────────
        "btn_fast_silence_detection": "Wykrywanie ciszy",
        "btn_standalone_silence_detection": "Wykrywanie ciszy",
        "lbl_fast_silence_workspace": "Wykrywanie ciszy",
        "lbl_standalone_silence_workspace": "Wykrywanie ciszy",
        "msg_fast_silence": "Wykrywanie ciszy",
        "msg_standalone_silence": "Wykrywanie ciszy",
        "msg_fast_silence_processing_c": "Wykrywanie ciszy zakończone.",
        "msg_standalone_silence_processing_c": "Wykrywanie ciszy zakończone.",
        "btn_run_fast_silence": "Uruchom",
        "btn_run_standalone_silence": "Uruchom",
        "txt_initializing_fast_silence": "Inicjalizacja wykrywania ciszy...",
        "txt_initializing_standalone_silence": "Inicjalizacja wykrywania ciszy...",
        # ── Timeline / Track selection ──────────────────────────────────────────
        "msg_no_timelines_detected": "Nie wykryto osi czasu",
        "msg_no_audio_tracks_detected": "Nie wykryto ścieżek audio",
        "txt_all_tracks": "Wszystkie ścieżki",
        "msg_transcription_source": "BadWords - Źródłowa oś czasu: <i>{tl}</i> - Ścieżki: <i>{tr}</i>",
        "txt_all": "Wszystkie",
        "lbl_xml_preserve_track_order": "Zachowaj oryginalną kolejność ścieżek audio",
        "tt_xml_preserve_track_order": "Gdy włączone, ścieżki audio zachowują oryginalne indeksy w wynikowej osi czasu (np. A3 pozostaje A3). Gdy wyłączone (domyślnie), ścieżki są remapowane kolejno od A1.",
        "status_assembly_xml_build": "Budowanie XML montażu...",
        "status_assembly_xml_import": "Importowanie osi czasu z XML...",
        "status_assembly_colors": "Stosowanie kolorów klipów...",
        "lbl_timestamp_precise": "Dokładne znaczniki czasu (ms)",
        "tt_timestamp_precise": "Pokazuje pełną precyzję milisekundową w znacznikach czasu (np. [01:07.986] zamiast [01:08]).",
    },
    'de': {
        "lbl_hidden_start": "Anfang des Audios als unverständlich erkannt, übersprungen.",
        "btn_show_anyway": "trotzdem anzeigen",
        "chk_sync_davinci": "DaVinci Timeline bei Editionswechsel synchronisieren",
        "tt_refresh_timelines": "Timelines aktualisieren",
        "tt_reset_to_default": "Auf Standard zurücksetzen",
        "tt_advanced_locked": "Verfügbar in kommenden Unterversionen von BadWords 3.0",
        "btn_analyze": "Analysieren",
        "btn_analyze_compare": "Analysieren (Vergleichen)",
        "btn_analyze_standalone": "Analysieren (Ohne Skript)",
        "btn_apply": "Anwenden",
        "btn_assemble": "Montieren",
        "btn_clear": "Löschen",
        "btn_close": "Schließen",
        "btn_export_project": "Projekt exportieren",
        "btn_fast_silence_detection": "Schnelle Stille-Erkennung",
        "btn_import_project": "Projekt importieren",
        "btn_import_script": "Skript importieren",
        "btn_restart_later": "Später neu starten",
        "btn_restart_now": "Jetzt neu starten",
        "btn_restore_defaults": "Alle Einstellungen Zurücksetzen",
        "btn_run_fast_silence": "Schnelle Stille starten",
        "btn_save": "Speichern",
        "btn_back_to_transcription": "Zurück zur Transkription",
        "lbl_font_preview": "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
        "lbl_add_custom_marker": "+ eigenen Marker hinzufügen...",
        "lbl_cut_silence_directly": "Stille direkt schneiden",
        "lbl_detect_and_cut_silence": "Stille erkennen und schneiden",
        "lbl_detect_and_mark_silence": "Stille erkennen und markieren",
        "lbl_device": "Gerät",
        "lbl_display_mode": "Anzeigemodus",
        "lbl_fast_silence_workspace": "Stille-Arbeitsbereich",
        "lbl_font_size_pt": "Schriftgröße (pt)",
        "lbl_initializing": "Initialisiere...",
        "lbl_lang": "Sprache",
        "lbl_language": "Sprache",
        "lbl_view_mode": "Einstellungsansicht (Einfach/Erweitert)",
        "lbl_line_spacing_px": "Zeilenabstand (px)",
        "lbl_loading": "Lade...",
        "lbl_mark_filler_words_automat": "Füllwörter automatisch markieren",
        "lbl_mark_inaudible_fragments": "Unhörbare Fragmente markieren",
        "lbl_mark_silence_with_color": "Stille farbig markieren",
        "lbl_marking_mode": "Markierungsmodus",
        "lbl_model": "Modell",
        "lbl_offset_s": "Versatz (s)",
        "lbl_padding_s": "Polster (s)",
        "lbl_pinned_favorites": "Angeheftete Favoriten",
        "lbl_ripple_delete_red_clips": "Rote Clips automatisch löschen (<span style='color: #ed4245;'>Ripple Delete</span>)",
        "lbl_show_detected_typos": "Erkannte Tippfehler anzeigen",
        "lbl_show_inaudible_fragments": "Unhörbare Fragmente anzeigen",
        "lbl_silence_threshold_db": "Stille-Schwelle (dB)",
        "lbl_snap_max_s": "Einrasten (s)",
        "lbl_threshold_db": "Schwelle (dB)",
        "lbl_min_silence_dur": "Mindest-Stille-Dauer (s)",
        "lbl_timeline_selection": "Timeline-Auswahl",
        "lbl_tracks_selection": "Spur-Auswahl",
        "lbl_transcript_font": "Transkript-Schriftart",
        "lbl_transcription_workspace": "Transkriptions-Arbeitsbereich",
        "lbl_words": "Wörter",
        "msg_analysis_failed": "Analyse fehlgeschlagen.",
        "msg_fast_silence": "Schnelle Stille",
        "msg_fast_silence_processing_c": "Stille-Verarbeitung abgeschlossen.",
        "msg_no_active_transcription_t": "Keine aktive Transkriptions-Timeline erkannt.",
        "msg_no_silence_segments_detec": "Keine Stille-Segmente erkannt.",
        "msg_please_import_or_paste_a": "Bitte zuerst ein Skript einfügen oder importieren.",
        "msg_restart_lang": "Sprache geändert. Bitte BadWords neu starten.",
        "msg_success": "Erfolg",
        "msg_the_transcription_process": "Der Transkriptionsprozess ist abgeschlossen.",
        "msg_timeline_assembled_succes": "Timeline erfolgreich montiert.",
        "msg_title_language_changed": "Sprache geändert",
        "msg_title_settings": "Einstellungen",
        "msg_title_restore_defaults": "Standard wiederherstellen",
        "msg_restore_defaults_confirm": "Möchten Sie wirklich alle Einstellungen auf ihre Standardwerte zurücksetzen?",
        "opt_continuous_flow": "Fließtext",
        "lbl_error": "Fehler",
        "txt_detecting_silence": "Stille erkennen...",
        "txt_building_data": "Datenstruktur aufbauen...",
        "txt_fast_silence_complete": "Schnelle Stille abgeschlossen.",
        "txt_slowing_audio": "Audio verlangsamen...",
        "txt_enhancing_audio": "Audio verbessern...",
        "txt_transcribing_pct": "Transkription läuft...",
        "txt_detecting_source": "Optimale Quelle erkennen...",
        "txt_calculating_cuts": "Schnitte berechnen...",
        "msg_warning": "Warnung",
        "opt_segmented_blocks": "Segmentierte Blöcke",
        "ph_paste_script_here": "Skript hier einfügen...",
        "ph_search": "Suchen...",
        "rad_blue_retake": "Blau (Retake)",
        "rad_eraser_clear": "Radierer (Löschen)",
        "rad_green_typo": "Grün (Tippfehler)",
        "rad_red_cut_filler": "Rot (Schnitt/Füller)",
        "tab_appearance": "Aussehen",
        "tab_audio_sync": "Audio-Sync",
        "tab_general": "Allgemein",
        "tool_assembly": "Montage",
        "tool_filler_words": "Füllwörter",
        "tool_main_panel": "Hauptpanel",
        "tool_mark_inaudible": "Unhörbare markieren",
        "tool_quit": "Beenden",
        "tool_ripple_delete": "Ripple Delete",
        "tool_script_analysis": "Skript-Analyse",
        "tool_settings": "Einstellungen",
        "tool_show_inaudible": "Unhörbare anzeigen",
        "tool_show_typos": "Tippfehler anzeigen",
        "tool_silence": "Stille",
        "tooltip_clear_all_markings": "Alle Markierungen löschen",
        "tooltip_dev": "Funktion noch in Entwicklung",
        "tt_revert_to_default": "Auf Standard zurücksetzen",
        "txt_assembling_timeline": "Timeline wird montiert...",
        "txt_finishing": "Abschließen...",
        "txt_initializing_analysis": "Initialisiere Analyse...",
        "txt_initializing_assembly": "Initialisiere Montage...",
        "txt_initializing_fast_silence": "Initialisiere Schnelle Stille...",
        "txt_save": "Speichern",
        "txt_saved": "Gespeichert!",
        "txt_select": "Auswählen...",
        "txt_select_tracks": "Spuren auswählen...",
        "msg_engine_render": "Audio rendern...",
        "msg_engine_check_model": "Modell prüfen...",
        "msg_engine_whisper_run": "Whisper starten...",
        "msg_engine_whisper_init": "Transkription initiieren...",
        "msg_engine_silence": "Stille erkennen...",
        "msg_engine_processing": "Verarbeiten...",
        "msg_engine_init_analysis": "Daten abschließen...",
        "msg_engine_fast_silence_detect": "Stille erkennen (FFmpeg)...",
        "msg_engine_fast_silence_build": "Datenstruktur aufbauen...",
        "msg_engine_fast_silence_done": "Schnelle Stille abgeschlossen.",
        "msg_engine_assembly_init": "Montage initiieren...",
        "msg_engine_assembly_source": "Optimale Quelle erkennen...",
        "msg_engine_slow_audio": "Audio verlangsamen...",
        "btn_ok": "OK",
        "btn_view_basic": "Einfache Ansicht",
        "btn_view_advanced": "Erweiterte Ansicht",
        "lbl_chunk_punct_count": "Interpunktionszeichen pro Block",
        "msg_restore_title": "Alle Einstellungen Zurücksetzen",
        "msg_restore_desc": "Alle Einstellungen werden auf Standardwerte zurückgesetzt. Bist du sicher?",
        "msg_clear_title": "Transkript löschen",
        "msg_clear_desc": "Möchten Sie wirklich alle Markierungen löschen?",
        "status_render": "Audio wird gerendert...",
        "status_direct_source": "Audio direkt aus der Quelle extrahieren...",
        "status_slow": "Audio wird verlangsamt...",
        "status_check_model": "Modell wird geprüft...",
        "status_downloading_model": "Modell wird heruntergeladen...",
        "status_downloading_pct": "Modell herunterladen: {pct}%",
        "first_run_hint_1": "Der erste Start dauert etwas — die AI richtet sich auf deiner Festplatte ein.",
        "first_run_hint_2": "Lokale AI-Modelle werden aufgewärmt. Danach geht es blitzschnell!",
        "first_run_hint_3": "Die AI packt ihre virtuellen Koffer aus. Einen Moment bitte.",
        "first_run_hint_4": "Die AI wird für deine Hardware optimiert. Das passiert nur einmal.",
        "first_run_hint_5": "Der erste Durchlauf dauert immer länger. Versprochen, danach rennt es!",
        "first_run_hint_6": "Lokale AI-Ebenen werden kompiliert. Fast geschafft...",
        "first_run_hint_7": "Die AI macht sich mit deinem Betriebssystem vertraut.",
        "first_run_hint_8": "Alles wird lokal eingerichtet. Zeit für einen kurzen Schluck Wasser.",
        "first_run_hint_9": "Speicher wird für die AI zugewiesen... Die Initialisierung braucht Zeit.",
        "first_run_hint_10": "Die AI kalibriert zum allerersten Mal ihre lokalen Parameter.",
        "analysis_hint_1": "Die AI spitzt die Ohren. Pssst...",
        "analysis_hint_2": "Wir verwandeln Schallwellen mit Lichtgeschwindigkeit in Text.",
        "analysis_hint_3": "Die AI analysiert jedes deiner 'Ähm' und 'Öhm' gnadenlos.",
        "analysis_hint_4": "Wörter werden aus den Audiospuren extrahiert...",
        "analysis_hint_5": "Die AI liest deine Audiodatei im Schnelldurchlauf.",
        "analysis_hint_6": "Aus menschlichen Geräuschen werden ordentliche Vokabeln.",
        "analysis_hint_7": "Die Tonspur wird durchkämmt. Kein Wort entkommt der AI.",
        "analysis_hint_8": "Die AI formt dein Audio wie ein digitaler Bildhauer.",
        "analysis_hint_9": "Das Audio wird passgenau für die Timeline zugeschnitten.",
        "analysis_hint_10": "Das Transkript wird auf Hochglanz poliert. Gleich fertig!",
        "status_whisper_init": "Transkription wird initialisiert...",
        "status_transcribing": "Transkription läuft...",
        "status_silence": "Stille-Erkennung...",
        "status_process": "Verarbeitung...",
        "status_finalize": "Daten werden finalisiert...",
        "status_assembly_init": "Montage wird initialisiert...",
        "status_assembly_source": "Optimale Quelle wird erkannt...",
        "status_calc_cuts": "Schnitte werden berechnet...",
        "status_assembly_resolve": "Montage in Resolve...",
        "status_assembly_clips": "Clips werden montiert...",
        "msg_quit_title": "BadWords beenden",
        "msg_quit_desc": "Möchten Sie das Programm wirklich beenden?",
        "btn_yes": "Ja",
        "btn_no": "Nein",
        "title_telemetry": "BadWords Statistiken",
        "msg_telemetry": "Hallo! 👋\n\nUm mir bei der Entwicklung von BadWords zu helfen, stimmen Sie zu, nach der Installation / einem Update einen einmaligen, zu 100 % anonymen Ping zu senden?\n\nEs werden nur die App-Version und der OS-Typ gesendet. Audio-Dateien, Skripte oder persönliche Daten werden NIEMALS gesammelt.\n\nDie Geolokalisierung ist optional — deaktivieren Sie das Kästchen unten, um sie auszuschalten.\n\nSie können diese Einstellungen jederzeit unter Einstellungen → Telemetrie ändern.",
        "btn_telemetry_yes": "Ich stimme zu",
        "btn_telemetry_no": "Nein, danke",
        "chk_telemetry_geo": "Allgemeinen Standort teilen (Land/Stadt)",
        "tab_interface": "Oberfläche",
        "lbl_always_on_top": "Immer im Vordergrund",
        "lbl_hidden_panels": "Ausgeblendete Panels",
        "btn_export_settings": "Einstellungen exportieren",
        "btn_import_settings": "Einstellungen importieren",
        "tab_algorithms": "Algorithmen",
        "tab_shortcuts": "Tastaturkürzel",
        "lbl_algo_fuzzy": "Schwellenwert Fuzzy-Matching (%)",
        "lbl_algo_lookahead": "Retake-Vorschau (Wörter)",
        "lbl_algo_penalty": "Distanz-Strafe",
        "lbl_algo_anchor": "Ankertiefe",
        "shortcut_mark_red": "Roten Marker wählen",
        "shortcut_mark_blue": "Blauen Marker wählen",
        "shortcut_mark_green": "Grünen Marker wählen",
        "shortcut_mark_eraser": "Radierer wählen",
        "shortcut_jump_to_word": "Zum Wort springen",
        "shortcut_play_stop": "Wiedergabe / Stopp",
        "shortcut_search": "Suchen",
        "shortcut_open_settings": "Einstellungen öffnen",
        "shortcut_none": "(keine)",
        "shortcut_custom_marker_fmt": "Zu Markierung \"{name}\" wechseln",
        "tab_custom_markers": "Benutzerdefinierte Markierungen",
        "tab_telemetry": "Telemetrie",
        "btn_add_marker": "Markierung hinzufügen",
        "btn_export_markers": "Exportieren",
        "btn_import_markers": "Importieren",
        "msg_no_markers_to_export": "Keine benutzerdefinierten Markierungen zum Exportieren.",
        "msg_import_invalid_format": "Ungültiges Dateiformat. JSON-Datei mit \"custom_markers\"-Liste erwartet.",
        "msg_import_no_valid_markers": "Keine gültigen Markierungen in der Datei gefunden.",
        "btn_remove_marker": "Markierung entfernen",
        "btn_edit_marker": "Bearbeiten",
        "resolve_color_orange": "Orange",
        "resolve_color_apricot": "Aprikose",
        "resolve_color_yellow": "Gelb",
        "resolve_color_lime": "Limette",
        "resolve_color_green": "Grün",

        "resolve_color_teal": "Blaugrün",
        "resolve_color_blue": "Blau",
        "resolve_color_purple": "Lila",
        "resolve_color_pink": "Rosa",
        "resolve_color_beige": "Beige",
        "resolve_color_brown": "Braun",
        "placeholder_marker_name": "z.B. Mein Marker",
        "lbl_marker_name": "Markierungsname",
        "lbl_marker_color": "Farbe",
        "chk_telemetry_opt_in": "Anonyme Telemetrie aktivieren",
        "msg_telemetry_settings": "Wir sammeln minimale anonyme Daten (OS-Version, App-Version), um BadWords zu verbessern. Es werden niemals Audio- oder persönliche Dateien gesendet.",
        "link_repo_label": "BadWords auf GitHub",
        "link_repo_desc": "Quellcode durchsuchen, Probleme melden und das Projekt verfolgen",
        "link_coffee_label": "Kauf mir einen Kaffee",
        "link_coffee_desc": "Unterstütze die Entwicklung von BadWords",
        "msg_compute_fail_title": "Hardware nicht unterstützt",
        "msg_compute_fail_desc": "Ihre CPU/GPU unterstützt den ausgewählten Compute-Typ nicht. Bitte wählen Sie einen anderen Typ oder verwenden Sie 'Auto'.",
        "lbl_accent_color": "Akzentfarbe",
        "lbl_app_icon": "App-Symbol",
        "lbl_vad_filter": "VAD-Filter",
        "lbl_beam_size": "Strahlgröße",
        "lbl_temperature": "Temperatur",
        "lbl_condition_prev": "Auf vorherigem Text konditionieren",
        "lbl_logprob": "Log-Wahrscheinlichkeitsschwelle",
        "lbl_no_speech": "Schwellenwert für Stille",
        "tab_transcript": "Transkript",
        "tab_ai_engine": "KI-Engine",
        "lbl_chunk_max_words": "Max. Wörter pro Block",
        "lbl_chunk_lookahead": "Vorschau-Wörter",
        "lbl_chunk_min_chars": "Min. Zeichen pro Block",
        "lbl_compute_type": "Rechentyp",
        "lbl_initial_prompt": "Anfangs-Prompt",
        "lbl_patience": "Geduld (Patience)",
        "lbl_compression_ratio": "Kompressionsrate",
        "lbl_no_repeat_ngram": "Keine Wiederholung N-Gramm",
        "lbl_regroup": "Neu gruppieren",
        "lbl_suppress_silence": "Stille unterdrücken",
        "lbl_q_levels": "Q-Level",
        "lbl_k_size": "K-Größe",
        "msg_restart_required": "Änderungen angewendet. Volle Wirkung erst nach Neustart sichtbar.",
        "msg_restart_lang_pending": "Sprache geändert. Volle Änderungen gelten nach Neustart.",
        "msg_unsaved_title": "Ungespeicherte Änderungen",
        "msg_unsaved_desc": "Sie haben ungespeicherte Änderungen. Was möchten Sie tun?\n\n",
        "btn_discard": "Verwerfen",
        "btn_save_all": "Alle speichern",
        "btn_discard_all": "Alle verwerfen",
        "btn_cancel": "Abbrechen",
        "msg_assembly_failed": "Montage fehlgeschlagen. Prüfen Sie das Protokoll.",
        "msg_load_project_failed": "Projekt konnte nicht geladen werden",
        "msg_fs_error": "Standalone Silence Fehler",
        "search_placeholder": "Suchen...",
        "search_results_counter_empty": "0/0",
        "search_tooltip_prev": "Vorherige (Umschalt+Eingabetaste)",
        "search_tooltip_next": "Nächste (Eingabetaste)",
        "search_tooltip_close": "Schließen (Esc)",
        # ── Standalone Silence Detection ───────────────────────────────────────
        "btn_fast_silence_detection": "Stille-Erkennung",
        "btn_standalone_silence_detection": "Stille-Erkennung",
        "lbl_fast_silence_workspace": "Stille-Erkennung",
        "lbl_standalone_silence_workspace": "Stille-Erkennung",
        "msg_fast_silence": "Stille-Erkennung",
        "msg_standalone_silence": "Stille-Erkennung",
        "msg_fast_silence_processing_c": "Stille-Erkennung abgeschlossen.",
        "msg_standalone_silence_processing_c": "Stille-Erkennung abgeschlossen.",
        "btn_run_fast_silence": "Starten",
        "btn_run_standalone_silence": "Starten",
        "txt_initializing_fast_silence": "Stille-Erkennung initialisieren...",
        "txt_initializing_standalone_silence": "Stille-Erkennung initialisieren...",
        # ── Timeline / Track selection ──────────────────────────────────────────
        "msg_no_timelines_detected": "Keine Timelines erkannt",
        "msg_no_audio_tracks_detected": "Keine Audiospuren erkannt",
        "txt_all_tracks": "Alle Spuren",
        "msg_transcription_source": "BadWords - Quell-Timeline: <i>{tl}</i> - Spuren: <i>{tr}</i>",
        "txt_all": "Alle",
        "lbl_xml_preserve_track_order": "Originale Audiospurreihenfolge beibehalten",
        "tt_xml_preserve_track_order": "Wenn aktiviert, behalten Audiospuren ihre ursprünglichen Indizes in der Ausgabe-Timeline (z.B. A3 bleibt A3). Wenn deaktiviert (Standard), werden Spuren sequenziell ab A1 neu nummeriert.",
        "status_assembly_xml_build": "XML-Montage wird erstellt...",
        "status_assembly_xml_import": "XML-Timeline wird importiert...",
        "status_assembly_colors": "Clip-Farben werden angewendet...",
        "lbl_timestamp_precise": "Präzise Zeitstempel (ms)",
        "tt_timestamp_precise": "Zeigt Millisekunden-Genauigkeit in Segment-Zeitstempeln (z.B. [01:07.986] statt [01:08]).",
    },
    'es': {
        "lbl_hidden_start": "Inicio del audio detectado como inaudible, omitido.",
        "btn_show_anyway": "mostrar de todos modos",
        "chk_sync_davinci": "Sincronizar línea de tiempo de DaVinci al cambiar de edición",
        "tt_refresh_timelines": "Actualizar líneas de tiempo",
        "tt_reset_to_default": "Restablecer por defecto",
        "tt_advanced_locked": "Disponible en próximas subversiones de BadWords 3.0",
        "btn_analyze": "Analizar",
        "btn_analyze_compare": "Analizar (Comparar)",
        "btn_analyze_standalone": "Analizar (Independiente)",
        "btn_apply": "Aplicar",
        "btn_assemble": "Ensamblar",
        "btn_clear": "Borrar",
        "btn_close": "Cerrar",
        "btn_export_project": "Exportar Proyecto",
        "btn_fast_silence_detection": "Detección Rápida de Silencio",
        "btn_import_project": "Importar Proyecto",
        "btn_import_script": "Importar Guion",
        "btn_restart_later": "Reiniciar más tarde",
        "btn_restart_now": "Reiniciar ahora",
        "btn_restore_defaults": "Restablecer Todo",
        "btn_run_fast_silence": "Ejecutar Silencio Rápido",
        "btn_save": "Guardar",
        "btn_back_to_transcription": "Volver a Transcripción",
        "lbl_font_preview": "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
        "lbl_add_custom_marker": "+ añadir marcador personalizado...",
        "lbl_cut_silence_directly": "Cortar silencio directamente",
        "lbl_detect_and_cut_silence": "Detectar y cortar silencio",
        "lbl_detect_and_mark_silence": "Detectar y marcar silencio",
        "lbl_device": "Dispositivo",
        "lbl_display_mode": "Modo de Visualización",
        "lbl_fast_silence_workspace": "Espacio de Silencio Rápido",
        "lbl_font_size_pt": "Tamaño de fuente (pt)",
        "lbl_initializing": "Inicializando...",
        "lbl_lang": "Idioma",
        "lbl_language": "Idioma",
        "lbl_view_mode": "Vista de Ajustes (Básico/Avanzado)",
        "lbl_line_spacing_px": "Interlineado (px)",
        "lbl_loading": "Cargando...",
        "lbl_mark_filler_words_automat": "Marcar muletillas automáticamente",
        "lbl_mark_inaudible_fragments": "Marcar fragmentos inaudibles",
        "lbl_mark_silence_with_color": "Marcar silencio con color",
        "lbl_marking_mode": "Modo de Marcado",
        "lbl_model": "Modelo",
        "lbl_offset_s": "Desplazamiento (s)",
        "lbl_padding_s": "Margen (s)",
        "lbl_pinned_favorites": "Favoritos Anclados",
        "lbl_ripple_delete_red_clips": "Eliminar clips rojos (<span style='color: #ed4245;'>Ripple Delete</span>)",
        "lbl_show_detected_typos": "Mostrar erratas detectadas",
        "lbl_show_inaudible_fragments": "Mostrar fragmentos inaudibles",
        "lbl_silence_threshold_db": "Umbral de silencio (dB)",
        "lbl_snap_max_s": "Ajuste máx. (s)",
        "lbl_threshold_db": "Umbral (dB)",
        "lbl_min_silence_dur": "Duración mínima de silencio (s)",
        "lbl_timeline_selection": "Selección de Timeline",
        "lbl_tracks_selection": "Selección de Pistas",
        "lbl_transcript_font": "Fuente de transcripción",
        "lbl_transcription_workspace": "Espacio de Transcripción",
        "lbl_words": "palabras",
        "msg_analysis_failed": "Análisis fallido.",
        "msg_fast_silence": "Silencio Rápido",
        "msg_fast_silence_processing_c": "Procesamiento de silencio completo.",
        "msg_no_active_transcription_t": "No se detectó timeline de transcripción activa.",
        "msg_no_silence_segments_detec": "No se detectaron segmentos de silencio.",
        "msg_please_import_or_paste_a": "Por favor, importa o pega un guion primero.",
        "msg_restart_lang": "Idioma cambiado. Por favor, reinicia BadWords.",
        "msg_success": "Éxito",
        "msg_the_transcription_process": "El proceso de transcripción ha terminado.",
        "msg_timeline_assembled_succes": "Timeline ensamblada con éxito.",
        "msg_title_language_changed": "Idioma Cambiado",
        "msg_title_settings": "Configuración",
        "msg_title_restore_defaults": "Restaurar Predeterminados",
        "msg_restore_defaults_confirm": "¿Estás seguro de que deseas restablecer todas las configuraciones a sus valores predeterminados?",
        "opt_continuous_flow": "Flujo Continuo",
        "lbl_error": "Error",
        "txt_detecting_silence": "Detectando silencio...",
        "txt_building_data": "Construyendo estructura de datos...",
        "txt_fast_silence_complete": "Silencio Rápido completado.",
        "txt_slowing_audio": "Ralentizando audio...",
        "txt_enhancing_audio": "Mejorando audio...",
        "txt_transcribing_pct": "Transcribiendo...",
        "txt_detecting_source": "Detectando origen óptimo...",
        "txt_calculating_cuts": "Calculando cortes...",
        "msg_warning": "Advertencia",
        "opt_segmented_blocks": "Bloques segmentados",
        "ph_paste_script_here": "Pega el guion aquí...",
        "ph_search": "Buscar...",
        "rad_blue_retake": "Azul (Retoma)",
        "rad_eraser_clear": "Borrador (Limpiar)",
        "rad_green_typo": "Verde (Errata)",
        "rad_red_cut_filler": "Rojo (Cortar/Relleno)",
        "tab_appearance": "Apariencia",
        "tab_audio_sync": "Sincronización",
        "tab_general": "General",
        "tool_assembly": "Ensamblaje",
        "tool_filler_words": "Muletillas",
        "tool_main_panel": "Panel Principal",
        "tool_mark_inaudible": "Marcar Inaudible",
        "tool_quit": "Salir",
        "tool_ripple_delete": "Ripple Delete",
        "tool_script_analysis": "Análisis de Guion",
        "tool_settings": "Configuración",
        "tool_show_inaudible": "Mostrar Inaudible",
        "tool_show_typos": "Mostrar Erratas",
        "tool_silence": "Silencio",
        "tooltip_clear_all_markings": "Borrar todas las marcas",
        "tooltip_dev": "Función en desarrollo",
        "tt_revert_to_default": "Volver a predeterminado",
        "txt_assembling_timeline": "Ensamblando timeline...",
        "txt_finishing": "Finalizando...",
        "txt_initializing_analysis": "Inicializando análisis...",
        "txt_initializing_assembly": "Inicializando ensamblaje...",
        "txt_initializing_fast_silence": "Inicializando Silencio Rápido...",
        "txt_save": "Guardar",
        "txt_saved": "¡Guardado!",
        "txt_select": "Seleccionar...",
        "txt_select_tracks": "Seleccionar pistas...",
        "msg_engine_render": "Renderizando audio...",
        "msg_engine_check_model": "Comprobando modelo...",
        "msg_engine_whisper_run": "Iniciando Whisper...",
        "msg_engine_whisper_init": "Inicializando transcripción...",
        "msg_engine_silence": "Detección de silencio...",
        "msg_engine_processing": "Procesando...",
        "msg_engine_init_analysis": "Finalizando datos...",
        "msg_engine_fast_silence_detect": "Detectando silencio (FFmpeg)...",
        "msg_engine_fast_silence_build": "Construyendo estructura de datos...",
        "msg_engine_fast_silence_done": "Silencio Rápido completado.",
        "msg_engine_assembly_init": "Inicializando Ensamblaje...",
        "msg_engine_assembly_source": "Detectando origen óptimo...",
        "msg_engine_slow_audio": "Ralentizando audio...",
        "btn_ok": "OK",
        "btn_view_basic": "Vista Básica",
        "btn_view_advanced": "Vista Avanzada",
        "lbl_chunk_punct_count": "Signos de puntuación por bloque",
        "msg_restore_title": "Restablecer Todo",
        "msg_restore_desc": "Se restablecerán TODOS los ajustes a sus valores predeterminados. ¿Estás seguro?",
        "msg_clear_title": "Borrar Transcripción",
        "msg_clear_desc": "¿Estás seguro de que deseas borrar todas las marcas?",
        "status_render": "Renderizando audio...",
        "status_direct_source": "Extrayendo audio directamente de la fuente...",
        "status_slow": "Ralentizando audio...",
        "status_check_model": "Comprobando modelo...",
        "status_downloading_model": "Descargando modelo...",
        "status_downloading_pct": "Descargando modelo: {pct}%",
        "first_run_hint_1": "El primer inicio tarda un poco — la AI está guardando en caché sus datos locales.",
        "first_run_hint_2": "Calentando el motor local de la AI. ¡Las próximas veces volará!",
        "first_run_hint_3": "La AI está deshaciendo sus maletas virtuales en tu disco.",
        "first_run_hint_4": "Optimizando la AI para tu equipo. Este proceso es de una sola vez.",
        "first_run_hint_5": "La primera vez siempre tarda más. ¡Prometido que luego irá como la seda!",
        "first_run_hint_6": "Compilando capas locales de la AI. Ya casi está...",
        "first_run_hint_7": "La AI se está familiarizando con tu sistema local.",
        "first_run_hint_8": "Configurando todo en local. Un buen momento para beber agua.",
        "first_run_hint_9": "Asignando memoria para la AI... Esta inicialización lleva su tiempo.",
        "first_run_hint_10": "La AI está ajustando sus parámetros locales por primera vez.",
        "analysis_hint_1": "La AI está escuchando atentamente. Shhh...",
        "analysis_hint_2": "Convirtiendo tu audio en texto a la velocidad de la luz.",
        "analysis_hint_3": "La AI está contando cada vez que dices 'ehhh' sin piedad.",
        "analysis_hint_4": "Extrayendo palabras de las ondas sonoras...",
        "analysis_hint_5": "La AI está haciendo una lectura rápida de tu archivo de audio.",
        "analysis_hint_6": "Traduciendo el acento a texto puro y duro.",
        "analysis_hint_7": "Revisando la pista de audio. Ni una sola palabra escapará.",
        "analysis_hint_8": "La AI está esculpiendo tu audio como si fuera arcilla digital.",
        "analysis_hint_9": "Cortando y ajustando el audio para una sincronización perfecta.",
        "analysis_hint_10": "Puliendo la transcripción. ¡Aguanta un poco más!",
        "status_whisper_init": "Inicializando transcripción...",
        "status_transcribing": "Transcribiendo...",
        "status_silence": "Detección de silencio...",
        "status_process": "Procesando...",
        "status_finalize": "Finalizando datos...",
        "status_assembly_init": "Inicializando Ensamblaje...",
        "status_assembly_source": "Detectando fuente óptima...",
        "status_calc_cuts": "Calculando cortes...",
        "status_assembly_resolve": "Ensamblando en Resolve...",
        "status_assembly_clips": "Ensamblando clips...",
        "msg_quit_title": "Salir de BadWords",
        "msg_quit_desc": "¿Estás seguro de que quieres salir?",
        "btn_yes": "Sí",
        "btn_no": "No",
        "title_telemetry": "Estadísticas de BadWords",
        "msg_telemetry": "¡Hola! 👋\n\nPara ayudarme a mejorar BadWords, ¿aceptas enviar un ping único y 100 % anónimo después de la instalación / actualización?\n\nSolo se envían la versión de la aplicación y el tipo de SO. NUNCA se recopilan archivos de audio, guiones ni datos personales.\n\nLa geolocalización es opcional; desmarca la casilla de abajo para desactivarla.\n\nPuedes cambiar estas preferencias en cualquier momento en Ajustes → Telemetría.",
        "btn_telemetry_yes": "Acepto",
        "btn_telemetry_no": "No, gracias",
        "chk_telemetry_geo": "Compartir tu ubicación general (país/ciudad)",
        "tab_interface": "Interfaz",
        "lbl_always_on_top": "Siempre visible",
        "lbl_hidden_panels": "Paneles ocultos",
        "btn_export_settings": "Exportar configuración",
        "btn_import_settings": "Importar configuración",
        "tab_algorithms": "Algoritmos",
        "tab_shortcuts": "Atajos de teclado",
        "lbl_algo_fuzzy": "Umbral de coincidencia difusa (%)",
        "lbl_algo_lookahead": "Vista anticipada de retoma (palabras)",
        "lbl_algo_penalty": "Penalización de distancia",
        "lbl_algo_anchor": "Profundidad de anclaje",
        "shortcut_mark_red": "Marcador rojo",
        "shortcut_mark_blue": "Marcador azul",
        "shortcut_mark_green": "Marcador verde",
        "shortcut_mark_eraser": "Goma de borrar",
        "shortcut_jump_to_word": "Saltar a palabra",
        "shortcut_play_stop": "Reproducir / Detener",
        "shortcut_search": "Buscar",
        "shortcut_open_settings": "Abrir ajustes",
        "shortcut_none": "(ninguno)",
        "shortcut_custom_marker_fmt": "Cambiar al marcador \"{name}\"",
        "tab_custom_markers": "Marcadores personalizados",
        "tab_telemetry": "Telemetría",
        "btn_add_marker": "Agregar marcador",
        "btn_export_markers": "Exportar",
        "btn_import_markers": "Importar",
        "msg_no_markers_to_export": "No hay marcadores personalizados para exportar.",
        "msg_import_invalid_format": "Formato de archivo no válido. Se esperaba un archivo JSON con una lista \"custom_markers\".",
        "msg_import_no_valid_markers": "No se encontraron marcadores válidos en el archivo.",
        "btn_remove_marker": "Eliminar marcador",
        "btn_edit_marker": "Editar",
        "resolve_color_orange": "Naranja",
        "resolve_color_apricot": "Albaricoque",
        "resolve_color_yellow": "Amarillo",
        "resolve_color_lime": "Lima",
        "resolve_color_green": "Verde",
        "resolve_color_teal": "Verde azulado",
        "resolve_color_blue": "Azul",
        "resolve_color_purple": "Morado",
        "resolve_color_pink": "Rosa",
        "resolve_color_beige": "Beige",
        "resolve_color_brown": "Marrón",
        "placeholder_marker_name": "ej. Mi marcador",
        "lbl_marker_name": "Nombre de marcador",
        "lbl_marker_color": "Color",
        "chk_telemetry_opt_in": "Activar telemetría anónima",
        "msg_telemetry_settings": "Recopilamos datos anónimos mínimos (versión del SO, versión de la app) para mejorar BadWords. Nunca se envían archivos de audio ni personales.",
        "link_repo_label": "BadWords en GitHub",
        "link_repo_desc": "Explora el código fuente, reporta errores y sigue el proyecto",
        "link_coffee_label": "Invítame un café",
        "link_coffee_desc": "Apoya el desarrollo de BadWords",
        "msg_compute_fail_title": "Hardware no compatible",
        "msg_compute_fail_desc": "Su CPU/GPU no admite el tipo de cómputo seleccionado. Seleccione otro tipo o use 'Auto'.",
        "lbl_accent_color": "Color de acento",
        "lbl_app_icon": "Icono de la aplicación",
        "lbl_vad_filter": "Filtro VAD",
        "lbl_beam_size": "Tamaño de haz",
        "lbl_temperature": "Temperatura",
        "lbl_condition_prev": "Condicionar en texto anterior",
        "lbl_logprob": "Umbral log-prob",
        "lbl_no_speech": "Umbral de silencio",
        "tab_transcript": "Transcripción",
        "tab_ai_engine": "Motor IA",
        "lbl_chunk_max_words": "Máx. palabras por bloque",
        "lbl_chunk_lookahead": "Palabras de anticipación",
        "lbl_chunk_min_chars": "Mín. caracteres por bloque",
        "lbl_compute_type": "Tipo de cómputo",
        "lbl_initial_prompt": "Indicación inicial",
        "lbl_patience": "Paciencia",
        "lbl_compression_ratio": "Ratio de compresión",
        "lbl_no_repeat_ngram": "No repetir N-Gramas",
        "lbl_regroup": "Reagrupar",
        "lbl_suppress_silence": "Suprimir silencio",
        "lbl_q_levels": "Niveles Q",
        "lbl_k_size": "Tamaño K",
        "msg_restart_required": "Cambios aplicados. El efecto completo será visible en el próximo inicio.",
        "msg_restart_lang_pending": "Idioma cambiado. El efecto completo será visible al reiniciar.",
        "msg_unsaved_title": "Cambios no guardados",
        "msg_unsaved_desc": "Tienes cambios no guardados. ¿Qué te gustaría hacer?\n\n",
        "btn_discard": "Descartar",
        "btn_save_all": "Guardar todo",
        "btn_discard_all": "Descartar todo",
        "btn_cancel": "Cancelar",
        "msg_assembly_failed": "Montaje fallido. Revise el registro.",
        "msg_load_project_failed": "Error al cargar el proyecto",
        "msg_fs_error": "Error Standalone Silence",
        "search_placeholder": "Buscar...",
        "search_results_counter_empty": "0/0",
        "search_tooltip_prev": "Anterior (Mayús+Intro)",
        "search_tooltip_next": "Siguiente (Intro)",
        "search_tooltip_close": "Cerrar (Esc)",
        # ── Standalone Silence Detection ───────────────────────────────────────
        "btn_fast_silence_detection": "Detección de silencio",
        "btn_standalone_silence_detection": "Detección de silencio",
        "lbl_fast_silence_workspace": "Detección de silencio",
        "lbl_standalone_silence_workspace": "Detección de silencio",
        "msg_fast_silence": "Detección de silencio",
        "msg_standalone_silence": "Detección de silencio",
        "msg_fast_silence_processing_c": "Detección de silencio completada.",
        "msg_standalone_silence_processing_c": "Detección de silencio completada.",
        "btn_run_fast_silence": "Ejecutar",
        "btn_run_standalone_silence": "Ejecutar",
        "txt_initializing_fast_silence": "Inicializando detección...",
        "txt_initializing_standalone_silence": "Inicializando detección...",
        # ── Timeline / Track selection ──────────────────────────────────────────
        "msg_no_timelines_detected": "No se detectaron timelines",
        "msg_no_audio_tracks_detected": "No se detectaron pistas de audio",
        "txt_all_tracks": "Todas las pistas",
        "msg_transcription_source": "BadWords - Timeline de origen: <i>{tl}</i> - Pistas: <i>{tr}</i>",
        "txt_all": "Todas",
        "lbl_xml_preserve_track_order": "Conservar el orden original de pistas de audio",
        "tt_xml_preserve_track_order": "Cuando está activado, las pistas de audio mantienen sus índices originales en la línea de tiempo de salida (p. ej., A3 sigue siendo A3). Cuando está desactivado (predeterminado), las pistas se reasignan secuencialmente desde A1.",
        "status_assembly_xml_build": "Construyendo ensamblaje XML...",
        "status_assembly_xml_import": "Importando línea de tiempo XML...",
        "status_assembly_colors": "Aplicando colores de clips...",
        "lbl_timestamp_precise": "Marcas de tiempo precisas (ms)",
        "tt_timestamp_precise": "Muestra la precisión completa en milisegundos en las marcas de tiempo (p. ej. [01:07.986] en lugar de [01:08]).",
    },
    'fr': {
        "lbl_hidden_start": "Début de l'audio détecté comme inaudible, ignoré.",
        "btn_show_anyway": "afficher quand même",
        "chk_sync_davinci": "Synchroniser la timeline DaVinci lors du changement d'édition",
        "tt_refresh_timelines": "Rafraîchir les timelines",
        "tt_reset_to_default": "Réinitialiser par défaut",
        "tt_advanced_locked": "Disponible dans les prochaines sous-versions de BadWords 3.0",
        "btn_analyze": "Analyser",
        "btn_analyze_compare": "Analyser (Comparer)",
        "btn_analyze_standalone": "Analyser (Autonome)",
        "btn_apply": "Appliquer",
        "btn_assemble": "Assembler",
        "btn_clear": "Effacer",
        "btn_close": "Fermer",
        "btn_export_project": "Exporter le Projet",
        "btn_fast_silence_detection": "Détection Rapide de Silence",
        "btn_import_project": "Importer le Projet",
        "btn_import_script": "Importer le Script",
        "btn_restart_later": "Redémarrer plus tard",
        "btn_restart_now": "Redémarrer maintenant",
        "btn_restore_defaults": "Réinitialiser Tout",
        "btn_run_fast_silence": "Lancer Silence Rapide",
        "btn_save": "Enregistrer",
        "btn_back_to_transcription": "Retour à la Transcription",
        "lbl_font_preview": "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
        "lbl_add_custom_marker": "+ ajouter un marqueur personnalisé...",
        "lbl_cut_silence_directly": "Couper le silence directement",
        "lbl_detect_and_cut_silence": "Détecter et couper le silence",
        "lbl_detect_and_mark_silence": "Détecter et marquer le silence",
        "lbl_device": "Périphérique",
        "lbl_display_mode": "Mode d'affichage",
        "lbl_fast_silence_workspace": "Espace Silence Rapide",
        "lbl_font_size_pt": "Taille de police (pt)",
        "lbl_initializing": "Initialisation...",
        "lbl_lang": "Langue",
        "lbl_language": "Langue",
        "lbl_view_mode": "Vue Paramètres (Base/Avancée)",
        "lbl_line_spacing_px": "Espacement des lignes (px)",
        "lbl_loading": "Chargement...",
        "lbl_mark_filler_words_automat": "Marquer les mots de remplissage auto",
        "lbl_mark_inaudible_fragments": "Marquer les fragments inaudibles",
        "lbl_mark_silence_with_color": "Marquer le silence en couleur",
        "lbl_marking_mode": "Mode de marquage",
        "lbl_model": "Modèle",
        "lbl_offset_s": "Décalage (s)",
        "lbl_padding_s": "Marge (s)",
        "lbl_pinned_favorites": "Favoris épinglés",
        "lbl_ripple_delete_red_clips": "Supprimer avec raccord (<span style='color: #ed4245;'>Ripple Delete</span>)",
        "lbl_show_detected_typos": "Afficher les coquilles détectées",
        "lbl_show_inaudible_fragments": "Afficher les fragments inaudibles",
        "lbl_silence_threshold_db": "Seuil de silence (dB)",
        "lbl_snap_max_s": "Alignement max (s)",
        "lbl_threshold_db": "Seuil (dB)",
        "lbl_min_silence_dur": "Durée minimale de silence (s)",
        "lbl_timeline_selection": "Sélection de la Timeline",
        "lbl_tracks_selection": "Sélection des Pistes",
        "lbl_transcript_font": "Police de transcription",
        "lbl_transcription_workspace": "Espace de Transcription",
        "lbl_words": "mots",
        "msg_analysis_failed": "L'analyse a échoué.",
        "msg_fast_silence": "Silence Rapide",
        "msg_fast_silence_processing_c": "Traitement du silence terminé.",
        "msg_no_active_transcription_t": "Aucune timeline de transcription active détectée.",
        "msg_no_silence_segments_detec": "Aucun segment de silence détecté.",
        "msg_please_import_or_paste_a": "Veuillez d'abord importer ou coller un script.",
        "msg_restart_lang": "Langue modifiée. Veuillez redémarrer BadWords.",
        "msg_success": "Succès",
        "msg_the_transcription_process": "Le processus de transcription est terminé.",
        "msg_timeline_assembled_succes": "Timeline assemblée avec succès.",
        "msg_title_language_changed": "Langue modifiée",
        "msg_title_settings": "Paramètres",
        "msg_title_restore_defaults": "Restaurer par défaut",
        "msg_restore_defaults_confirm": "Êtes-vous sûr de vouloir réinitialiser tous les paramètres à leurs valeurs par défaut ?",
        "opt_continuous_flow": "Flux continu",
        "lbl_error": "Erreur",
        "txt_detecting_silence": "Détection de silence...",
        "txt_building_data": "Construction de la structure de données...",
        "txt_fast_silence_complete": "Silence Rapide terminé.",
        "txt_slowing_audio": "Ralentissement audio...",
        "txt_enhancing_audio": "Amélioration audio...",
        "txt_transcribing_pct": "Transcription en cours...",
        "txt_detecting_source": "Détection de la source optimale...",
        "txt_calculating_cuts": "Calcul des coupes...",
        "msg_warning": "Avertissement",
        "opt_segmented_blocks": "Blocs segmentés",
        "ph_paste_script_here": "Collez le script ici...",
        "ph_search": "Rechercher...",
        "rad_blue_retake": "Bleu (Reprise)",
        "rad_eraser_clear": "Gomme (Effacer)",
        "rad_green_typo": "Vert (Coquille)",
        "rad_red_cut_filler": "Rouge (Couper/Remplissage)",
        "tab_appearance": "Apparence",
        "tab_audio_sync": "Sync Audio",
        "tab_general": "Général",
        "tool_assembly": "Assemblage",
        "tool_filler_words": "Mots de remplissage",
        "tool_main_panel": "Panneau principal",
        "tool_mark_inaudible": "Marquer inaudible",
        "tool_quit": "Quitter",
        "tool_ripple_delete": "Ripple Delete",
        "tool_script_analysis": "Analyse de Script",
        "tool_settings": "Paramètres",
        "tool_show_inaudible": "Afficher inaudible",
        "tool_show_typos": "Afficher les coquilles",
        "tool_silence": "Silence",
        "tooltip_clear_all_markings": "Effacer toutes les marques",
        "tooltip_dev": "Fonctionnalité en développement",
        "tt_revert_to_default": "Restaurer par défaut",
        "txt_assembling_timeline": "Assemblage de la timeline...",
        "txt_finishing": "Finalisation...",
        "txt_initializing_analysis": "Initialisation de l'analyse...",
        "txt_initializing_assembly": "Initialisation de l'assemblage...",
        "txt_initializing_fast_silence": "Initialisation du Silence Rapide...",
        "txt_save": "Enregistrer",
        "txt_saved": "Enregistré !",
        "txt_select": "Sélectionner...",
        "txt_select_tracks": "Sélectionner les pistes...",
        "msg_engine_render": "Rendu audio en cours...",
        "msg_engine_check_model": "Vérification du modèle...",
        "msg_engine_whisper_run": "Démarrage de Whisper...",
        "msg_engine_whisper_init": "Initialisation de la transcription...",
        "msg_engine_silence": "Détection de silence...",
        "msg_engine_processing": "Traitement...",
        "msg_engine_init_analysis": "Finalisation des données...",
        "msg_engine_fast_silence_detect": "Détection de silence (FFmpeg)...",
        "msg_engine_fast_silence_build": "Construction de la structure...",
        "msg_engine_fast_silence_done": "Silence Rapide terminé.",
        "msg_engine_assembly_init": "Initialisation de l'assemblage...",
        "msg_engine_assembly_source": "Détection de la source optimale...",
        "msg_engine_slow_audio": "Ralentissement audio...",
        "btn_ok": "OK",
        "btn_view_basic": "Vue de base",
        "btn_view_advanced": "Vue avancée",
        "lbl_chunk_punct_count": "Signes de ponctuation par bloc",
        "msg_restore_title": "Réinitialiser Tout",
        "msg_restore_desc": "Tous les paramètres seront réinitialisés aux valeurs d'usine. Êtes-vous sûr ?",
        "msg_clear_title": "Effacer la Transcription",
        "msg_clear_desc": "Êtes-vous sûr de vouloir effacer toutes les marques ?",
        "status_render": "Rendu audio...",
        "status_direct_source": "Extraction directe de l'audio depuis la source...",
        "status_slow": "Ralentissement audio...",
        "status_check_model": "Vérification du modèle...",
        "status_downloading_model": "Téléchargement du modèle...",
        "status_downloading_pct": "Téléchargement du modèle: {pct}%",
        "first_run_hint_1": "Le premier lancement est plus long — l'AI met en cache ses données locales.",
        "first_run_hint_2": "Préchauffage du moteur AI local. Les prochaines fois seront fulgurantes !",
        "first_run_hint_3": "L'AI défait ses valises virtuelles sur votre disque. Un instant.",
        "first_run_hint_4": "Optimisation de l'AI pour votre matériel. C'est un processus unique.",
        "first_run_hint_5": "La première exécution est toujours plus longue. Promis, après ça ira vite !",
        "first_run_hint_6": "Compilation des couches AI locales en cours. Presque terminé...",
        "first_run_hint_7": "L'AI fait connaissance avec votre système d'exploitation.",
        "first_run_hint_8": "Configuration locale en cours. Le moment parfait pour prendre un verre.",
        "first_run_hint_9": "Allocation de mémoire pour l'AI... L'initialisation demande du temps.",
        "first_run_hint_10": "L'AI ajuste ses paramètres locaux pour la toute première fois.",
        "analysis_hint_1": "L'AI écoute très attentivement. Chut...",
        "analysis_hint_2": "Transformation de votre audio en texte à la vitesse de l'éclair.",
        "analysis_hint_3": "L'AI compte méticuleusement chacun de vos 'euh'.",
        "analysis_hint_4": "Extraction des mots à partir des ondes sonores...",
        "analysis_hint_5": "L'AI fait de la lecture rapide sur votre fichier audio.",
        "analysis_hint_6": "Traduction de vos bruits humains en mots intelligibles.",
        "analysis_hint_7": "L'AI épluche la piste audio. Aucun mot ne lui échappera.",
        "analysis_hint_8": "L'AI sculpte votre audio avec une précision chirurgicale.",
        "analysis_hint_9": "Découpage et ajustement de l'audio pour une synchronisation parfaite.",
        "analysis_hint_10": "Peaufinage de la transcription. Restez dans les parages !",
        "status_whisper_init": "Initialisation de la transcription...",
        "status_transcribing": "Transcription en cours...",
        "status_silence": "Détection de silence...",
        "status_process": "Traitement...",
        "status_finalize": "Finalisation des données...",
        "status_assembly_init": "Initialisation de l'assemblage...",
        "status_assembly_source": "Détection de la source optimale...",
        "status_calc_cuts": "Calcul des coupes...",
        "status_assembly_resolve": "Assemblage dans Resolve...",
        "status_assembly_clips": "Assemblage des clips...",
        "msg_quit_title": "Quitter BadWords",
        "msg_quit_desc": "Êtes-vous sûr de vouloir quitter ?",
        "btn_yes": "Oui",
        "btn_no": "Non",
        "title_telemetry": "Statistiques BadWords",
        "msg_telemetry": "Bonjour ! 👋\n\nPour m'aider à améliorer BadWords, acceptez-vous d'envoyer un ping unique et 100 % anonyme après l'installation / la mise à jour ?\n\nSeules la version de l'application et le type d'OS sont envoyés. Aucun fichier audio ou donnée personnelle n'est JAMAIS collecté.\n\nLa géolocalisation est facultative — décochez la case ci-dessous pour la désactiver.\n\nVous pouvez modifier ces préférences à tout moment dans Paramètres → Télémétrie.",
        "btn_telemetry_yes": "J'accepte",
        "btn_telemetry_no": "Non merci",
        "chk_telemetry_geo": "Partager votre position générale (pays/ville)",
        "tab_interface": "Interface",
        "lbl_always_on_top": "Toujours au premier plan",
        "lbl_hidden_panels": "Panneaux masqués",
        "btn_export_settings": "Exporter les paramètres",
        "btn_import_settings": "Importer les paramètres",
        "tab_algorithms": "Algorithmes",
        "tab_shortcuts": "Raccourcis clavier",
        "lbl_algo_fuzzy": "Seuil de correspondance floue (%)",
        "lbl_algo_lookahead": "Anticipation de reprise (mots)",
        "lbl_algo_penalty": "Pénalité de distance",
        "lbl_algo_anchor": "Profondeur d'ancrage",
        "shortcut_mark_red": "Marqueur rouge",
        "shortcut_mark_blue": "Marqueur bleu",
        "shortcut_mark_green": "Marqueur vert",
        "shortcut_mark_eraser": "Gomme",
        "shortcut_jump_to_word": "Aller au mot",
        "shortcut_play_stop": "Lecture / Arrêt",
        "shortcut_search": "Rechercher",
        "shortcut_open_settings": "Ouvrir les paramètres",
        "shortcut_none": "(aucun)",
        "shortcut_custom_marker_fmt": "Passer au marqueur \"{name}\"",
        "tab_custom_markers": "Marqueurs personnalisés",
        "tab_telemetry": "Télémétrie",
        "btn_add_marker": "Ajouter un marqueur",
        "btn_export_markers": "Exporter",
        "btn_import_markers": "Importer",
        "msg_no_markers_to_export": "Aucun marqueur personnalisé à exporter.",
        "msg_import_invalid_format": "Format de fichier invalide. Un fichier JSON avec une liste \"custom_markers\" est attendu.",
        "msg_import_no_valid_markers": "Aucun marqueur valide trouvé dans le fichier.",
        "btn_remove_marker": "Supprimer le marqueur",
        "btn_edit_marker": "Modifier",
        "resolve_color_orange": "Orange",
        "resolve_color_apricot": "Abricot",
        "resolve_color_yellow": "Jaune",
        "resolve_color_lime": "Citron vert",
        "resolve_color_green": "Vert",

        "resolve_color_teal": "Sarcelle",
        "resolve_color_blue": "Bleu",
        "resolve_color_purple": "Violet",
        "resolve_color_pink": "Rose",
        "resolve_color_beige": "Beige",
        "resolve_color_brown": "Brun",
        "placeholder_marker_name": "ex. Mon marqueur",
        "lbl_marker_name": "Nom du marqueur",
        "lbl_marker_color": "Couleur",
        "chk_telemetry_opt_in": "Activer la télémétrie anonyme",
        "msg_telemetry_settings": "Nous collectons des données anonymes minimales (version OS, version app) pour améliorer BadWords. Aucun fichier audio ou personnel n'est jamais envoyé.",
        "link_repo_label": "BadWords sur GitHub",
        "link_repo_desc": "Parcourez le code source, signalez des problèmes et suivez le projet",
        "link_coffee_label": "Offrez-moi un café",
        "link_coffee_desc": "Soutenez le développement de BadWords",
        "msg_compute_fail_title": "Matériel non supporté",
        "msg_compute_fail_desc": "Votre CPU/GPU ne supporte pas le type de calcul sélectionné. Veuillez choisir un autre type ou utiliser 'Auto'.",
        "lbl_accent_color": "Couleur d'accent",
        "lbl_app_icon": "Icône de l'application",
        "lbl_vad_filter": "Filtre VAD",
        "lbl_beam_size": "Taille du faisceau",
        "lbl_temperature": "Température",
        "lbl_condition_prev": "Conditionner sur le texte précédent",
        "lbl_logprob": "Seuil log-prob",
        "lbl_no_speech": "Seuil absence de parole",
        "tab_transcript": "Transcription",
        "tab_ai_engine": "Moteur IA",
        "lbl_chunk_max_words": "Mots max. par bloc",
        "lbl_chunk_lookahead": "Mots d'anticipation",
        "lbl_chunk_min_chars": "Caractères min. par bloc",
        "lbl_compute_type": "Type de calcul",
        "lbl_initial_prompt": "Invite initiale",
        "lbl_patience": "Patience",
        "lbl_compression_ratio": "Ratio de compression",
        "lbl_no_repeat_ngram": "Ne pas répéter N-Grammes",
        "lbl_regroup": "Regrouper",
        "lbl_suppress_silence": "Supprimer le silence",
        "lbl_q_levels": "Niveaux Q",
        "lbl_k_size": "Taille K",
        "msg_restart_required": "Modifications appliquées. L'effet complet sera visible au prochain lancement.",
        "msg_restart_lang_pending": "Langue modifiée. L'effet complet sera visible au redémarrage.",
        "msg_unsaved_title": "Modifications non enregistrées",
        "msg_unsaved_desc": "Vous avez des modifications non enregistrées. Que voulez-vous faire ?\n\n",
        "btn_discard": "Ignorer",
        "btn_save_all": "Tout enregistrer",
        "btn_discard_all": "Tout ignorer",
        "btn_cancel": "Annuler",
        "msg_assembly_failed": "Montage échoué. Vérifiez le journal.",
        "msg_load_project_failed": "Impossible de charger le projet",
        "msg_fs_error": "Erreur Standalone Silence",
        "search_placeholder": "Rechercher...",
        "search_results_counter_empty": "0/0",
        "search_tooltip_prev": "Précédent (Maj+Entrée)",
        "search_tooltip_next": "Suivant (Entrée)",
        "search_tooltip_close": "Fermer (Échap)",
        # ── Standalone Silence Detection ───────────────────────────────────────
        "btn_fast_silence_detection": "Détection de silence",
        "btn_standalone_silence_detection": "Détection de silence",
        "lbl_fast_silence_workspace": "Détection de silence",
        "lbl_standalone_silence_workspace": "Détection de silence",
        "msg_fast_silence": "Détection de silence",
        "msg_standalone_silence": "Détection de silence",
        "msg_fast_silence_processing_c": "Détection de silence terminée.",
        "msg_standalone_silence_processing_c": "Détection de silence terminée.",
        "btn_run_fast_silence": "Lancer",
        "btn_run_standalone_silence": "Lancer",
        "txt_initializing_fast_silence": "Initialisation de la détection...",
        "txt_initializing_standalone_silence": "Initialisation de la détection...",
        # ── Timeline / Track selection ──────────────────────────────────────────
        "msg_no_timelines_detected": "Aucune timeline détectée",
        "msg_no_audio_tracks_detected": "Aucune piste audio détectée",
        "txt_all_tracks": "Toutes les pistes",
        "msg_transcription_source": "BadWords - Timeline source: <i>{tl}</i> - Pistes: <i>{tr}</i>",
        "txt_all": "Toutes",
        "lbl_xml_preserve_track_order": "Conserver l'ordre original des pistes audio",
        "tt_xml_preserve_track_order": "Lorsqu'il est activé, les pistes audio conservent leurs indices d'origine dans la timeline de sortie (ex. A3 reste A3). Lorsqu'il est désactivé (par défaut), les pistes sont renumérotées séquentiellement à partir de A1.",
        "status_assembly_xml_build": "Construction de l'assemblage XML...",
        "status_assembly_xml_import": "Importation de la timeline XML...",
        "status_assembly_colors": "Application des couleurs de clips...",
        "lbl_timestamp_precise": "Horodatages précis (ms)",
        "tt_timestamp_precise": "Affiche la précision en millisecondes dans les horodatages de segments (ex. [01:07.986] au lieu de [01:08]).",
    },
    'it': {
        "lbl_hidden_start": "Inizio dell'audio rilevato come inaudibile, saltato.",
        "btn_show_anyway": "mostra comunque",
        "chk_sync_davinci": "Sincronizza timeline DaVinci al cambio di edizione",
        "tt_refresh_timelines": "Aggiorna le timeline",
        "tt_reset_to_default": "Ripristina impostazioni predefinite",
        "tt_advanced_locked": "Disponibile nelle prossime sottoversioni di BadWords 3.0",
        "btn_analyze": "Analizza",
        "btn_analyze_compare": "Analizza (Confronta)",
        "btn_analyze_standalone": "Analizza (Indipendente)",
        "btn_apply": "Applica",
        "btn_assemble": "Assembla",
        "btn_clear": "Pulisci",
        "btn_close": "Chiudi",
        "btn_export_project": "Esporta Progetto",
        "btn_fast_silence_detection": "Rilevamento Rapido Silenzio",
        "btn_import_project": "Importa Progetto",
        "btn_import_script": "Importa Script",
        "btn_restart_later": "Riavvia più tardi",
        "btn_restart_now": "Riavvia ora",
        "btn_restore_defaults": "Ripristina Tutto",
        "btn_run_fast_silence": "Avvia Silenzio Rapido",
        "btn_save": "Salva",
        "btn_back_to_transcription": "Torna alla Trascrizione",
        "lbl_font_preview": "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
        "lbl_add_custom_marker": "+ aggiungi indicatore personalizzato...",
        "lbl_cut_silence_directly": "Taglia silenzio direttamente",
        "lbl_detect_and_cut_silence": "Rileva e taglia silenzio",
        "lbl_detect_and_mark_silence": "Rileva e segna silenzio",
        "lbl_device": "Dispositivo",
        "lbl_display_mode": "Modalità di Visualizzazione",
        "lbl_fast_silence_workspace": "Area di Lavoro Silenzio",
        "lbl_font_size_pt": "Dimensione carattere (pt)",
        "lbl_initializing": "Inizializzazione...",
        "lbl_lang": "Lingua",
        "lbl_language": "Lingua",
        "lbl_view_mode": "Vista Impostazioni (Base/Avanzata)",
        "lbl_line_spacing_px": "Interlinea (px)",
        "lbl_loading": "Caricamento...",
        "lbl_mark_filler_words_automat": "Segna riempitivi automaticamente",
        "lbl_mark_inaudible_fragments": "Segna frammenti inudibili",
        "lbl_mark_silence_with_color": "Segna silenzio con colore",
        "lbl_marking_mode": "Modalità Marcatura",
        "lbl_model": "Modello",
        "lbl_offset_s": "Offset (s)",
        "lbl_padding_s": "Margine (s)",
        "lbl_pinned_favorites": "Preferiti Fissati",
        "lbl_ripple_delete_red_clips": "Elimina clip rosse (<span style='color: #ed4245;'>Ripple Delete</span>)",
        "lbl_show_detected_typos": "Mostra refusi rilevati",
        "lbl_show_inaudible_fragments": "Mostra frammenti inudibili",
        "lbl_silence_threshold_db": "Soglia di silenzio (dB)",
        "lbl_snap_max_s": "Snap max (s)",
        "lbl_threshold_db": "Soglia (dB)",
        "lbl_min_silence_dur": "Durata minima del silenzio (s)",
        "lbl_timeline_selection": "Selezione Timeline",
        "lbl_tracks_selection": "Selezione Tracce",
        "lbl_transcript_font": "Carattere trascrizione",
        "lbl_transcription_workspace": "Area di Trascrizione",
        "lbl_words": "parole",
        "msg_analysis_failed": "Analisi fallita.",
        "msg_fast_silence": "Silenzio Rapido",
        "msg_fast_silence_processing_c": "Elaborazione silenzio completata.",
        "msg_no_active_transcription_t": "Nessuna timeline di trascrizione attiva rilevata.",
        "msg_no_silence_segments_detec": "Nessun segmento di silenzio rilevato.",
        "msg_please_import_or_paste_a": "Per favore importa o incolla prima uno script.",
        "msg_restart_lang": "Lingua cambiata. Per favore riavvia BadWords.",
        "msg_success": "Successo",
        "msg_the_transcription_process": "Il processo di trascrizione è terminato.",
        "msg_timeline_assembled_succes": "Timeline assemblata con successo.",
        "msg_title_language_changed": "Lingua Cambiata",
        "msg_title_settings": "Impostazioni",
        "msg_title_restore_defaults": "Ripristina Predefiniti",
        "msg_restore_defaults_confirm": "Sei sicuro di voler ripristinare tutte le impostazioni ai valori predefiniti?",
        "opt_continuous_flow": "Flusso Continuo",
        "lbl_error": "Errore",
        "txt_detecting_silence": "Rilevamento silenzio...",
        "txt_building_data": "Costruzione struttura dati...",
        "txt_fast_silence_complete": "Silenzio Rapido completato.",
        "txt_slowing_audio": "Rallentamento audio...",
        "txt_enhancing_audio": "Miglioramento audio...",
        "txt_transcribing_pct": "Trascrizione in corso...",
        "txt_detecting_source": "Rilevamento sorgente ottimale...",
        "txt_calculating_cuts": "Calcolo tagli...",
        "msg_warning": "Avviso",
        "opt_segmented_blocks": "Blocchi segmentati",
        "ph_paste_script_here": "Incolla lo script qui...",
        "ph_search": "Cerca...",
        "rad_blue_retake": "Blu (Rifare)",
        "rad_eraser_clear": "Gomma (Pulisci)",
        "rad_green_typo": "Verde (Refuso)",
        "rad_red_cut_filler": "Rosso (Taglio/Riempitivo)",
        "tab_appearance": "Aspetto",
        "tab_audio_sync": "Sync Audio",
        "tab_general": "Generale",
        "tool_assembly": "Assemblaggio",
        "tool_filler_words": "Parole Riempitive",
        "tool_main_panel": "Pannello Principale",
        "tool_mark_inaudible": "Segna Inudibile",
        "tool_quit": "Esci",
        "tool_ripple_delete": "Ripple Delete",
        "tool_script_analysis": "Analisi Script",
        "tool_settings": "Impostazioni",
        "tool_show_inaudible": "Mostra Inudibile",
        "tool_show_typos": "Mostra Refusi",
        "tool_silence": "Silenzio",
        "tooltip_clear_all_markings": "Cancella tutti i segni",
        "tooltip_dev": "Funzionalità in sviluppo",
        "tt_revert_to_default": "Ripristina predefinito",
        "txt_assembling_timeline": "Assemblaggio timeline...",
        "txt_finishing": "Completamento...",
        "txt_initializing_analysis": "Inizializzazione analisi...",
        "txt_initializing_assembly": "Inizializzazione assemblaggio...",
        "txt_initializing_fast_silence": "Inizializzazione Silenzio Rapido...",
        "txt_save": "Salva",
        "txt_saved": "Salvato!",
        "txt_select": "Seleziona...",
        "txt_select_tracks": "Seleziona tracce...",
        "msg_engine_render": "Rendering audio in corso...",
        "msg_engine_check_model": "Controllo modello...",
        "msg_engine_whisper_run": "Avvio Whisper...",
        "msg_engine_whisper_init": "Inizializzazione trascrizione...",
        "msg_engine_silence": "Rilevamento silenzio...",
        "msg_engine_processing": "Elaborazione...",
        "msg_engine_init_analysis": "Finalizzazione dati...",
        "msg_engine_fast_silence_detect": "Rilevamento silenzio (FFmpeg)...",
        "msg_engine_fast_silence_build": "Costruzione struttura dati...",
        "msg_engine_fast_silence_done": "Silenzio Rapido completato.",
        "msg_engine_assembly_init": "Inizializzazione assemblaggio...",
        "msg_engine_assembly_source": "Rilevamento sorgente ottimale...",
        "msg_engine_slow_audio": "Rallentamento audio...",
        "btn_ok": "OK",
        "btn_view_basic": "Vista Base",
        "btn_view_advanced": "Vista Avanzada",
        "lbl_chunk_punct_count": "Segni di punteggiatura per blocco",
        "msg_restore_title": "Ripristina Tutto",
        "msg_restore_desc": "Tutte le impostazioni verranno ripristinate ai valori predefiniti. Sei sicuro?",
        "msg_clear_title": "Cancella Trascrizione",
        "msg_clear_desc": "Sei sicuro di voler cancellare tutti i segni?",
        "status_render": "Rendering audio in corso...",
        "status_direct_source": "Estrazione diretta audio dalla sorgente...",
        "status_slow": "Rallentamento audio in corso...",
        "status_check_model": "Controllo del modello in corso...",
        "status_downloading_model": "Download del modello in corso...",
        "status_downloading_pct": "Download del modello: {pct}%",
        "first_run_hint_1": "Il primo avvio richiede tempo — l'AI sta salvando i suoi dati in locale.",
        "first_run_hint_2": "Riscaldamento del motore AI locale. La prossima volta sarà un razzo!",
        "first_run_hint_3": "L'AI sta disfacendo le sue valigie virtuali sul tuo disco.",
        "first_run_hint_4": "Ottimizzazione dell'AI per il tuo hardware. Succede solo una volta.",
        "first_run_hint_5": "Il primo avvio è sempre il più lungo. Promesso, dopo andrà liscia!",
        "first_run_hint_6": "Compilazione dei layer AI locali. Ci siamo quasi...",
        "first_run_hint_7": "L'AI sta facendo amicizia con il tuo sistema.",
        "first_run_hint_8": "Configurazione locale in corso. Ottimo momento per un sorso d'acqua.",
        "first_run_hint_9": "Allocazione della memoria per l'AI... Serve un po' di pazienza.",
        "first_run_hint_10": "L'AI sta regolando i suoi parametri locali per la prima volta.",
        "analysis_hint_1": "L'AI sta ascoltando il tuo audio con molta attenzione. Shhh...",
        "analysis_hint_2": "Trasformazione dell'audio in testo a velocità curvatura.",
        "analysis_hint_3": "L'AI sta analizzando spietatamente ogni tuo singolo 'ehm'.",
        "analysis_hint_4": "Estrazione delle parole dalle onde sonore in corso...",
        "analysis_hint_5": "L'AI sta leggendo il tuo file audio a velocità pazzesca.",
        "analysis_hint_6": "Traduzione dell'audio in testo, gesticolando virtualmente.",
        "analysis_hint_7": "Scansione della traccia audio. Nessuna parola sfuggirà.",
        "analysis_hint_8": "L'AI sta scolpendo il tuo audio come un artista digitale.",
        "analysis_hint_9": "Taglio e cucito dell'audio per una sincronizzazione perfetta.",
        "analysis_hint_10": "Lucidatura della trascrizione. Resisti, abbiamo quasi finito!",
        "status_whisper_init": "Inizializzazione trascrizione...",
        "status_transcribing": "Trascrizione in corso...",
        "status_silence": "Rilevamento silenzio...",
        "status_process": "Elaborazione in corso...",
        "status_finalize": "Finalizzazione dati...",
        "status_assembly_init": "Inizializzazione Assemblaggio...",
        "status_assembly_source": "Rilevamento sorgente ottimale...",
        "status_calc_cuts": "Calcolo Tagli...",
        "status_assembly_resolve": "Assemblaggio in Resolve...",
        "status_assembly_clips": "Assemblaggio clip...",
        "msg_quit_title": "Esci da BadWords",
        "msg_quit_desc": "Sei sicuro di voler uscire?",
        "btn_yes": "Sì",
        "btn_no": "No",
        "title_telemetry": "Statistiche BadWords",
        "msg_telemetry": "Ciao! 👋\n\nPer aiutarmi a migliorare BadWords, accetti di inviare un ping singolo e al 100% anonimo dopo l'installazione / l'aggiornamento?\n\nVengono inviati solo la versione dell'app e il tipo di SO. NESSUN file audio, script o dato personale viene MAI raccolto.\n\nLa geolocalizzazione è facoltativa — deseleziona la casella qui sotto per disabilitarla.\n\nPuoi modificare queste preferenze in qualsiasi momento in Impostazioni → Telemetria.",
        "btn_telemetry_yes": "Accetto",
        "btn_telemetry_no": "No, grazie",
        "chk_telemetry_geo": "Condividi la tua posizione generale (paese/città)",
        "tab_interface": "Interfaccia",
        "lbl_always_on_top": "Sempre in primo piano",
        "lbl_hidden_panels": "Pannelli nascosti",
        "btn_export_settings": "Esporta impostazioni",
        "btn_import_settings": "Importa impostazioni",
        "tab_algorithms": "Algoritmi",
        "tab_shortcuts": "Scorciatoie da tastiera",
        "lbl_algo_fuzzy": "Soglia di corrispondenza fuzzy (%)",
        "lbl_algo_lookahead": "Preavviso di ripresa (parole)",
        "lbl_algo_penalty": "Penaltà di distanza",
        "lbl_algo_anchor": "Profondità di ancoraggio",
        "shortcut_mark_red": "Marcatore rosso",
        "shortcut_mark_blue": "Marcatore blu",
        "shortcut_mark_green": "Marcatore verde",
        "shortcut_mark_eraser": "Gomma",
        "shortcut_jump_to_word": "Vai alla parola",
        "shortcut_play_stop": "Riproduci / Ferma",
        "shortcut_search": "Cerca",
        "shortcut_open_settings": "Apri impostazioni",
        "shortcut_none": "(nessuno)",
        "shortcut_custom_marker_fmt": "Passa all'indicatore \"{name}\"",
        "tab_custom_markers": "Indicatori personalizzati",
        "tab_telemetry": "Telemetria",
        "btn_add_marker": "Aggiungi indicatore",
        "btn_export_markers": "Esporta",
        "btn_import_markers": "Importa",
        "msg_no_markers_to_export": "Nessun indicatore personalizzato da esportare.",
        "msg_import_invalid_format": "Formato file non valido. File JSON con lista \"custom_markers\" atteso.",
        "msg_import_no_valid_markers": "Nessun indicatore valido trovato nel file.",
        "btn_remove_marker": "Rimuovi indicatore",
        "btn_edit_marker": "Modifica",
        "resolve_color_orange": "Arancione",
        "resolve_color_apricot": "Albicocca",
        "resolve_color_yellow": "Giallo",
        "resolve_color_lime": "Lime",
        "resolve_color_green": "Verde",
        "resolve_color_teal": "Verde acqua",

        "resolve_color_blue": "Blu",
        "resolve_color_purple": "Viola",
        "resolve_color_pink": "Rosa",
        "resolve_color_beige": "Beige",
        "resolve_color_brown": "Marrone",
        "placeholder_marker_name": "es. Il mio marker",
        "lbl_marker_name": "Nome indicatore",
        "lbl_marker_color": "Colore",
        "chk_telemetry_opt_in": "Abilita telemetria anonima",
        "msg_telemetry_settings": "Raccogliamo dati anonimi minimi (versione OS, versione app) per migliorare BadWords. Non vengono mai inviati file audio o personali.",
        "link_repo_label": "BadWords su GitHub",
        "link_repo_desc": "Esplora il codice sorgente, segnala problemi e segui il progetto",
        "link_coffee_label": "Offrimi un caffè",
        "link_coffee_desc": "Supporta lo sviluppo di BadWords",
        "msg_compute_fail_title": "Hardware non supportato",
        "msg_compute_fail_desc": "La tua CPU/GPU non supporta il tipo di calcolo selezionato. Seleziona un tipo diverso o utilizza 'Auto'.",
        "lbl_accent_color": "Colore accento",
        "lbl_app_icon": "Icona app",
        "lbl_vad_filter": "Filtro VAD",
        "lbl_beam_size": "Dimensione del beam",
        "lbl_temperature": "Temperatura",
        "lbl_condition_prev": "Condiziona sul testo precedente",
        "lbl_logprob": "Soglia log-prob",
        "lbl_no_speech": "Soglia assenza voce",
        "tab_transcript": "Trascrizione",
        "tab_ai_engine": "Motore IA",
        "lbl_chunk_max_words": "Parole max per blocco",
        "lbl_chunk_lookahead": "Parole di anticipo",
        "lbl_chunk_min_chars": "Caratteri min per blocco",
        "lbl_compute_type": "Tipo di calcolo",
        "lbl_initial_prompt": "Prompt iniziale",
        "lbl_patience": "Pazienza",
        "lbl_compression_ratio": "Rapporto di compressione",
        "lbl_no_repeat_ngram": "Non ripetere N-Grammi",
        "lbl_regroup": "Raggruppa",
        "lbl_suppress_silence": "Sopprimi silenzio",
        "lbl_q_levels": "Livelli Q",
        "lbl_k_size": "Dimensione K",
        "msg_restart_required": "Modifiche applicate. L'effetto completo sarà visibile al prossimo avvio.",
        "msg_restart_lang_pending": "Lingua cambiata. L'effetto completo sarà visibile al riavvio.",
        "msg_unsaved_title": "Modifiche non salvate",
        "msg_unsaved_desc": "Hai modifiche non salvate. Cosa vorresti fare?\n\n",
        "btn_discard": "Scarta",
        "btn_save_all": "Salva tutto",
        "btn_discard_all": "Scarta tutto",
        "btn_cancel": "Annulla",
        "msg_assembly_failed": "Assemblaggio fallito. Controlla il registro.",
        "msg_load_project_failed": "Impossibile caricare il progetto",
        "msg_fs_error": "Errore Standalone Silence",
        "search_placeholder": "Cerca...",
        "search_results_counter_empty": "0/0",
        "search_tooltip_prev": "Precedente (Maiusc+Invio)",
        "search_tooltip_next": "Successivo (Invio)",
        "search_tooltip_close": "Chiudi (Esc)",
        # ── Standalone Silence Detection ───────────────────────────────────────
        "btn_fast_silence_detection": "Rilevamento silenzi",
        "btn_standalone_silence_detection": "Rilevamento silenzi",
        "lbl_fast_silence_workspace": "Rilevamento silenzi",
        "lbl_standalone_silence_workspace": "Rilevamento silenzi",
        "msg_fast_silence": "Rilevamento silenzi",
        "msg_standalone_silence": "Rilevamento silenzi",
        "msg_fast_silence_processing_c": "Rilevamento completato.",
        "msg_standalone_silence_processing_c": "Rilevamento completato.",
        "btn_run_fast_silence": "Avvia",
        "btn_run_standalone_silence": "Avvia",
        "txt_initializing_fast_silence": "Inizializzazione rilevamento...",
        "txt_initializing_standalone_silence": "Inizializzazione rilevamento...",
        # ── Timeline / Track selection ──────────────────────────────────────────
        "msg_no_timelines_detected": "Nessuna timeline rilevata",
        "msg_no_audio_tracks_detected": "Nessuna traccia audio rilevata",
        "txt_all_tracks": "Tutte le tracce",
        "msg_transcription_source": "BadWords - Timeline di origine: <i>{tl}</i> - Tracce: <i>{tr}</i>",
        "txt_all": "Tutte",
        "lbl_xml_preserve_track_order": "Mantieni l'ordine originale delle tracce audio",
        "tt_xml_preserve_track_order": "Se attivato, le tracce audio mantengono i loro indici originali nella timeline di output (es. A3 rimane A3). Se disattivato (predefinito), le tracce vengono rimappate sequenzialmente a partire da A1.",
        "status_assembly_xml_build": "Costruzione dell'assemblaggio XML...",
        "status_assembly_xml_import": "Importazione della timeline XML...",
        "status_assembly_colors": "Applicazione dei colori dei clip...",
        "lbl_timestamp_precise": "Timestamp precisi (ms)",
        "tt_timestamp_precise": "Mostra la precisione in millisecondi nei timestamp dei segmenti (es. [01:07.986] invece di [01:08]).",
    },
    'pt': {
        "lbl_hidden_start": "Início do áudio detectado como inaudível, ignorado.",
        "btn_show_anyway": "mostrar mesmo assim",
        "chk_sync_davinci": "Sincronizar linha do tempo DaVinci ao mudar de edição",
        "tt_refresh_timelines": "Atualizar linhas do tempo",
        "tt_reset_to_default": "Restaurar padrões",
        "tt_advanced_locked": "Disponível em próximas subversões do BadWords 3.0",
        "btn_analyze": "Analisar",
        "btn_analyze_compare": "Analisar (Comparar)",
        "btn_analyze_standalone": "Analisar (Independente)",
        "btn_apply": "Aplicar",
        "btn_assemble": "Montar",
        "btn_clear": "Limpar",
        "btn_close": "Fechar",
        "btn_export_project": "Exportar Projeto",
        "btn_fast_silence_detection": "Detecção Rápida de Silêncio",
        "btn_import_project": "Importar Projeto",
        "btn_import_script": "Importar Script",
        "btn_restart_later": "Reiniciar mais tarde",
        "btn_restart_now": "Reiniciar agora",
        "btn_restore_defaults": "Redefinir Tudo",
        "btn_run_fast_silence": "Executar Silêncio Rápido",
        "btn_save": "Salvar",
        "btn_back_to_transcription": "Voltar para Transcrição",
        "lbl_font_preview": "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
        "lbl_add_custom_marker": "+ adicionar marcador personalizado...",
        "lbl_cut_silence_directly": "Cortar silêncio diretamente",
        "lbl_detect_and_cut_silence": "Detectar e cortar silêncio",
        "lbl_detect_and_mark_silence": "Detectar e marcar silêncio",
        "lbl_device": "Dispositivo",
        "lbl_display_mode": "Modo de Exibição",
        "lbl_fast_silence_workspace": "Espaço de Silêncio Rápido",
        "lbl_font_size_pt": "Tamanho da fonte (pt)",
        "lbl_initializing": "Inicializando...",
        "lbl_lang": "Idioma",
        "lbl_language": "Idioma",
        "lbl_view_mode": "Vista Configurações (Básico/Avançado)",
        "lbl_line_spacing_px": "Espaçamento de linha (px)",
        "lbl_loading": "Carregando...",
        "lbl_mark_filler_words_automat": "Marcar preenchimentos automaticamente",
        "lbl_mark_inaudible_fragments": "Marcar fragmentos inaudíveis",
        "lbl_mark_silence_with_color": "Marcar silêncio com cor",
        "lbl_marking_mode": "Modo de Marcação",
        "lbl_model": "Modelo",
        "lbl_offset_s": "Deslocamento (s)",
        "lbl_padding_s": "Margem (s)",
        "lbl_pinned_favorites": "Favoritos Fixados",
        "lbl_ripple_delete_red_clips": "Excluir clipes vermelhos (<span style='color: #ed4245;'>Ripple Delete</span>)",
        "lbl_show_detected_typos": "Mostrar erros de digitação detectados",
        "lbl_show_inaudible_fragments": "Mostrar fragmentos inaudíveis",
        "lbl_silence_threshold_db": "Limiar de silêncio (dB)",
        "lbl_snap_max_s": "Ajuste máx (s)",
        "lbl_threshold_db": "Limiar (dB)",
        "lbl_min_silence_dur": "Duração mínima do silêncio (s)",
        "lbl_timeline_selection": "Seleção da Timeline",
        "lbl_tracks_selection": "Seleção de Faixas",
        "lbl_transcript_font": "Fonte da transcrição",
        "lbl_transcription_workspace": "Espaço de Transcrição",
        "lbl_words": "palavras",
        "msg_analysis_failed": "A análise falhou.",
        "msg_fast_silence": "Silêncio Rápido",
        "msg_fast_silence_processing_c": "Processamento de silêncio concluído.",
        "msg_no_active_transcription_t": "Nenhuma timeline de transcrição ativa detectada.",
        "msg_no_silence_segments_detec": "Nenhum segmento de silêncio detectado.",
        "msg_please_import_or_paste_a": "Por favor, importe ou cole um script primeiro.",
        "msg_restart_lang": "Idioma alterado. Por favor, reinicie o BadWords.",
        "msg_success": "Sucesso",
        "msg_the_transcription_process": "O processo de transcrição foi concluído.",
        "msg_timeline_assembled_succes": "Timeline montada com sucesso.",
        "msg_title_language_changed": "Idioma Alterado",
        "msg_title_settings": "Configurações",
        "msg_title_restore_defaults": "Restaurar Padrões",
        "msg_restore_defaults_confirm": "Tem certeza de que deseja redefinir todas as configurações para os valores padrão?",
        "opt_continuous_flow": "Fluxo Contínuo",
        "lbl_error": "Erro",
        "txt_detecting_silence": "Detectando silêncio...",
        "txt_building_data": "Construindo estrutura de dados...",
        "txt_fast_silence_complete": "Silêncio Rápido concluído.",
        "txt_slowing_audio": "Desacelerando áudio...",
        "txt_enhancing_audio": "Aprimorando áudio...",
        "txt_transcribing_pct": "Transcrevendo...",
        "txt_detecting_source": "Detectando origem ideal...",
        "txt_calculating_cuts": "Calculando cortes...",
        "msg_warning": "Aviso",
        "opt_segmented_blocks": "Blocos segmentados",
        "ph_paste_script_here": "Cole o script aqui...",
        "ph_search": "Pesquisar...",
        "rad_blue_retake": "Azul (Retake)",
        "rad_eraser_clear": "Borracha (Limpar)",
        "rad_green_typo": "Verde (Erro/Typo)",
        "rad_red_cut_filler": "Vermelho (Cortar/Preenchimento)",
        "tab_appearance": "Aparência",
        "tab_audio_sync": "Sincronização",
        "tab_general": "Geral",
        "tool_assembly": "Montagem",
        "tool_filler_words": "Preenchimentos",
        "tool_main_panel": "Painel Principal",
        "tool_mark_inaudible": "Marcar Inaudível",
        "tool_quit": "Sair",
        "tool_ripple_delete": "Ripple Delete",
        "tool_script_analysis": "Análise de Script",
        "tool_settings": "Configurações",
        "tool_show_inaudible": "Mostrar Inaudível",
        "tool_show_typos": "Mostrar Erros (Typos)",
        "tool_silence": "Silêncio",
        "tooltip_clear_all_markings": "Limpar todas as marcações",
        "tooltip_dev": "Recurso em desenvolvimento",
        "tt_revert_to_default": "Restaurar para o padrão",
        "txt_assembling_timeline": "Montando timeline...",
        "txt_finishing": "Finalizando...",
        "txt_initializing_analysis": "Inicializando análise...",
        "txt_initializing_assembly": "Inicializando montagem...",
        "txt_initializing_fast_silence": "Inicializando Silêncio Rápido...",
        "txt_save": "Salvar",
        "txt_saved": "Salvo!",
        "txt_select": "Selecionar...",
        "txt_select_tracks": "Selecionar faixas...",
        "msg_engine_render": "Renderizando áudio...",
        "msg_engine_check_model": "Verificando modelo...",
        "msg_engine_whisper_run": "Iniciando Whisper...",
        "msg_engine_whisper_init": "Inicializando transcrição...",
        "msg_engine_silence": "Detecção de silêncio...",
        "msg_engine_processing": "Processando...",
        "msg_engine_init_analysis": "Finalizando dados...",
        "msg_engine_fast_silence_detect": "Detectando silêncio (FFmpeg)...",
        "msg_engine_fast_silence_build": "Construindo estrutura de dados...",
        "msg_engine_fast_silence_done": "Silêncio Rápido concluído.",
        "msg_engine_assembly_init": "Inicializando montagem...",
        "msg_engine_assembly_source": "Detectando origem ideal...",
        "msg_engine_slow_audio": "Desacelerando áudio...",
        "btn_ok": "OK",
        "btn_view_basic": "Vista Básica",
        "btn_view_advanced": "Vista Avançada",
        "lbl_chunk_punct_count": "Sinais de pontuação por bloco",
        "msg_restore_title": "Redefinir Tudo",
        "msg_restore_desc": "Todas as configurações serão redefinidas para os padrões de fábrica. Tem certeza?",
        "msg_clear_title": "Limpar Transcrição",
        "msg_clear_desc": "Tem certeza de que deseja limpar todas as marcas?",
        "status_render": "Renderizando áudio...",
        "status_direct_source": "Extraindo áudio diretamente da fonte...",
        "status_slow": "Desacelerando áudio...",
        "status_check_model": "Verificando modelo...",
        "status_downloading_model": "Baixando modelo...",
        "status_downloading_pct": "Baixando modelo: {pct}%",
        "first_run_hint_1": "O primeiro arranque demora mais — a AI está a criar a cache no disco.",
        "first_run_hint_2": "A aquecer o motor local da AI. As próximas vezes serão muito rápidas!",
        "first_run_hint_3": "A AI está a desfazer as malas virtuais no seu computador. Um momento.",
        "first_run_hint_4": "A otimizar a AI para o seu hardware. É um processo que ocorre só uma vez.",
        "first_run_hint_5": "A primeira execução demora sempre mais. Prometo que depois será num instante!",
        "first_run_hint_6": "A compilar as camadas locais da AI. Quase lá...",
        "first_run_hint_7": "A AI está a familiarizar-se com o seu sistema operativo.",
        "first_run_hint_8": "A configurar tudo localmente. O momento ideal para beber uma água.",
        "first_run_hint_9": "A alocar memória para a AI... Esta inicialização inicial leva tempo.",
        "first_run_hint_10": "A AI está a calibrar os seus parâmetros locais pela primeira vez.",
        "analysis_hint_1": "A AI está com os ouvidos bem atentos ao seu áudio. Shhh...",
        "analysis_hint_2": "A transformar o seu áudio em texto à velocidade da luz.",
        "analysis_hint_3": "A AI está a anotar implacavelmente cada 'humm' que você diz.",
        "analysis_hint_4": "A extrair palavras diretamente das ondas sonoras...",
        "analysis_hint_5": "A AI está a ler o seu ficheiro de áudio a alta velocidade.",
        "analysis_hint_6": "A transformar os seus barulhos humanos em palavras de verdade.",
        "analysis_hint_7": "A analisar a faixa de áudio. Nenhuma palavra vai escapar.",
        "analysis_hint_8": "A AI está a esculpir o seu áudio como barro digital.",
        "analysis_hint_9": "A cortar e a ajustar o áudio para uma sincronização perfeita.",
        "analysis_hint_10": "A polir a transcrição. Aguente firme, está quase!",
        "status_whisper_init": "Inicializando transcrição...",
        "status_transcribing": "Transcrevendo...",
        "status_silence": "Detecção de silêncio...",
        "status_process": "Processando...",
        "status_finalize": "Finalizando dados...",
        "status_assembly_init": "Inicializando Montagem...",
        "status_assembly_source": "Detectando fonte ideal...",
        "status_calc_cuts": "Calculando Cortes...",
        "status_assembly_resolve": "Montando no Resolve...",
        "status_assembly_clips": "Montando clipes...",
        "msg_quit_title": "Sair do BadWords",
        "msg_quit_desc": "Tem certeza de que deseja sair?",
        "btn_yes": "Sim",
        "btn_no": "Não",
        "title_telemetry": "Estatísticas do BadWords",
        "msg_telemetry": "Olá! 👋\n\nPara me ajudar a melhorar o BadWords, você concorda em enviar um ping único e 100% anônimo após a instalação / atualização?\n\nApenas a versão do aplicativo e o SO são enviados. NENHUM arquivo de áudio, script ou dado pessoal é coletado NUNCA.\n\nA geolocalização é opcional — desmarque a caixa abaixo para desativá-la.\n\nVocê pode alterar essas preferências a qualquer momento em Configurações → Telemetria.",
        "btn_telemetry_yes": "Eu concordo",
        "btn_telemetry_no": "Não, obrigado",
        "chk_telemetry_geo": "Compartilhe sua localização geral (país/cidade)",
        "tab_interface": "Interface",
        "lbl_always_on_top": "Sempre visível",
        "lbl_hidden_panels": "Painéis ocultos",
        "btn_export_settings": "Exportar configurações",
        "btn_import_settings": "Importar configurações",
        "tab_algorithms": "Algoritmos",
        "tab_shortcuts": "Atalhos de teclado",
        "lbl_algo_fuzzy": "Limite de correspondência difusa (%)",
        "lbl_algo_lookahead": "Antecipação de retomada (palavras)",
        "lbl_algo_penalty": "Penalidade de distância",
        "lbl_algo_anchor": "Profundidade de âncora",
        "shortcut_mark_red": "Marcador vermelho",
        "shortcut_mark_blue": "Marcador azul",
        "shortcut_mark_green": "Marcador verde",
        "shortcut_mark_eraser": "Borracha",
        "shortcut_jump_to_word": "Ir para palavra",
        "shortcut_play_stop": "Reproduzir / Parar",
        "shortcut_search": "Pesquisar",
        "shortcut_open_settings": "Abrir configurações",
        "shortcut_none": "(nenhum)",
        "shortcut_custom_marker_fmt": "Alternar para o marcador \"{name}\"",
        "tab_custom_markers": "Marcadores personalizados",
        "tab_telemetry": "Telemetria",
        "btn_add_marker": "Adicionar marcador",
        "btn_export_markers": "Exportar",
        "btn_import_markers": "Importar",
        "msg_no_markers_to_export": "Nenhum marcador personalizado para exportar.",
        "msg_import_invalid_format": "Formato de arquivo inválido. Esperado um arquivo JSON com uma lista \"custom_markers\".",
        "msg_import_no_valid_markers": "Nenhum marcador válido encontrado no arquivo.",
        "btn_remove_marker": "Remover marcador",
        "btn_edit_marker": "Editar",
        "resolve_color_orange": "Laranja",
        "resolve_color_apricot": "Damasco",
        "resolve_color_yellow": "Amarelo",
        "resolve_color_lime": "Lima",
        "resolve_color_green": "Verde",
        "resolve_color_teal": "Azul-petróleo",

        "resolve_color_blue": "Azul",
        "resolve_color_purple": "Roxo",
        "resolve_color_pink": "Rosa",
        "resolve_color_beige": "Bege",
        "resolve_color_brown": "Marrom",
        "placeholder_marker_name": "ex. Meu marcador",
        "lbl_marker_name": "Nome do marcador",
        "lbl_marker_color": "Cor",
        "chk_telemetry_opt_in": "Ativar telemetria anônima",
        "msg_telemetry_settings": "Coletamos dados anônimos mínimos (versão do SO, versão do app) para melhorar o BadWords. Nenhum arquivo de áudio ou pessoal é enviado.",
        "link_repo_label": "BadWords no GitHub",
        "link_repo_desc": "Explore o código-fonte, reporte problemas e acompanhe o projeto",
        "link_coffee_label": "Me pague um café",
        "link_coffee_desc": "Apoie o desenvolvimento do BadWords",
        "msg_compute_fail_title": "Hardware não suportado",
        "msg_compute_fail_desc": "Sua CPU/GPU não suporta o tipo de cómputo selecionado. Selecione outro tipo ou use 'Auto'.",
        "lbl_accent_color": "Cor de destaque",
        "lbl_app_icon": "Ícone do aplicativo",
        "lbl_vad_filter": "Filtro VAD",
        "lbl_beam_size": "Tamanho do feixe",
        "lbl_temperature": "Temperatura",
        "lbl_condition_prev": "Condicionar no texto anterior",
        "lbl_logprob": "Limiar log-prob",
        "lbl_no_speech": "Limiar de ausência de fala",
        "tab_transcript": "Transcrição",
        "tab_ai_engine": "Motor IA",
        "lbl_chunk_max_words": "Máx. palavras por bloco",
        "lbl_chunk_lookahead": "Palavras de antecipação",
        "lbl_chunk_min_chars": "Mín. caracteres por bloco",
        "lbl_compute_type": "Tipo de cómputo",
        "lbl_initial_prompt": "Prompt inicial",
        "lbl_patience": "Paciência",
        "lbl_compression_ratio": "Taxa de compressão",
        "lbl_no_repeat_ngram": "Não repetir N-Grama",
        "lbl_regroup": "Reagrupar",
        "lbl_suppress_silence": "Suprimir silêncio",
        "lbl_q_levels": "Níveis Q",
        "lbl_k_size": "Tamanho K",
        "msg_restart_required": "Alterações aplicadas. O efeito completo será visível na próxima inicialização.",
        "msg_restart_lang_pending": "Idioma alterado. O efeito completo será visível na próxima inicialização.",
        "msg_unsaved_title": "Alterações não salvas",
        "msg_unsaved_desc": "Você tem alterações não salvas. O que gostaria de fazer?\n\n",
        "btn_discard": "Descartar",
        "btn_save_all": "Salvar tudo",
        "btn_discard_all": "Descartar tudo",
        "btn_cancel": "Cancelar",
        "msg_assembly_failed": "Montagem falhou. Verifique o registro.",
        "msg_load_project_failed": "Não foi possível carregar o projeto",
        "msg_fs_error": "Erro Standalone Silence",
        "search_placeholder": "Pesquisar...",
        "search_results_counter_empty": "0/0",
        "search_tooltip_prev": "Anterior (Shift+Enter)",
        "search_tooltip_next": "Próximo (Enter)",
        "search_tooltip_close": "Fechar (Esc)",
        # ── Standalone Silence Detection ───────────────────────────────────────
        "btn_fast_silence_detection": "Detecção de silêncio",
        "btn_standalone_silence_detection": "Detecção de silêncio",
        "lbl_fast_silence_workspace": "Detecção de silêncio",
        "lbl_standalone_silence_workspace": "Detecção de silêncio",
        "msg_fast_silence": "Detecção de silêncio",
        "msg_standalone_silence": "Detecção de silêncio",
        "msg_fast_silence_processing_c": "Detecção de silêncio concluída.",
        "msg_standalone_silence_processing_c": "Detecção de silêncio concluída.",
        "btn_run_fast_silence": "Executar",
        "btn_run_standalone_silence": "Executar",
        "txt_initializing_fast_silence": "Inicializando detecção...",
        "txt_initializing_standalone_silence": "Inicializando detecção...",
        # ── Timeline / Track selection ──────────────────────────────────────────
        "msg_no_timelines_detected": "Nenhuma timeline detectada",
        "msg_no_audio_tracks_detected": "Nenhuma faixa de áudio detectada",
        "txt_all_tracks": "Todas as faixas",
        "msg_transcription_source": "BadWords - Timeline de origem: <i>{tl}</i> - Faixas: <i>{tr}</i>",
        "txt_all": "Todas",
        "lbl_xml_preserve_track_order": "Preservar a ordem original das faixas de áudio",
        "tt_xml_preserve_track_order": "Quando ativado, as faixas de áudio mantêm os seus índices originais na linha do tempo de saída (ex. A3 permanece A3). Quando desativado (padrão), as faixas são remapeadas sequencialmente a partir de A1.",
        "status_assembly_xml_build": "Construindo montagem XML...",
        "status_assembly_xml_import": "Importando timeline XML...",
        "status_assembly_colors": "Aplicando cores dos clipes...",
        "lbl_timestamp_precise": "Timestamps precisos (ms)",
        "tt_timestamp_precise": "Exibe precisão total em milissegundos nos timestamps dos segmentos (ex. [01:07.986] em vez de [01:08]).",
    },
    'uk': {
        "lbl_hidden_start": "Початок аудіо розпізнано як нерозбірливий, пропущено.",
        "btn_show_anyway": "все одно показати",
        "chk_sync_davinci": "Синхронізувати таймлайн DaVinci при зміні редакції",
        "tt_refresh_timelines": "Оновити таймлайни",
        "tt_reset_to_default": "Скинути до замовчувань",
        "tt_advanced_locked": "Доступно у наступних підверсіях BadWords 3.0",
        "btn_analyze": "Аналізувати",
        "btn_analyze_compare": "Аналіз (Порівняння)",
        "btn_analyze_standalone": "Аналіз (Автономний)",
        "btn_apply": "Застосувати",
        "btn_assemble": "Змонтувати",
        "btn_clear": "Очистити",
        "btn_close": "Закрити",
        "btn_export_project": "Експорт Проєкту",
        "btn_fast_silence_detection": "Швидке Виявлення Тиші",
        "btn_import_project": "Імпорт Проєкту",
        "btn_import_script": "Імпорт Сценарію",
        "btn_restart_later": "Перезапустити пізніше",
        "btn_restart_now": "Перезапустити зараз",
        "btn_restore_defaults": "Скинути Всі Налаштування",
        "btn_run_fast_silence": "Запуск Швидкої Тиші",
        "btn_save": "Зберегти",
        "btn_back_to_transcription": "Назад до Транскрипції",
        "lbl_font_preview": "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
        "lbl_add_custom_marker": "+ додати власний маркер...",
        "lbl_cut_silence_directly": "Вирізати тишу безпосередньо",
        "lbl_detect_and_cut_silence": "Виявити та вирізати тишу",
        "lbl_detect_and_mark_silence": "Виявити та позначити тишу",
        "lbl_device": "Пристрій",
        "lbl_display_mode": "Режим Відображення",
        "lbl_fast_silence_workspace": "Простір Швидкої Тиші",
        "lbl_font_size_pt": "Розмір шрифту (pt)",
        "lbl_initializing": "Ініціалізація...",
        "lbl_lang": "Мова",
        "lbl_language": "Мова",
        "lbl_view_mode": "Вигляд Налаштувань (Основний/Розширений)",
        "lbl_line_spacing_px": "Міжрядковий інтервал (px)",
        "lbl_loading": "Завантаження...",
        "lbl_mark_filler_words_automat": "Автоматично позначати паразити",
        "lbl_mark_inaudible_fragments": "Позначати нерозбірливі фрагменти",
        "lbl_mark_silence_with_color": "Позначати тишу кольором",
        "lbl_marking_mode": "Режим Позначення",
        "lbl_model": "Модель",
        "lbl_offset_s": "Зсув (с)",
        "lbl_padding_s": "Відступ (с)",
        "lbl_pinned_favorites": "Закреплені Інструменти",
        "lbl_ripple_delete_red_clips": "Видалити червоні кліпи (<span style='color: #ed4245;'>Ripple Delete</span>)",
        "lbl_show_detected_typos": "Показати виявлені помилки",
        "lbl_show_inaudible_fragments": "Показати нерозбірливі фрагменти",
        "lbl_silence_threshold_db": "Поріг тиші (dB)",
        "lbl_snap_max_s": "Прилипання макс. (с)",
        "lbl_threshold_db": "Поріг (dB)",
        "lbl_min_silence_dur": "Мін. тривалість тиші (с)",
        "lbl_timeline_selection": "Вибір Таймлайну",
        "lbl_tracks_selection": "Вибір Доріжок",
        "lbl_transcript_font": "Шрифт транскрипту",
        "lbl_transcription_workspace": "Простір Транскрипції",
        "lbl_words": "слів",
        "msg_analysis_failed": "Аналіз не вдався.",
        "msg_fast_silence": "Швидка Тиша",
        "msg_fast_silence_processing_c": "Обробка тиші завершена.",
        "msg_no_active_transcription_t": "Не виявлено активного таймлайну.",
        "msg_no_silence_segments_detec": "Сегментів тиші не виявлено.",
        "msg_please_import_or_paste_a": "Будь ласка, спочатку імпортуйте або вставьте сценарій.",
        "msg_restart_lang": "Мову змінено. Перезапустіть BadWords.",
        "msg_success": "Успіх",
        "msg_the_transcription_process": "Процес транскрипції завершено.",
        "msg_timeline_assembled_succes": "Таймлайн успішно змонтовано.",
        "msg_title_language_changed": "Мову Змінено",
        "msg_title_settings": "Налаштування",
        "msg_title_restore_defaults": "Відновити типові",
        "msg_restore_defaults_confirm": "Ви впевнені, що хочете скинути всі налаштування до типових значень?",
        "opt_continuous_flow": "Безперервний текст",
        "lbl_error": "Помилка",
        "txt_detecting_silence": "Виявлення тиші...",
        "txt_building_data": "Створення структури даних...",
        "txt_fast_silence_complete": "Швидка Тиша завершена.",
        "txt_slowing_audio": "Уповільнення аудіо...",
        "txt_enhancing_audio": "Покращення аудіо...",
        "txt_transcribing_pct": "Транскрибування...",
        "txt_detecting_source": "Виявлення оптимального джерела...",
        "txt_calculating_cuts": "Обчислення зрізів...",
        "msg_warning": "Попередження",
        "opt_segmented_blocks": "Сегментовані блоки",
        "ph_paste_script_here": "Вставте сценарій сюди...",
        "ph_search": "Пошук...",
        "rad_blue_retake": "Синій (Дубль)",
        "rad_eraser_clear": "Гумка (Очистити)",
        "rad_green_typo": "Зелений (Помилка)",
        "rad_red_cut_filler": "Червоний (Вирізати/Сміття)",
        "tab_appearance": "Вигляд",
        "tab_audio_sync": "Синхронізація",
        "tab_general": "Загальні",
        "tool_assembly": "Монтаж",
        "tool_filler_words": "Слова-паразити",
        "tool_main_panel": "Головна Панель",
        "tool_mark_inaudible": "Позначити Нерозбірливе",
        "tool_quit": "Вийти",
        "tool_ripple_delete": "Ripple Delete",
        "tool_script_analysis": "Аналіз Сценарію",
        "tool_settings": "Налаштування",
        "tool_show_inaudible": "Показати Нерозбірливе",
        "tool_show_typos": "Показати Помилки",
        "tool_silence": "Тиша",
        "tooltip_clear_all_markings": "Очистити всі позначки",
        "tooltip_dev": "Функція в розробці",
        "tt_revert_to_default": "Повернути типові",
        "txt_assembling_timeline": "Монтування таймлайну...",
        "txt_finishing": "Завершення...",
        "txt_initializing_analysis": "Ініціалізація аналізу...",
        "txt_initializing_assembly": "Ініціалізація монтажу...",
        "txt_initializing_fast_silence": "Ініціалізація Швидкої Тиші...",
        "txt_save": "Зберегти",
        "txt_saved": "Збережено!",
        "txt_select": "Обрати...",
        "txt_select_tracks": "Обрати доріжки...",
        "msg_engine_render": "Рендеринг аудіо...",
        "msg_engine_check_model": "Перевірка моделі...",
        "msg_engine_whisper_run": "Запуск Whisper...",
        "msg_engine_whisper_init": "Ініціалізація транскрипції...",
        "msg_engine_silence": "Виявлення тиші...",
        "msg_engine_processing": "Обробка...",
        "msg_engine_init_analysis": "Завершення даних...",
        "msg_engine_fast_silence_detect": "Виявлення тиші (FFmpeg)...",
        "msg_engine_fast_silence_build": "Створення структури даних...",
        "msg_engine_fast_silence_done": "Швидка тиша завершена.",
        "msg_engine_assembly_init": "Ініціалізація монтажу...",
        "msg_engine_assembly_source": "Виявлення оптимального джерела...",
        "msg_engine_slow_audio": "Уповільнення аудіо...",
        "btn_ok": "ОК",
        "btn_view_basic": "Базовий Вигляд",
        "btn_view_advanced": "Розширений Вигляд",
        "lbl_chunk_punct_count": "Знаки пунктуації на блок",
        "msg_restore_title": "Скинути Всі Налаштування",
        "msg_restore_desc": "Всі налаштування будуть скинуті до стандартних значень. Ви впевнені?",
        "msg_clear_title": "Очистити Транскрипт",
        "msg_clear_desc": "Ви впевнені, що хочете очистити всі позначки?",
        "status_render": "Рендеринг аудіо...",
        "status_direct_source": "Пряме витягнення аудіо з джерела...",
        "status_slow": "Уповільнення аудіо...",
        "status_check_model": "Перевірка моделі...",
        "status_downloading_model": "Завантаження моделі...",
        "status_downloading_pct": "Завантаження моделі: {pct}%",
        "first_run_hint_1": "Перший запуск триває довше — AI кешує дані на вашому диску.",
        "first_run_hint_2": "Розігріваємо локальний двигун AI. Наступного разу все літатиме!",
        "first_run_hint_3": "AI розпаковує свої віртуальні валізи на вашому ПК. Зачекайте.",
        "first_run_hint_4": "AI оптимізується під ваше залізо. Це робиться лише один раз.",
        "first_run_hint_5": "Перший запуск завжди найдовший. Обіцяю, далі буде набагато швидше!",
        "first_run_hint_6": "Компільовуємо локальні шари моделі. Вже майже...",
        "first_run_hint_7": "AI саме знайомиться з вашою операційною системою.",
        "first_run_hint_8": "Налаштовуємо локальне середовище. Гарний час випити води.",
        "first_run_hint_9": "Виділяємо пам'ять для AI... Первинна ініціалізація вимагає часу.",
        "first_run_hint_10": "AI вперше калібрує свої локальні параметри.",
        "analysis_hint_1": "AI уважно вслухається у ваше аудіо. Тссс...",
        "analysis_hint_2": "Перетворюємо ваші слова на текст зі швидкістю світла.",
        "analysis_hint_3": "AI безжалісно фіксує кожне ваше 'еее' та 'ммм'.",
        "analysis_hint_4": "Витягуємо слова прямо зі звукових хвиль...",
        "analysis_hint_5": "AI читає ваш аудіофайл на максимальній швидкості.",
        "analysis_hint_6": "Перетворюємо людський гомін на красивий, грамотний текст.",
        "analysis_hint_7": "Скануємо аудіодоріжку. Жодне слово не сховається від AI.",
        "analysis_hint_8": "AI працює зі звуком, мов скульптор із глиною.",
        "analysis_hint_9": "Точно нарізаємо аудіо для ідеальної синхронізації.",
        "analysis_hint_10": "Відполіровуємо транскрипцію до блиску. Вже закінчуємо!",
        "status_whisper_init": "Ініціалізація транскрипції...",
        "status_transcribing": "Транскрибування...",
        "status_silence": "Виявлення тиші...",
        "status_process": "Обробка...",
        "status_finalize": "Завершення даних...",
        "status_assembly_init": "Ініціалізація Монтажу...",
        "status_assembly_source": "Виявлення оптимального джерела...",
        "status_calc_cuts": "Обчислення Зрізів...",
        "status_assembly_resolve": "Монтування в Resolve...",
        "status_assembly_clips": "Монтування кліпів...",
        "msg_quit_title": "Вийти з BadWords",
        "msg_quit_desc": "Ви впевнені, що хочете вийти?",
        "btn_yes": "Так",
        "btn_no": "Ні",
        "title_telemetry": "Статистика BadWords",
        "msg_telemetry": "Привіт! 👋\n\nЩоб допомогти покращити BadWords, чи погоджуєтесь ви надіслати одноразовий 100% анонімний пінг після встановлення / оновлення?\n\nНадсилаються лише версія програми та тип ОС. Жодні аудіофайли чи особисті дані НІКОЛИ не збираються.\n\nГеолокація є необов’язковою — зніміть прапорець нижче, щоб вимкнути її.\n\nВи можете змінити ці налаштування будь-коли в Налаштування → Телеметрія.",
        "btn_telemetry_yes": "Я погоджуюсь",
        "btn_telemetry_no": "Ні, дякую",
        "chk_telemetry_geo": "Поділитися своїм загальним місцем розташування",
        "tab_interface": "Інтерфейс",
        "lbl_always_on_top": "Завжди поверх",
        "lbl_hidden_panels": "Приховані панелі",
        "btn_export_settings": "Експортувати налаштування",
        "btn_import_settings": "Імпортувати налаштування",
        "tab_algorithms": "Алгоритми",
        "tab_shortcuts": "Гарячі клавіші",
        "lbl_algo_fuzzy": "Поріг нечіткого збігу (%)",
        "lbl_algo_lookahead": "Випередження дублювання (слова)",
        "lbl_algo_penalty": "Штраф за відстань",
        "lbl_algo_anchor": "Глибина анкера",
        "shortcut_mark_red": "Червоний маркер",
        "shortcut_mark_blue": "Синій маркер",
        "shortcut_mark_green": "Зелений маркер",
        "shortcut_mark_eraser": "Гумка",
        "shortcut_jump_to_word": "Перейти до слова",
        "shortcut_play_stop": "Грати / Зупинити",
        "shortcut_search": "Пошук",
        "shortcut_open_settings": "Відкрити налаштування",
        "shortcut_none": "(немає)",
        "shortcut_custom_marker_fmt": "Переключитися на позначку \"{name}\"",
        "tab_custom_markers": "Позначки користувача",
        "tab_telemetry": "Телеметрія",
        "btn_add_marker": "Додати позначку",
        "btn_export_markers": "Експортувати",
        "btn_import_markers": "Імпортувати",
        "msg_no_markers_to_export": "Немає нестандартних позначок для експорту.",
        "msg_import_invalid_format": "Невірний формат файлу. Очікується JSON-файл зі списком \"custom_markers\".",
        "msg_import_no_valid_markers": "У файлі не знайдено жодної дійсної позначки.",
        "btn_remove_marker": "Видалити позначку",
        "btn_edit_marker": "Редагувати",
        "resolve_color_orange": "Помаранчевий",
        "resolve_color_apricot": "Абрикосовий",
        "resolve_color_yellow": "Жовтий",
        "resolve_color_lime": "Лаймовий",
        "resolve_color_green": "Зелений",

        "resolve_color_teal": "Бірюзовий",
        "resolve_color_blue": "Синій",
        "resolve_color_purple": "Фіолетовий",
        "resolve_color_pink": "Рожевий",
        "resolve_color_beige": "Бежевий",
        "resolve_color_brown": "Коричневий",
        "placeholder_marker_name": "напр. Мій маркер",
        "lbl_marker_name": "Назва позначки",
        "lbl_marker_color": "Колір",
        "chk_telemetry_opt_in": "Увімкнути анонімну телеметрію",
        "msg_telemetry_settings": "Ми збираємо мінімальні анонімні дані (версія ОС, версія додатку) для поліпшення BadWords. Аудіо чи особисті файли ніколи не надсилаються.",
        "link_repo_label": "BadWords на GitHub",
        "link_repo_desc": "Переглядайте вихідний код, повідомляйте про проблеми та стежте за проєктом",
        "link_coffee_label": "Пригостіть мене кавою",
        "link_coffee_desc": "Підтримайте розвиток BadWords",
        "msg_compute_fail_title": "Залізо не підтримується",
        "msg_compute_fail_desc": "Ваш CPU/GPU не підтримує вибраний тип обчислень. Виберіть інший тип або використайте 'Auto'.",
        "lbl_accent_color": "Акцентний колір",
        "lbl_app_icon": "Іконка додатку",
        "lbl_vad_filter": "Фільтр VAD",
        "lbl_beam_size": "Розмір променя",
        "lbl_temperature": "Температура",
        "lbl_condition_prev": "Умовнюватися на попередньому тексті",
        "lbl_logprob": "Поріг log-prob",
        "lbl_no_speech": "Поріг відсутності мовлення",
        "tab_transcript": "Транскрипція",
        "tab_ai_engine": "Двигун АІ",
        "lbl_chunk_max_words": "Макс. слів у блоці",
        "lbl_chunk_lookahead": "Слова випередження",
        "lbl_chunk_min_chars": "Мін. знаків у блоці",
        "lbl_compute_type": "Тип обчислень",
        "lbl_initial_prompt": "Початковий промпт",
        "lbl_patience": "Терпіння (Patience)",
        "lbl_compression_ratio": "Коефіцієнт стиснення",
        "lbl_no_repeat_ngram": "Не повторювати N-Gram",
        "lbl_regroup": "Перегрупування",
        "lbl_suppress_silence": "Придушення тиші",
        "lbl_q_levels": "Рівні Q",
        "lbl_k_size": "Розмір K",
        "msg_restart_required": "Зміни застосовано. Повний ефект буде помітний після наступного запуску.",
        "msg_restart_lang_pending": "Мову змінено. Повні зміни застосуються після перезапуску.",
        "msg_unsaved_title": "Незбережені зміни",
        "msg_unsaved_desc": "У вас є незбережені зміни. Що б ви хотіли зробити?\n\n",
        "btn_discard": "Відхилити",
        "btn_save_all": "Зберегти все",
        "btn_discard_all": "Відхилити все",
        "btn_cancel": "Скасувати",
        "msg_assembly_failed": "Монтаж не вдався. Перевірте журнал.",
        "msg_load_project_failed": "Не вдалося завантажити проєкт",
        "msg_fs_error": "Помилка Standalone Silence",
        "search_placeholder": "Пошук...",
        "search_results_counter_empty": "0/0",
        "search_tooltip_prev": "Попередній (Shift+Enter)",
        "search_tooltip_next": "Наступний (Enter)",
        "search_tooltip_close": "Закрити (Esc)",
        # ── Standalone Silence Detection ───────────────────────────────────────
        "btn_fast_silence_detection": "Виявлення тиші",
        "btn_standalone_silence_detection": "Виявлення тиші",
        "lbl_fast_silence_workspace": "Виявлення тиші",
        "lbl_standalone_silence_workspace": "Виявлення тиші",
        "msg_fast_silence": "Виявлення тиші",
        "msg_standalone_silence": "Виявлення тиші",
        "msg_fast_silence_processing_c": "Виявлення тиші завершено.",
        "msg_standalone_silence_processing_c": "Виявлення тиші завершено.",
        "btn_run_fast_silence": "Запустити",
        "btn_run_standalone_silence": "Запустити",
        "txt_initializing_fast_silence": "Ініціалізація виявлення...",
        "txt_initializing_standalone_silence": "Ініціалізація виявлення...",
        # ── Timeline / Track selection ──────────────────────────────────────────
        "msg_no_timelines_detected": "Таймлайни не виявлено",
        "msg_no_audio_tracks_detected": "Аудіодоріжки не виявлено",
        "txt_all_tracks": "Усі доріжки",
        "msg_transcription_source": "BadWords - Вихідний таймлайн: <i>{tl}</i> - Доріжки: <i>{tr}</i>",
        "txt_all": "Усі",
        "lbl_xml_preserve_track_order": "Зберегти оригінальний порядок аудіодоріжок",
        "tt_xml_preserve_track_order": "Якщо увімкнено, аудіодоріжки зберігають свої вихідні індекси у підсумковій часовій шкалі (напр. A3 залишається A3). Якщо вимкнено (типово), доріжки перенумеровуються послідовно починаючи з A1.",
        "status_assembly_xml_build": "Побудова XML-монтажу...",
        "status_assembly_xml_import": "Імпортування XML-часової шкали...",
        "status_assembly_colors": "Застосування кольорів кліпів...",
        "lbl_timestamp_precise": "Точні мітки часу (мс)",
        "tt_timestamp_precise": "Показує повну точність у мілісекундах у мітках часу сегментів (напр. [01:07.986] замість [01:08]).",
    },
    'nl': {
        "lbl_hidden_start": "Begin van audio gedetecteerd als onverstaanbaar, overgeslagen.",
        "btn_show_anyway": "toch weergeven",
        "chk_sync_davinci": "DaVinci-tijdlijn synchroniseren bij het wisselen van bewerking",
        "tt_refresh_timelines": "Tijdlijnen vernieuwen",
        "tt_reset_to_default": "Herstellen naar standaard",
        "tt_advanced_locked": "Beschikbaar in aankomende subversies van BadWords 3.0",
        "btn_analyze": "Analyseren",
        "btn_analyze_compare": "Analyseren (Vergelijken)",
        "btn_analyze_standalone": "Analyseren (Losstaand)",
        "btn_apply": "Toepassen",
        "btn_assemble": "Monteren",
        "btn_clear": "Wissen",
        "btn_close": "Sluiten",
        "btn_export_project": "Project Exporteren",
        "btn_fast_silence_detection": "Snelle Stiltedetectie",
        "btn_import_project": "Project Importeren",
        "btn_import_script": "Script Importeren",
        "btn_restart_later": "Later opnieuw opstarten",
        "btn_restart_now": "Nu opnieuw opstarten",
        "btn_restore_defaults": "Alles Resetten",
        "btn_run_fast_silence": "Snelle Stilte Uitvoeren",
        "btn_save": "Opslaan",
        "btn_back_to_transcription": "Terug naar Transcriptie",
        "lbl_font_preview": "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
        "lbl_add_custom_marker": "+ aangepaste markering toevoegen...",
        "lbl_cut_silence_directly": "Stilte direct knippen",
        "lbl_detect_and_cut_silence": "Stilte detecteren en knippen",
        "lbl_detect_and_mark_silence": "Stilte detecteren en markeren",
        "lbl_device": "Apparaat",
        "lbl_display_mode": "Weergavemodus",
        "lbl_fast_silence_workspace": "Snelle Stilte Werkruimte",
        "lbl_font_size_pt": "Lettergrootte (pt)",
        "lbl_initializing": "Initialiseren...",
        "lbl_lang": "Taal",
        "lbl_language": "Taal",
        "lbl_view_mode": "Instellingen Weergave (Basis/Geavanceerd)",
        "lbl_line_spacing_px": "Regelafstand (px)",
        "lbl_loading": "Laden...",
        "lbl_mark_filler_words_automat": "Opvulwoorden automatisch markeren",
        "lbl_mark_inaudible_fragments": "Onhoorbare fragmenten markeren",
        "lbl_mark_silence_with_color": "Stilte markeren met kleur",
        "lbl_marking_mode": "Markeermodus",
        "lbl_model": "Model",
        "lbl_offset_s": "Offset (s)",
        "lbl_padding_s": "Marge (s)",
        "lbl_pinned_favorites": "Vastgepelde Favorieten",
        "lbl_ripple_delete_red_clips": "Rode clips verwijderen (<span style='color: #ed4245;'>Ripple Delete</span>)",
        "lbl_show_detected_typos": "Gedetecteerde typfouten tonen",
        "lbl_show_inaudible_fragments": "Onhoorbare fragmenten tonen",
        "lbl_silence_threshold_db": "Stiltedrempel (dB)",
        "lbl_snap_max_s": "Snap max (s)",
        "lbl_threshold_db": "Drempel (dB)",
        "lbl_min_silence_dur": "Minimale stilteduur (s)",
        "lbl_timeline_selection": "Timeline Selectie",
        "lbl_tracks_selection": "Tracks Selectie",
        "lbl_transcript_font": "Transcript lettertype",
        "lbl_transcription_workspace": "Transcriptie Werkruimte",
        "lbl_words": "woorden",
        "msg_analysis_failed": "Analyse mislukt.",
        "msg_fast_silence": "Snelle Stilte",
        "msg_fast_silence_processing_c": "Stilteverwerking voltooid.",
        "msg_no_active_transcription_t": "Geen actieve transcriptie-timeline gedetecteerd.",
        "msg_no_silence_segments_detec": "Geen stiltesegmenten gedetecteerd.",
        "msg_please_import_or_paste_a": "Importeer of plak eerst een script.",
        "msg_restart_lang": "Taal gewijzigd. Start BadWords opnieuw op.",
        "msg_success": "Succes",
        "msg_the_transcription_process": "Het transcriptieproces is voltooid.",
        "msg_timeline_assembled_succes": "Timeline succesvol gemonteerd.",
        "msg_title_language_changed": "Taal Gewijzigd",
        "msg_title_settings": "Instellingen",
        "msg_title_restore_defaults": "Standaard Herstellen",
        "msg_restore_defaults_confirm": "Weet u zeker dat u alle instellingen wilt resetten naar hun standaardwaarden?",
        "opt_continuous_flow": "Doorlopende Tekst",
        "lbl_error": "Fout",
        "txt_detecting_silence": "Stilte detecteren...",
        "txt_building_data": "Gegevensstructuur opbouwen...",
        "txt_fast_silence_complete": "Snelle Stilte voltooid.",
        "txt_slowing_audio": "Audio vertragen...",
        "txt_enhancing_audio": "Audio verbeteren...",
        "txt_transcribing_pct": "Transcriberen...",
        "txt_detecting_source": "Optimale bron detecteren...",
        "txt_calculating_cuts": "Snitten berekenen...",
        "msg_warning": "Waarschuwing",
        "opt_segmented_blocks": "Gesegmenteerde blokken",
        "ph_paste_script_here": "Plak script hier...",
        "ph_search": "Zoeken...",
        "rad_blue_retake": "Blauw (Retake)",
        "rad_eraser_clear": "Gum (Wissen)",
        "rad_green_typo": "Groen (Typfout)",
        "rad_red_cut_filler": "Rood (Knippen/Opvulling)",
        "tab_appearance": "Uiterlijk",
        "tab_audio_sync": "Audio Sync",
        "tab_general": "Algemeen",
        "tool_assembly": "Montage",
        "tool_filler_words": "Opvulwoorden",
        "tool_main_panel": "Hoofdvenster",
        "tool_mark_inaudible": "Onhoorbaar Markeren",
        "tool_quit": "Afsluiten",
        "tool_ripple_delete": "Ripple Delete",
        "tool_script_analysis": "Scriptanalyse",
        "tool_settings": "Instellingen",
        "tool_show_inaudible": "Toon Onhoorbaar",
        "tool_show_typos": "Toon Typfouten",
        "tool_silence": "Stilte",
        "tooltip_clear_all_markings": "Alle markeringen wissen",
        "tooltip_dev": "Functie in ontwikkeling",
        "tt_revert_to_default": "Standaard herstellen",
        "txt_assembling_timeline": "Timeline monteren...",
        "txt_finishing": "Afronden...",
        "txt_initializing_analysis": "Analyse initialiseren...",
        "txt_initializing_assembly": "Montage initialiseren...",
        "txt_initializing_fast_silence": "Snelle Stilte Initialiseren...",
        "txt_save": "Opslaan",
        "txt_saved": "Opgeslagen!",
        "txt_select": "Selecteren...",
        "txt_select_tracks": "Tracks selecteren...",
        "msg_engine_render": "Audio renderen...",
        "msg_engine_check_model": "Model controleren...",
        "msg_engine_whisper_run": "Whisper starten...",
        "msg_engine_whisper_init": "Transcriptie initialiseren...",
        "msg_engine_silence": "Stiltedetectie...",
        "msg_engine_processing": "Verwerken...",
        "msg_engine_init_analysis": "Gegevens afronden...",
        "msg_engine_fast_silence_detect": "Stilte detecteren (FFmpeg)...",
        "msg_engine_fast_silence_build": "Gegevensstructuur opbouwen...",
        "msg_engine_fast_silence_done": "Snelle Stilte voltooid.",
        "msg_engine_assembly_init": "Montage initialiseren...",
        "msg_engine_assembly_source": "Optimale bron detecteren...",
        "msg_engine_slow_audio": "Audio vertragen...",
        "btn_ok": "OK",
        "btn_view_basic": "Basisweergave",
        "btn_view_advanced": "Geavanceerde weergave",
        "lbl_chunk_punct_count": "Leestekens per blok",
        "msg_restore_title": "Alles Resetten",
        "msg_restore_desc": "Alle instellingen worden teruggezet naar de standaardwaarden. Weet je het zeker?",
        "msg_clear_title": "Transcript Wissen",
        "msg_clear_desc": "Weet u zeker dat u alle markeringen wilt wissen?",
        "status_render": "Audio renderen...",
        "status_direct_source": "Audio rechtstreeks uit bron extraheren...",
        "status_slow": "Audio vertragen...",
        "status_check_model": "Model controleren...",
        "status_downloading_model": "Model downloaden...",
        "status_downloading_pct": "Model downloaden: {pct}%",
        "first_run_hint_1": "De eerste start duurt wat langer — de AI slaat zijn gegevens lokaal op.",
        "first_run_hint_2": "De lokale AI-motor wordt warmgedraaid. Hierna gaat het razendsnel!",
        "first_run_hint_3": "De AI is zijn virtuele koffers aan het uitpakken op je schijf.",
        "first_run_hint_4": "De AI optimaliseert zich voor jouw hardware. Dit is eenmalig.",
        "first_run_hint_5": "De eerste keer duurt altijd het langst. Beloofd dat het hierna vlotter gaat!",
        "first_run_hint_6": "Lokale AI-lagen worden gecompileerd. We zijn er bijna...",
        "first_run_hint_7": "De AI is zich aan het oriënteren op jouw besturingssysteem.",
        "first_run_hint_8": "Alles wordt lokaal klaargezet. Tijd om even een slokje water te nemen.",
        "first_run_hint_9": "Geheugen wordt toegewezen aan de AI... Dit kost even tijd.",
        "first_run_hint_10": "De AI kalibreert voor de allereerste keer zijn lokale parameters.",
        "analysis_hint_1": "De AI luistert aandachtig naar je audio. Ssst...",
        "analysis_hint_2": "We zetten je audio om in tekst met de snelheid van het licht.",
        "analysis_hint_3": "De AI telt meedogenloos elke 'uhh' en 'hmm' die je zegt.",
        "analysis_hint_4": "Woorden worden direct uit de geluidsgolven geëxtraheerd...",
        "analysis_hint_5": "De AI is aan het snellezen door je audiobestand heen.",
        "analysis_hint_6": "We maken duidelijke tekst van menselijke geluiden.",
        "analysis_hint_7": "De audiotrack wordt uitgespit. Geen enkel woord ontsnapt.",
        "analysis_hint_8": "De AI boetseert je audio als een digitale kunstenaar.",
        "analysis_hint_9": "Het audio wordt perfect op maat gesneden voor de tijdlijn.",
        "analysis_hint_10": "De transcriptie wordt nog even opgepoetst. Bijna klaar!",
        "status_whisper_init": "Transcriptie initialiseren...",
        "status_transcribing": "Transcriberen...",
        "status_silence": "Stiltedetectie...",
        "status_process": "Verwerken...",
        "status_finalize": "Gegevens afronden...",
        "status_assembly_init": "Montage initialiseren...",
        "status_assembly_source": "Optimale bron detecteren...",
        "status_calc_cuts": "Snitten Berekenen...",
        "status_assembly_resolve": "Monteren in Resolve...",
        "status_assembly_clips": "Clips monteren...",
        "msg_quit_title": "BadWords afsluiten",
        "msg_quit_desc": "Weet je zeker dat je wilt afsluiten?",
        "btn_yes": "Ja",
        "btn_no": "Nee",
        "title_telemetry": "BadWords Statistieken",
        "msg_telemetry": "Hallo! 👋\n\nOm me te helpen BadWords te verbeteren, ga je ermee akkoord om na de installatie / update een eenmalige, 100% anonieme ping te sturen?\n\nAlleen de app-versie en het besturingssysteem worden verzonden. Er worden NOOIT audiobestanden of persoonlijke gegevens verzameld.\n\nGeolocatie is optioneel — vink het onderstaande vakje uit om het uit te schakelen.\n\nJe kunt deze voorkeuren op elk moment wijzigen via Instellingen → Telemetrie.",
        "btn_telemetry_yes": "Ik ga akkoord",
        "btn_telemetry_no": "Nee, bedankt",
        "chk_telemetry_geo": "Deel je algemene locatie (land/stad)",
        "tab_interface": "Interface",
        "lbl_always_on_top": "Altijd bovenaan",
        "lbl_hidden_panels": "Verborgen panelen",
        "btn_export_settings": "Instellingen exporteren",
        "btn_import_settings": "Instellingen importeren",
        "tab_algorithms": "Algoritmen",
        "tab_shortcuts": "Sneltoetsen",
        "lbl_algo_fuzzy": "Drempelwaarde fuzzy-matching (%)",
        "lbl_algo_lookahead": "Vooruitblik herneming (woorden)",
        "lbl_algo_penalty": "Afstandsstraf",
        "lbl_algo_anchor": "Ankerdiepte",
        "shortcut_mark_red": "Rode markering",
        "shortcut_mark_blue": "Blauwe markering",
        "shortcut_mark_green": "Groene markering",
        "shortcut_mark_eraser": "Gummetje",
        "shortcut_jump_to_word": "Naar woord springen",
        "shortcut_play_stop": "Afspelen / Stoppen",
        "shortcut_search": "Zoeken",
        "shortcut_open_settings": "Instellingen openen",
        "shortcut_none": "(geen)",
        "shortcut_custom_marker_fmt": "Overschakelen naar markering \"{name}\"",
        "tab_custom_markers": "Aangepaste markeringen",
        "tab_telemetry": "Telemetrie",
        "btn_add_marker": "Markering toevoegen",
        "btn_export_markers": "Exporteren",
        "btn_import_markers": "Importeren",
        "msg_no_markers_to_export": "Geen aangepaste markeringen om te exporteren.",
        "msg_import_invalid_format": "Ongeldig bestandsformaat. JSON-bestand met \"custom_markers\"-lijst verwacht.",
        "msg_import_no_valid_markers": "Geen geldige markeringen gevonden in het bestand.",
        "btn_remove_marker": "Markering verwijderen",
        "btn_edit_marker": "Bewerken",
        "resolve_color_orange": "Oranje",
        "resolve_color_apricot": "Abrikoos",
        "resolve_color_yellow": "Geel",
        "resolve_color_lime": "Limoen",
        "resolve_color_green": "Groen",

        "resolve_color_teal": "Groenblauw",
        "resolve_color_blue": "Blauw",
        "resolve_color_purple": "Paars",
        "resolve_color_pink": "Roze",
        "resolve_color_beige": "Beige",
        "resolve_color_brown": "Bruin",
        "placeholder_marker_name": "bijv. Mijn marker",
        "lbl_marker_name": "Markeringsnaam",
        "lbl_marker_color": "Kleur",
        "chk_telemetry_opt_in": "Anonieme telemetrie inschakelen",
        "msg_telemetry_settings": "We verzamelen minimale anonieme gegevens (OS-versie, app-versie) om BadWords te verbeteren. Er worden nooit audio- of persoonlijke bestanden verzonden.",
        "link_repo_label": "BadWords op GitHub",
        "link_repo_desc": "Bekijk de broncode, meld problemen en volg het project",
        "link_coffee_label": "Trakteer me op een koffie",
        "link_coffee_desc": "Steun de ontwikkeling van BadWords",
        "msg_compute_fail_title": "Hardware niet ondersteund",
        "msg_compute_fail_desc": "Uw CPU/GPU ondersteunt het geselecteerde rekentype niet. Kies een ander type of gebruik 'Auto'.",
        "lbl_accent_color": "Accentkleur",
        "lbl_app_icon": "App-pictogram",
        "lbl_vad_filter": "VAD-filter",
        "lbl_beam_size": "Straalgrootte",
        "lbl_temperature": "Temperatuur",
        "lbl_condition_prev": "Conditioneren op vorige tekst",
        "lbl_logprob": "Log-prob drempelwaarde",
        "lbl_no_speech": "Drempel voor stilte",
        "tab_transcript": "Transcriptie",
        "tab_ai_engine": "AI-engine",
        "lbl_chunk_max_words": "Max woorden per blok",
        "lbl_chunk_lookahead": "Vooruitkijkwoorden",
        "lbl_chunk_min_chars": "Min tekens per blok",
        "lbl_compute_type": "Rekentype",
        "lbl_initial_prompt": "Initieel verzoek",
        "lbl_patience": "Geduld",
        "lbl_compression_ratio": "Compressieverhouding",
        "lbl_no_repeat_ngram": "Geen N-Gram herhalen",
        "lbl_regroup": "Hergroeperen",
        "lbl_suppress_silence": "Stilte onderdrukken",
        "lbl_q_levels": "Q Niveaus",
        "lbl_k_size": "K Grootte",
        "msg_restart_required": "Wijzigingen toegepast. Het volledige effect is zichtbaar bij de volgende start.",
        "msg_restart_lang_pending": "Taal gewijzigd. Het volledige effect is zichtbaar bij herstart.",
        "msg_unsaved_title": "Niet-opgeslagen wijzigingen",
        "msg_unsaved_desc": "U heeft niet-opgeslagen wijzigingen. Wat wilt u doen?\n\n",
        "btn_discard": "Weggooien",
        "btn_save_all": "Alles opslaan",
        "btn_discard_all": "Alles weggooien",
        "btn_cancel": "Annuleren",
        "msg_assembly_failed": "Montage mislukt. Controleer het logbestand.",
        "msg_load_project_failed": "Project kon niet worden geladen",
        "msg_fs_error": "Standalone Silence Fout",
        "search_placeholder": "Zoeken...",
        "search_results_counter_empty": "0/0",
        "search_tooltip_prev": "Vorige (Shift+Enter)",
        "search_tooltip_next": "Volgende (Enter)",
        "search_tooltip_close": "Sluiten (Esc)",
        # ── Standalone Silence Detection ───────────────────────────────────────
        "btn_fast_silence_detection": "Stiltedetectie",
        "btn_standalone_silence_detection": "Stiltedetectie",
        "lbl_fast_silence_workspace": "Stiltedetectie",
        "lbl_standalone_silence_workspace": "Stiltedetectie",
        "msg_fast_silence": "Stiltedetectie",
        "msg_standalone_silence": "Stiltedetectie",
        "msg_fast_silence_processing_c": "Stiltedetectie voltooid.",
        "msg_standalone_silence_processing_c": "Stiltedetectie voltooid.",
        "btn_run_fast_silence": "Uitvoeren",
        "btn_run_standalone_silence": "Uitvoeren",
        "txt_initializing_fast_silence": "Stiltedetectie initialiseren...",
        "txt_initializing_standalone_silence": "Stiltedetectie initialiseren...",
        # ── Timeline / Track selection ──────────────────────────────────────────
        "msg_no_timelines_detected": "Geen timelines gedetecteerd",
        "msg_no_audio_tracks_detected": "Geen audiosporen gedetecteerd",
        "txt_all_tracks": "Alle sporen",
        "msg_transcription_source": "BadWords - Bron timeline: <i>{tl}</i> - Sporen: <i>{tr}</i>",
        "txt_all": "Alle",
        "lbl_xml_preserve_track_order": "Originele volgorde van audiosporen bewaren",
        "tt_xml_preserve_track_order": "Indien ingeschakeld, behouden audiosporen hun oorspronkelijke indices in de uitvoer-tijdlijn (bijv. A3 blijft A3). Indien uitgeschakeld (standaard), worden sporen opeenvolgend hernummerd vanaf A1.",
        "status_assembly_xml_build": "XML-montage bouwen...",
        "status_assembly_xml_import": "XML-tijdlijn importeren...",
        "status_assembly_colors": "Clipkleuren toepassen...",
        "lbl_timestamp_precise": "Nauwkeurige tijdstempels (ms)",
        "tt_timestamp_precise": "Toont volledige millisecondeprecisie in segmenttijdstempels (bijv. [01:07.986] in plaats van [01:08]).",
    },
    'ru': {
        "lbl_hidden_start": "Начало аудио распознано как неразборчивое, пропущено.",
        "btn_show_anyway": "все равно показать",
        "chk_sync_davinci": "Синхронизировать таймлайн DaVinci при смене редакции",
        "tt_refresh_timelines": "Обновить таймлайны",
        "tt_reset_to_default": "Сбросить по умолчанию",
        "tt_advanced_locked": "Доступно в следующих подверсиях BadWords 3.0",
        "btn_analyze": "Анализ",
        "btn_analyze_compare": "Анализ (Сравнение)",
        "btn_analyze_standalone": "Анализ (Автономный)",
        "btn_apply": "Применить",
        "btn_assemble": "Собрать",
        "btn_clear": "Очистить",
        "btn_close": "Закрыть",
        "btn_export_project": "Экспорт Проекта",
        "btn_fast_silence_detection": "Быстрое Обнаружение Тишины",
        "btn_import_project": "Импорт Проекта",
        "btn_import_script": "Импорт Сценария",
        "btn_restart_later": "Перезапустить позже",
        "btn_restart_now": "Перезапустить сейчас",
        "btn_restore_defaults": "Сбросить Всё",
        "btn_run_fast_silence": "Запуск Быстрой Тишины",
        "btn_save": "Сохранить",
        "btn_back_to_transcription": "Назад к Транскрипции",
        "lbl_font_preview": "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
        "lbl_add_custom_marker": "+ добавить пользовательский маркер...",
        "lbl_cut_silence_directly": "Вырезать тишину напрямую",
        "lbl_detect_and_cut_silence": "Обнаружить и вырезать тишину",
        "lbl_detect_and_mark_silence": "Обнаружить и пометить тишину",
        "lbl_device": "Устройство",
        "lbl_display_mode": "Режим Отображения",
        "lbl_fast_silence_workspace": "Пространство Быстрой Тишины",
        "lbl_font_size_pt": "Размер шрифту (pt)",
        "lbl_initializing": "Инициализация...",
        "lbl_lang": "Язык",
        "lbl_language": "Язык",
        "lbl_view_mode": "Вид Настроек (Базовый/Расширенный)",
        "lbl_line_spacing_px": "Межстрочный интервал (px)",
        "lbl_loading": "Загрузка...",
        "lbl_mark_filler_words_automat": "Автоматически помечать слова-паразиты",
        "lbl_mark_inaudible_fragments": "Помечать неразборчивые фрагменты",
        "lbl_mark_silence_with_color": "Помечать тишину цветом",
        "lbl_marking_mode": "Режим Пометки",
        "lbl_model": "Модель",
        "lbl_offset_s": "Сдвиг (с)",
        "lbl_padding_s": "Отступ (с)",
        "lbl_pinned_favorites": "Закрепленные Инструменты",
        "lbl_ripple_delete_red_clips": "Удалить красные клипы (<span style='color: #ed4245;'>Ripple Delete</span>)",
        "lbl_show_detected_typos": "Показать обнаруженные опечатки",
        "lbl_show_inaudible_fragments": "Показать неразборчивые фрагменты",
        "lbl_silence_threshold_db": "Порог тишины (dB)",
        "lbl_snap_max_s": "Прилипание макс. (с)",
        "lbl_threshold_db": "Порог (dB)",
        "lbl_min_silence_dur": "Мин. длительность тишины (с)",
        "lbl_timeline_selection": "Выбор Таймлайна",
        "lbl_tracks_selection": "Выбор Дорожек",
        "lbl_transcript_font": "Шрифт транскрипта",
        "lbl_transcription_workspace": "Пространство Транскрипции",
        "lbl_words": "слов",
        "msg_analysis_failed": "Анализ не удался.",
        "msg_fast_silence": "Быстрая Тишина",
        "msg_fast_silence_processing_c": "Обработка тишины завершена.",
        "msg_no_active_transcription_t": "Активный таймлайн не обнаружен.",
        "msg_no_silence_segments_detec": "Сегменты тишины не обнаружены.",
        "msg_please_import_or_paste_a": "Пожалуйста, сначала импортируйте или вставьте сценарий.",
        "msg_restart_lang": "Язык изменен. Перезапустите BadWords.",
        "msg_success": "Успех",
        "msg_the_transcription_process": "Процесс транскрипции завершен.",
        "msg_timeline_assembled_succes": "Таймлайн успешно собран.",
        "msg_title_language_changed": "Язык Изменен",
        "msg_title_settings": "Настройки",
        "msg_title_restore_defaults": "По умолчанию",
        "msg_restore_defaults_confirm": "Вы уверены, что хотите сбросить все настройки к значениям по умолчанию?",
        "opt_continuous_flow": "Непрерывный текст",
        "lbl_error": "Ошибка",
        "txt_detecting_silence": "Обнаружение тишины...",
        "txt_building_data": "Сборка структуры данных...",
        "txt_fast_silence_complete": "Быстрая Тишина завершена.",
        "txt_slowing_audio": "Замедление аудио...",
        "txt_enhancing_audio": "Улучшение аудио...",
        "txt_transcribing_pct": "Транскрибирование...",
        "txt_detecting_source": "Определение оптимального источника...",
        "txt_calculating_cuts": "Вычисление разрезов...",
        "msg_warning": "Предупреждение",
        "opt_segmented_blocks": "Сегментированные блоки",
        "ph_paste_script_here": "Вставьте сценарий сюда...",
        "ph_search": "Поиск...",
        "rad_blue_retake": "Синий (Дубль)",
        "rad_eraser_clear": "Ластик (Очистить)",
        "rad_green_typo": "Зелёный (Опечатка)",
        "rad_red_cut_filler": "Красный (Вырезать/Мусор)",
        "tab_appearance": "Внешний вид",
        "tab_audio_sync": "Синхронізація",
        "tab_general": "Общие",
        "tool_assembly": "Сборка",
        "tool_filler_words": "Слова-паразиты",
        "tool_main_panel": "Главная Панель",
        "tool_mark_inaudible": "Пометить Неразборчивое",
        "tool_quit": "Выход",
        "tool_ripple_delete": "Ripple Delete",
        "tool_script_analysis": "Анализ Сценария",
        "tool_settings": "Настройки",
        "tool_show_inaudible": "Показать Неразборчивое",
        "tool_show_typos": "Показать Опечатки",
        "tool_silence": "Тишина",
        "tooltip_clear_all_markings": "Очистить все пометки",
        "tooltip_dev": "Функция в разработке",
        "tt_revert_to_default": "Вернуть по умолчанию",
        "txt_assembling_timeline": "Сборка таймлайна...",
        "txt_finishing": "Завершение...",
        "txt_initializing_analysis": "Инициализация анализа...",
        "txt_initializing_assembly": "Инициализация сборки...",
        "txt_initializing_fast_silence": "Инициализация Быстрой Тишины...",
        "txt_save": "Сохранить",
        "txt_saved": "Сохранено!",
        "txt_select": "Выбрать...",
        "txt_select_tracks": "Выбрать дорожки...",
        "msg_engine_render": "Рендеринг аудио...",
        "msg_engine_check_model": "Проверка модели...",
        "msg_engine_whisper_run": "Запуск Whisper...",
        "msg_engine_whisper_init": "Инициализация транскрипции...",
        "msg_engine_silence": "Обнаружение тишины...",
        "msg_engine_processing": "Обработка...",
        "msg_engine_init_analysis": "Завершение данных...",
        "msg_engine_fast_silence_detect": "Обнаружение тишины (FFmpeg)...",
        "msg_engine_fast_silence_build": "Сборка структуры данных...",
        "msg_engine_fast_silence_done": "Быстрая Тишина завершена.",
        "msg_engine_assembly_init": "Инициализация сборки...",
        "msg_engine_assembly_source": "Определение оптимального источника...",
        "msg_engine_slow_audio": "Замедление аудио...",
        "btn_ok": "ОК",
        "btn_view_basic": "Базовый Вид",
        "btn_view_advanced": "Расширенный Вид",
        "lbl_chunk_punct_count": "Знаки препинания на блок",
        "msg_restore_title": "Сбросить Всё",
        "msg_restore_desc": "Все настройки будут сброшены до значений по умолчанию. Вы уверены?",
        "msg_clear_title": "Очистить Транскрипт",
        "msg_clear_desc": "Вы уверены, что хотите удалить все пометки?",
        "status_render": "Рендеринг аудио...",
        "status_direct_source": "Прямое извлечение аудио из источника...",
        "status_slow": "Замедление аудио...",
        "status_check_model": "Проверка модели...",
        "status_downloading_model": "Загрузка модели...",
        "status_downloading_pct": "Загрузка модели: {pct}%",
        "first_run_hint_1": "Первый запуск требует времени — AI локально кэширует свои веса.",
        "first_run_hint_2": "Разогреваем локальный движок AI. В следующий раз будет летать!",
        "first_run_hint_3": "AI распаковывает свои виртуальные чемоданы на вашем диске.",
        "first_run_hint_4": "AI оптимизируется под ваше железо. Это делается только один раз.",
        "first_run_hint_5": "Первый запуск всегда самый долгий. Обещаю, потом будет быстрее!",
        "first_run_hint_6": "Компилируем локальные слои модели. Уже почти...",
        "first_run_hint_7": "AI знакомится с вашей операционной системой.",
        "first_run_hint_8": "Настраиваем локальную среду. Отличный момент выпить воды.",
        "first_run_hint_9": "Выделяем память для AI... Первичная инициализация требует времени.",
        "first_run_hint_10": "AI впервые калибрует свои локальные параметры.",
        "analysis_hint_1": "AI внимательно вслушивается в ваше аудио. Тшшш...",
        "analysis_hint_2": "Превращаем звуковые волны в текст со скоростью света.",
        "analysis_hint_3": "AI безжалостно фиксирует каждое ваше 'э-э-э' и 'м-м-м'.",
        "analysis_hint_4": "Извлекаем слова прямо из звукового потока...",
        "analysis_hint_5": "AI читает ваш аудиофайл на максимальной скорости.",
        "analysis_hint_6": "Превращаем поток мыслей в стройный, грамотный текст.",
        "analysis_hint_7": "Сканируем аудиодорожку. Ни одно слово не ускользнет от AI.",
        "analysis_hint_8": "AI работает со звуком, как скульптор с цифровой глиной.",
        "analysis_hint_9": "Точно нарезаем аудио для идеальной синхронизации на таймлайне.",
        "analysis_hint_10": "Полируем транскрипцию до блеска. Почти закончили!",
        "status_whisper_init": "Инициализация транскрипции...",
        "status_transcribing": "Транскрибирование...",
        "status_silence": "Обнаружение тишины...",
        "status_process": "Обработка...",
        "status_finalize": "Финализация данных...",
        "status_assembly_init": "Инициализация сборки...",
        "status_assembly_source": "Обнаружение оптимального источника...",
        "status_calc_cuts": "Вычисление разрезов...",
        "status_assembly_resolve": "Сборка в Resolve...",
        "status_assembly_clips": "Сборка клипов...",
        "msg_quit_title": "Выйти из BadWords",
        "msg_quit_desc": "Вы уверены, что хотите выйти?",
        "btn_yes": "Да",
        "btn_no": "Нет",
        "title_telemetry": "Статистика BadWords",
        "msg_telemetry": "Привет! 👋\n\nЧтобы помочь улучшить BadWords, вы согласны отправить одноразовый, 100% анонимный пинг после установки / обновления?\n\nОтправляются только версия приложения и тип ОС. Никакие аудиофайлы или личные данные НИКОГДА не собираются.\n\nГеолокация не является обязательной — снимите флажок ниже, чтобы отключить ее.\n\nВы можете изменить эти настройки в любое время в Настройки → Телеметрия.",
        "btn_telemetry_yes": "Я согласен",
        "btn_telemetry_no": "Нет, спасибо",
        "chk_telemetry_geo": "Поделиться своим общим местоположением",
        "tab_interface": "Интерфейс",
        "lbl_always_on_top": "Всегда поверх окон",
        "lbl_hidden_panels": "Скрытые панели",
        "btn_export_settings": "Экспорт настроек",
        "btn_import_settings": "Импорт настроек",
        "tab_algorithms": "Алгоритмы",
        "tab_shortcuts": "Горячие клавиши",
        "lbl_algo_fuzzy": "Порог нечёткого совпадения (%)",
        "lbl_algo_lookahead": "Просмотр вперёд для ретейка (слова)",
        "lbl_algo_penalty": "Штраф за расстояние",
        "lbl_algo_anchor": "Глубина якоря",
        "shortcut_mark_red": "Красный маркер",
        "shortcut_mark_blue": "Синий маркер",
        "shortcut_mark_green": "Зелёный маркер",
        "shortcut_mark_eraser": "Ластик",
        "shortcut_jump_to_word": "Перейти к слову",
        "shortcut_play_stop": "Воспроизвести / Остановить",
        "shortcut_search": "Поиск",
        "shortcut_open_settings": "Открыть настройки",
        "shortcut_none": "(нет)",
        "shortcut_custom_marker_fmt": "Переключиться на маркер \"{name}\"",
        "tab_custom_markers": "Пользовательские маркеры",
        "tab_telemetry": "Телеметрия",
        "btn_add_marker": "Добавить маркер",
        "btn_export_markers": "Экспортировать",
        "btn_import_markers": "Импортировать",
        "msg_no_markers_to_export": "Нет пользовательских маркеров для экспорта.",
        "msg_import_invalid_format": "Неверный формат файла. Ожидается JSON-файл со списком \"custom_markers\".",
        "msg_import_no_valid_markers": "В файле не найдено допустимых маркеров.",
        "btn_remove_marker": "Удалить маркер",
        "btn_edit_marker": "Редактировать",
        "resolve_color_orange": "Оранжевый",
        "resolve_color_apricot": "Абрикосовый",
        "resolve_color_yellow": "Жёлтый",
        "resolve_color_lime": "Лаймовый",
        "resolve_color_green": "Зелёный",

        "resolve_color_teal": "Сине-зелёный",
        "resolve_color_blue": "Синий",
        "resolve_color_purple": "Фиолетовый",
        "resolve_color_pink": "Розовый",
        "resolve_color_beige": "Бежевый",
        "resolve_color_brown": "Коричневый",
        "placeholder_marker_name": "напр. Мой маркер",
        "lbl_marker_name": "Название маркера",
        "lbl_marker_color": "Цвет",
        "lbl_ver_pending_restart": "↓ Обновление установлено — перезапустите для применения",
        "chk_telemetry_opt_in": "Включить анонимную телеметрию",
        "msg_telemetry_settings": "Мы собираем минимальные анонимные данные (версия ОС, версия приложения) для улучшения BadWords. Аудио или личные файлы никогда не отправляются.",
        "link_repo_label": "BadWords на GitHub",
        "link_repo_desc": "Просматривайте исходный код, сообщайте о проблемах и следите за проектом",
        "link_coffee_label": "Угостите меня кофе",
        "link_coffee_desc": "Поддержите разработку BadWords",
        "msg_compute_fail_title": "Железо не поддерживается",
        "msg_compute_fail_desc": "Ваш CPU/GPU не поддерживает выбранный тип вычислений. Выберите другой тип или используйте 'Auto'.",
        "lbl_accent_color": "Цвет акцента",
        "lbl_app_icon": "Иконка приложения",
        "lbl_vad_filter": "Фильтр VAD",
        "lbl_beam_size": "Размер пучка",
        "lbl_temperature": "Температура",
        "lbl_condition_prev": "Условие на предыдущем тексте",
        "lbl_logprob": "Порог log-prob",
        "lbl_no_speech": "Порог отсутствия речи",
        "tab_transcript": "Транскрипция",
        "tab_ai_engine": "Движок АИ",
        "lbl_chunk_max_words": "Макс. слов в блоке",
        "lbl_chunk_lookahead": "Слова забега",
        "lbl_chunk_min_chars": "Мин. символов в блоке",
        "lbl_compute_type": "Тип вычислений",
        "lbl_initial_prompt": "Начальный промпт",
        "lbl_patience": "Терпение (Patience)",
        "lbl_compression_ratio": "Коэффициент сжатия",
        "lbl_no_repeat_ngram": "Не повторять N-Gram",
        "lbl_regroup": "Перегруппировка",
        "lbl_suppress_silence": "Подавление тишины",
        "lbl_q_levels": "Уровни Q",
        "lbl_k_size": "Размер K",
        "msg_restart_required": "Изменения применены. Полный эффект будет виден при следующем запуске.",
        "msg_restart_lang_pending": "Язык изменен. Полные изменения вступят в силу после перезапуска.",
        "msg_unsaved_title": "Несохраненные изменения",
        "msg_unsaved_desc": "У вас есть несохраненные изменения. Что бы вы хотели сделать?\n\n",
        "btn_discard": "Отменить",
        "btn_save_all": "Сохранить все",
        "btn_discard_all": "Отменить все",
        "btn_cancel": "Отмена",
        "msg_assembly_failed": "Монтаж не удался. Проверьте журнал.",
        "msg_load_project_failed": "Не удалось загрузить проект",
        "msg_fs_error": "Ошибка Standalone Silence",
        "search_placeholder": "Поиск...",
        "search_results_counter_empty": "0/0",
        "search_tooltip_prev": "Предыдущий (Shift+Enter)",
        "search_tooltip_next": "Следующий (Enter)",
        "search_tooltip_close": "Закрыть (Esc)",
        # ── Standalone Silence Detection ───────────────────────────────────────
        "btn_fast_silence_detection": "Обнаружение тишины",
        "btn_standalone_silence_detection": "Обнаружение тишины",
        "lbl_fast_silence_workspace": "Обнаружение тишины",
        "lbl_standalone_silence_workspace": "Обнаружение тишины",
        "msg_fast_silence": "Обнаружение тишины",
        "msg_standalone_silence": "Обнаружение тишины",
        "msg_fast_silence_processing_c": "Обнаружение завершено.",
        "msg_standalone_silence_processing_c": "Обнаружение завершено.",
        "btn_run_fast_silence": "Запуск",
        "btn_run_standalone_silence": "Запуск",
        "txt_initializing_fast_silence": "Инициализация обнаружения...",
        "txt_initializing_standalone_silence": "Инициализация обнаружения...",
        # ── Fix: Russian tab_audio_sync was Ukrainian ─────────────────────────
        "tab_audio_sync": "Синхронизация аудио",
        # ── Timeline / Track selection ──────────────────────────────────────────
        "msg_no_timelines_detected": "Таймлайны не обнаружены",
        "msg_no_audio_tracks_detected": "Аудиодорожки не обнаружены",
        "txt_all_tracks": "Все дорожки",
        "msg_transcription_source": "BadWords - Исходный таймлайн: <i>{tl}</i> - Дорожки: <i>{tr}</i>",
        "txt_all": "Все",
        "lbl_xml_preserve_track_order": "Сохранить оригинальный порядок аудиодорожек",
        "tt_xml_preserve_track_order": "При включении аудиодорожки сохраняют исходные индексы в итоговой временной шкале (напр. A3 остаётся A3). При отключении (по умолчанию) дорожки переназначаются последовательно начиная с A1.",
        "status_assembly_xml_build": "Построение XML-монтажа...",
        "status_assembly_xml_import": "Импорт XML-временной шкалы...",
        "status_assembly_colors": "Применение цветов клипов...",
        "lbl_timestamp_precise": "Точные временные метки (мс)",
        "tt_timestamp_precise": "Показывает полную точность в миллисекундах в временных метках сегментов (напр. [01:07.986] вместо [01:08]).",
    },
}

def get_trans(key, lang_code="en"):
    """
    Safely retrieves translation for a given key.
    Falls back to English if key/language missing.
    """
    lang_dict = TRANS.get(lang_code, TRANS["en"])
    return lang_dict.get(key, TRANS["en"].get(key, f"[{key}]"))


# ── CSD Title Bar i18n ─────────────────────────────────────────────────────
# Injected here so each individual language dict stays unchanged.
_CSD_TB_I18N = {
    'en': ('Minimize', 'Maximize'),
    'pl': ('Minimalizuj', 'Maksymalizuj'),
    'de': ('Minimieren', 'Maximieren'),
    'es': ('Minimizar', 'Maximizar'),
    'fr': ('Réduire', 'Agrandir'),
    'it': ('Riduci a icona', 'Ingrandisci'),
    'pt': ('Minimizar', 'Maximizar'),
    'uk': ('Згорнути', 'Розгорнути'),
    'nl': ('Minimaliseren', 'Maximaliseren'),
    'ru': ('Свернуть', 'Развернуть'),
}
for _lang_code, (_min_txt, _max_txt) in _CSD_TB_I18N.items():
    TRANS.setdefault(_lang_code, {}).update({
        'btn_minimize': _min_txt,
        'btn_maximize': _max_txt,
    })
del _CSD_TB_I18N, _lang_code, _min_txt, _max_txt


# ── Auto-Update i18n ──────────────────────────────────────────────────────────
# Injected here so individual language dicts stay unchanged.
_UPDATE_I18N = {
    'en': {
        'lbl_auto_check_updates':    'Notify me about new versions',
        'tt_auto_check_updates':     'Show a popup notification when a new BadWords release is detected. Updates are always checked silently in the background.',
        'lbl_auto_update_on_start':  'Auto-update on startup',
        'tt_auto_update_on_start':   'Automatically download and apply updates before BadWords loads. Requires "Auto check" to be enabled.',
        'lbl_ver_installed':         'Installed version',
        'lbl_ver_up_to_date':        '✓ Up to date',
        'lbl_ver_update_avail':      '↑ Update available:',
        'lbl_ver_pending_restart':   '↓ Update installed — restart to apply',
        'btn_settings_update_now':   'Update Now',
        'update_notify_auto_updating': 'Checking for updates…',
        'update_notify_title':       'Update Available',
        'update_notify_sub':         'A new version of BadWords is ready!',
        'update_notify_lbl_current': 'INSTALLED',
        'update_notify_lbl_latest':  'NEW VERSION',
        'update_notify_win_btn':     'Download from GitHub',
        'update_notify_btn_update':  'Update Now',
        'update_notify_updating':    'Updating…',
        'update_notify_wait':        'Please wait while the update is applied.',
        'update_notify_success':     'Update complete! Please restart BadWords.',
        'update_notify_failed':      'Update failed. Please try again or update manually.',
        'update_notify_btn_later':   'Remind me later',
        'update_notify_btn_dismiss': "Don't notify me again",
        'lbl_ultra_precise': "Ultra Precise Mode (Chunking)",
        'tt_ultra_precise': "Can greatly improve transcription precision even on weaker\nmodels — catching every stutter, hesitation and repetition\nverbatim. Especially recommended for longer recordings\nwhere standard mode tends to skip or merge small details.\n\nNote: This option can take noticeably more time to process,\ndepending on your hardware and platform\n(GPU acceleration is only available on NVIDIA)"
    },
    'pl': {
        'lbl_auto_check_updates':    'Powiadamiaj o nowych wersjach',
        'tt_auto_check_updates':     'Pokazuje powiadomienie gdy wykryta zostanie nowsza wersja BadWords. Sprawdzanie odbywa się zawsze w tle.',
        'lbl_auto_update_on_start':  'Auto-aktualizacja przy starcie',
        'tt_auto_update_on_start':   'Automatycznie pobiera i instaluje aktualizacje przed załadowaniem BadWords. Wymaga włączonej opcji "Auto check".',
        'lbl_ver_installed':         'Zainstalowana wersja',
        'lbl_ver_up_to_date':        '✓ Aktualna',
        'lbl_ver_update_avail':      '↑ Dostępna aktualizacja:',
        'lbl_ver_pending_restart':   '↓ Aktualizacja zainstalowana — uruchom ponownie',
        'btn_settings_update_now':   'Aktualizuj',
        'update_notify_auto_updating': 'Sprawdzanie aktualizacji…',
        'update_notify_title':       'Dostępna aktualizacja',
        'update_notify_sub':         'Nowa wersja BadWords jest gotowa!',
        'update_notify_lbl_current': 'ZAINSTALOWANA',
        'update_notify_lbl_latest':  'NOWA WERSJA',
        'update_notify_win_btn':     'Pobierz z GitHub',
        'update_notify_btn_update':  'Aktualizuj',
        'update_notify_updating':    'Aktualizuję…',
        'update_notify_wait':        'Proszę czekać, aktualizacja w toku.',
        'update_notify_success':     'Gotowe! Uruchom ponownie BadWords.',
        'update_notify_failed':      'Aktualizacja nie powiodła się. Spróbuj ponownie.',
        'update_notify_btn_later':   'Przypomnij później',
        'update_notify_btn_dismiss': 'Nie powiadamiaj mnie ponownie',
        'lbl_ultra_precise': "Tryb Ultra Precyzyjny (Chunking)",
        'tt_ultra_precise': "Może znacznie poprawić precyzję transkrypcji nawet na słabszych\nmodelach — wyłapując co do słowa każde zająknięcie, zawahanie i powtórzenie.\nSzczególnie zalecane dla dłuższych nagrań,\ngdzie tryb standardowy ma tendencję do pomijania lub łączenia drobnych detali.\n\nUwaga: Przetwarzanie z tą opcją może potrwać zauważalnie dłużej,\nw zależności od twojego sprzętu i platformy\n(Akceleracja GPU jest dostępna tylko dla kart NVIDIA)",
    },
    'de': {
        'lbl_auto_check_updates':    'Über neue Versionen benachrichtigen',
        'tt_auto_check_updates':     'Zeigt eine Benachrichtigung an, wenn eine neue BadWords-Version erkannt wird. Prüfung läuft immer still im Hintergrund.',
        'lbl_auto_update_on_start':  'Automatisch beim Start aktualisieren',
        'tt_auto_update_on_start':   'Lädt Updates vor dem Laden von BadWords automatisch herunter. "Auto check" muss aktiviert sein.',
        'lbl_ver_installed':         'Installierte Version',
        'lbl_ver_up_to_date':        '✓ Aktuell',
        'lbl_ver_update_avail':      '↑ Update verfügbar:',
        'lbl_ver_pending_restart':   '↓ Update installiert — Neustart erforderlich',
        'btn_settings_update_now':   'Jetzt aktualisieren',
        'update_notify_auto_updating': 'Auf Updates prüfen…',
        'update_notify_title':       'Update verfügbar',
        'update_notify_sub':         'Eine neue Version von BadWords ist bereit!',
        'update_notify_lbl_current': 'INSTALLIERT',
        'update_notify_lbl_latest':  'NEUE VERSION',
        'update_notify_win_btn':     'Von GitHub herunterladen',
        'update_notify_btn_update':  'Jetzt aktualisieren',
        'update_notify_updating':    'Aktualisiere…',
        'update_notify_wait':        'Bitte warten, Update wird angewendet.',
        'update_notify_success':     'Fertig! Bitte BadWords neu starten.',
        'update_notify_failed':      'Update fehlgeschlagen. Bitte erneut versuchen.',
        'update_notify_btn_later':   'Später erinnern',
        'update_notify_btn_dismiss': 'Nicht mehr benachrichtigen',
        'lbl_ultra_precise': "Ultra Präziser Modus (Chunking)",
        'tt_ultra_precise': "Kann die Transkriptionspräzision selbst bei schwächeren Modellen\nerheblich verbessern — und erfasst jedes Stottern, Zögern und\nWiederholen wortgetreu. Besonders empfohlen für längere Aufnahmen,\nbei denen der Standardmodus oft kleine Details überspringt oder zusammenfasst.\n\nHinweis: Die Verarbeitung mit dieser Option kann deutlich mehr Zeit beanspruchen,\nabhängig von Ihrer Hardware und Plattform\n(GPU-Beschleunigung ist nur bei NVIDIA verfügbar)",
    },
    'es': {
        'lbl_auto_check_updates':    'Notificarme sobre nuevas versiones',
        'tt_auto_check_updates':     'Muestra un aviso cuando se detecta una versión más reciente. La comprobación se realiza siempre en segundo plano.',
        'lbl_auto_update_on_start':  'Actualizar automáticamente al iniciar',
        'tt_auto_update_on_start':   'Descarga e instala actualizaciones antes de cargar BadWords. Requiere "Auto check" activado.',
        'lbl_ver_installed':         'Versión instalada',
        'lbl_ver_up_to_date':        '✓ Al día',
        'lbl_ver_update_avail':      '↑ Actualización disponible:',
        'lbl_ver_pending_restart':   '↓ Actualización instalada — reinicia para aplicar',
        'btn_settings_update_now':   'Actualizar ahora',
        'update_notify_auto_updating': 'Buscando actualizaciones…',
        'update_notify_title':       'Actualización disponible',
        'update_notify_sub':         '¡Una nueva versión de BadWords está lista!',
        'update_notify_lbl_current': 'INSTALADA',
        'update_notify_lbl_latest':  'NUEVA VERSIÓN',
        'update_notify_win_btn':     'Descargar desde GitHub',
        'update_notify_btn_update':  'Actualizar ahora',
        'update_notify_updating':    'Actualizando…',
        'update_notify_wait':        'Por favor espera mientras se aplica la actualización.',
        'update_notify_success':     '¡Listo! Por favor reinicia BadWords.',
        'update_notify_failed':      'Actualización fallida. Por favor inténtalo de nuevo.',
        'update_notify_btn_later':   'Recordarme más tarde',
        'update_notify_btn_dismiss': 'No notificarme más',
        'lbl_ultra_precise': "Modo Ultra Preciso (Chunking)",
        'tt_ultra_precise': "Puede mejorar enormemente la precisión de la transcripción incluso en modelos\nmás débiles, capturando literalmente cada tartamudeo, vacilación y repetición.\nEspecialmente recomendado para grabaciones largas\ndonde el modo estándar tiende a omitir o fusionar pequeños detalles.\n\nNota: Esta opción puede tardar notablemente más tiempo en procesarse,\ndependiendo de su hardware y plataforma\n(La aceleración por GPU solo está disponible en NVIDIA)",
    },
    'fr': {
        'lbl_auto_check_updates':    'Me notifier des nouvelles versions',
        'tt_auto_check_updates':     "Affiche une notification quand une nouvelle version est détectée. La vérification s'effectue toujours en arrière-plan.",
        'lbl_auto_update_on_start':  'Mise à jour automatique au démarrage',
        'tt_auto_update_on_start':   'Télécharge et applique les mises à jour avant le chargement de BadWords. Nécessite "Auto check" activé.',
        'lbl_ver_installed':         'Version installée',
        'lbl_ver_up_to_date':        '✓ À jour',
        'lbl_ver_update_avail':      '↑ Mise à jour disponible :',
        'lbl_ver_pending_restart':   '↓ Mise à jour installée — redémarrez pour appliquer',
        'btn_settings_update_now':   'Mettre à jour',

        'update_notify_auto_updating': 'Vérification des mises à jour…',
        'update_notify_title':       'Mise à jour disponible',
        'update_notify_sub':         'Une nouvelle version de BadWords est prête !',
        'update_notify_lbl_current': 'INSTALLÉE',
        'update_notify_lbl_latest':  'NOUVELLE VERSION',
        'update_notify_win_btn':     'Télécharger depuis GitHub',
        'update_notify_btn_update':  'Mettre à jour',
        'update_notify_updating':    'Mise à jour en cours…',
        'update_notify_wait':        "Veuillez patienter pendant l'application de la mise à jour.",
        'update_notify_success':     'Terminé ! Veuillez redémarrer BadWords.',
        'update_notify_failed':      'Échec de la mise à jour. Veuillez réessayer.',
        'update_notify_btn_later':   'Me rappeler plus tard',
        'update_notify_btn_dismiss': 'Ne plus me notifier',
        'lbl_ultra_precise': "Mode Ultra Précis (Chunking)",
        'tt_ultra_precise': "Peut grandement améliorer la précision de la transcription, même sur des modèles\nplus faibles — en capturant littéralement chaque bégaiement, hésitation et répétition.\nParticulièrement recommandé pour les enregistrements plus longs\noù le mode standard a tendance à omettre ou fusionner de petits détails.\n\nRemarque : Cette option peut prendre considérablement plus de temps à traiter,\nselon votre matériel et votre plateforme\n(L'accélération GPU n'est disponible que sur NVIDIA)",
    },
    'it': {
        'lbl_auto_check_updates':    'Notificami delle nuove versioni',
        'tt_auto_check_updates':     'Mostra una notifica quando viene rilevata una nuova versione. Il controllo avviene sempre in background.',
        'lbl_auto_update_on_start':  'Aggiornamento automatico all\'avvio',
        'tt_auto_update_on_start':   'Scarica e applica gli aggiornamenti prima del caricamento di BadWords. Richiede "Auto check" attivo.',
        'lbl_ver_installed':         'Versione installata',
        'lbl_ver_up_to_date':        '✓ Aggiornato',
        'lbl_ver_update_avail':      '↑ Aggiornamento disponibile:',
        'lbl_ver_pending_restart':   '↓ Aggiornamento installato — riavvia per applicare',
        'btn_settings_update_now':   'Aggiorna ora',
        'update_notify_auto_updating': 'Verifica aggiornamenti…',
        'update_notify_title':       'Aggiornamento disponibile',
        'update_notify_sub':         'Una nuova versione di BadWords è pronta!',
        'update_notify_lbl_current': 'INSTALLATA',
        'update_notify_lbl_latest':  'NUOVA VERSIONE',
        'update_notify_win_btn':     'Scarica da GitHub',
        'update_notify_btn_update':  'Aggiorna ora',
        'update_notify_updating':    'Aggiornamento in corso…',
        'update_notify_wait':        "Attendere mentre l'aggiornamento viene applicato.",
        'update_notify_success':     'Fatto! Per favore riavvia BadWords.',
        'update_notify_failed':      'Aggiornamento fallito. Riprova.',
        'update_notify_btn_later':   'Ricordamelo più tardi',
        'update_notify_btn_dismiss': 'Non notificarmi più',
        'lbl_ultra_precise': "Modalità Ultra Precisa (Chunking)",
        'tt_ultra_precise': "Può migliorare notevolmente la precisione della trascrizione anche su modelli\npiù deboli, catturando alla lettera ogni balbuzie, esitazione e ripetizione.\nParticolarmente consigliato per le registrazioni più lunghe\nin cui la modalità standard tende a saltare o unire piccoli dettagli.\n\nNota: L'elaborazione con questa opzione può richiedere molto più tempo,\na seconda dell'hardware e della piattaforma\n(L'accelerazione GPU è disponibile solo su NVIDIA)",
    },
    'pt': {
        'lbl_auto_check_updates':    'Notificar-me sobre novas versões',
        'tt_auto_check_updates':     'Exibe uma notificação quando uma nova versão é detectada. A verificação ocorre sempre em segundo plano.',
        'lbl_auto_update_on_start':  'Atualizar automaticamente ao iniciar',
        'tt_auto_update_on_start':   'Baixa e aplica atualizações antes de carregar o BadWords. Requer "Auto check" ativado.',
        'lbl_ver_installed':         'Versão instalada',
        'lbl_ver_up_to_date':        '✓ Atualizado',
        'lbl_ver_update_avail':      '↑ Atualização disponível:',
        'lbl_ver_pending_restart':   '↓ Atualização instalada — reinicie para aplicar',
        'btn_settings_update_now':   'Atualizar agora',
        'update_notify_auto_updating': 'Verificando atualizações…',
        'update_notify_title':       'Atualização disponível',
        'update_notify_sub':         'Uma nova versão do BadWords está pronta!',
        'update_notify_lbl_current': 'INSTALADA',
        'update_notify_lbl_latest':  'NOVA VERSÃO',
        'update_notify_win_btn':     'Baixar do GitHub',
        'update_notify_btn_update':  'Atualizar agora',
        'update_notify_updating':    'Atualizando…',
        'update_notify_wait':        'Aguarde enquanto a atualização é aplicada.',
        'update_notify_success':     'Concluído! Por favor reinicie o BadWords.',
        'update_notify_failed':      'Atualização falhou. Por favor tente novamente.',
        'update_notify_btn_later':   'Lembrar mais tarde',
        'update_notify_btn_dismiss': 'Não me notificar mais',
        'lbl_ultra_precise': "Modo Ultra Preciso (Chunking)",
        'tt_ultra_precise': "Pode melhorar muito a precisão da transcrição mesmo em modelos\nmais fracos — capturando literalmente cada gagueira, hesitação e repetição.\nEspecialmente recomendado para gravações mais longas,\nonde o modo padrão tende a pular ou fundir pequenos detalhes.\n\nNota: Esta opção pode levar visivelmente mais tempo para ser processada,\ndependendo do seu hardware e plataforma\n(A aceleração de GPU só está disponível na NVIDIA)",
    },
    'uk': {
        'lbl_auto_check_updates':    'Сповіщати про нові версії',
        'tt_auto_check_updates':     'Показує сповіщення при виявленні нової версії. Перевірка завжди відбувається у фоні.',
        'lbl_auto_update_on_start':  'Автооновлення при запуску',
        'tt_auto_update_on_start':   'Завантажує та застосовує оновлення до завантаження BadWords. Потребує увімкненої опції "Auto check".',
        'lbl_ver_installed':         'Встановлена версія',
        'lbl_ver_up_to_date':        '✓ Актуальна',
        'lbl_ver_update_avail':      '↑ Доступне оновлення:',
        'lbl_ver_pending_restart':   '↓ Оновлення встановлено — перезапустітььдля застосування',
        'btn_settings_update_now':   'Оновити зараз',
        'update_notify_auto_updating': 'Перевірка оновлень…',
        'update_notify_title':       'Доступне оновлення',
        'update_notify_sub':         'Нова версія BadWords готова!',
        'update_notify_lbl_current': 'ВСТАНОВЛЕНА',
        'update_notify_lbl_latest':  'НОВА ВЕРСІЯ',
        'update_notify_win_btn':     'Завантажити з GitHub',
        'update_notify_btn_update':  'Оновити зараз',
        'update_notify_updating':    'Оновлення…',
        'update_notify_wait':        'Зачекайте, оновлення застосовується.',
        'update_notify_success':     'Готово! Будь ласка, перезапустіть BadWords.',
        'update_notify_failed':      'Оновлення не вдалося. Спробуйте ще раз.',
        'update_notify_btn_later':   'Нагадати пізніше',
        'update_notify_btn_dismiss': 'Більше не сповіщати',
        'lbl_ultra_precise': "Ультраточний режим (Чанкінг)",
        'tt_ultra_precise': "Може значно покращити точність транскрипції навіть на слабших\nмоделях — дослівно вловлюючи кожне заїкання, заминку та повторення.\nОсобливо рекомендується для довгих записів,\nде стандартний режим має тенденцію пропускати або об'єднувати дрібні деталі.\n\nПримітка: Обробка з цією опцією може зайняти помітно більше часу,\nзалежно від вашого обладнання та платформи\n(Прискорення GPU доступне лише на NVIDIA)",
    },
    'nl': {
        'lbl_auto_check_updates':    'Mij informeren over nieuwe versies',
        'tt_auto_check_updates':     'Toont een melding wanneer een nieuwe versie wordt gedetecteerd. De controle verloopt altijd op de achtergrond.',
        'lbl_auto_update_on_start':  'Automatisch bijwerken bij opstarten',
        'tt_auto_update_on_start':   'Downloadt en installeert updates vóór het laden van BadWords. Vereist ingeschakelde "Auto check".',
        'lbl_ver_installed':         'Geïnstalleerde versie',
        'lbl_ver_up_to_date':        '✓ Up-to-date',
        'lbl_ver_update_avail':      '↑ Update beschikbaar:',
        'lbl_ver_pending_restart':   '↓ Update geïnstalleerd — herstart om toe te passen',
        'btn_settings_update_now':   'Nu updaten',
        'update_notify_auto_updating': 'Controleren op updates…',
        'update_notify_title':       'Update beschikbaar',
        'update_notify_sub':         'Een nieuwe versie van BadWords is klaar!',
        'update_notify_lbl_current': 'GEÏNSTALLEERD',
        'update_notify_lbl_latest':  'NIEUWE VERSIE',
        'update_notify_win_btn':     'Downloaden van GitHub',
        'update_notify_btn_update':  'Nu updaten',
        'update_notify_updating':    'Updaten…',
        'update_notify_wait':        'Even geduld, de update wordt toegepast.',
        'update_notify_success':     'Klaar! Herstart BadWords om de update toe te passen.',
        'update_notify_failed':      'Update mislukt. Probeer het opnieuw.',
        'update_notify_btn_later':   'Herinner me later',
        'update_notify_btn_dismiss': 'Niet meer melden',
        'lbl_ultra_precise': "Ultra Precieze Modus (Chunking)",
        'tt_ultra_precise': "Kan de transcriptieprecisie aanzienlijk verbeteren, zelfs op zwakkere\nmodellen — en legt elke stottering, aarzeling en herhaling woordelijk vast.\nVooral aanbevolen voor langere opnames\nwaarbij de standaardmodus vaak kleine details overslaat of samenvoegt.\n\nOpmerking: Het verwerken met deze optie kan merkbaar meer tijd kosten,\nafhankelijk van uw hardware en platform\n(GPU-versnelling is alleen beschikbaar op NVIDIA)",
    },
    'ru': {
        'lbl_auto_check_updates':    'Уведомлять о новых версиях',
        'tt_auto_check_updates':     'Показывает уведомление при обнаружении новой версии. Проверка всегда выполняется в фоне.',
        'lbl_auto_update_on_start':  'Авто-обновление при запуске',
        'tt_auto_update_on_start':   'Загружает и применяет обновления до загрузки BadWords. Требует включённой опции "Auto check".',
        'lbl_ver_installed':         'Установленная версия',
        'lbl_ver_up_to_date':        '✓ Актуальна',
        'lbl_ver_update_avail':      '↑ Доступно обновление:',
        'lbl_ver_pending_restart':   '↓ Обновление установлено — перезапустите для применения',
        'btn_settings_update_now':   'Обновить сейчас',
        'update_notify_auto_updating': 'Проверка обновлений…',
        'update_notify_title':       'Доступно обновление',
        'update_notify_sub':         'Новая версия BadWords готова к установке!',
        'update_notify_lbl_current': 'УСТАНОВЛЕНА',
        'update_notify_lbl_latest':  'НОВАЯ ВЕРСИЯ',
        'update_notify_win_btn':     'Скачать с GitHub',
        'update_notify_btn_update':  'Обновить сейчас',
        'update_notify_updating':    'Обновление…',
        'update_notify_wait':        'Подождите, обновление применяется.',
        'update_notify_success':     'Готово! Пожалуйста, перезапустите BadWords.',
        'update_notify_failed':      'Обновление не удалось. Попробуйте ещё раз.',
        'update_notify_btn_later':   'Напомнить позже',
        'update_notify_btn_dismiss': 'Больше не уведомлять',
        'lbl_ultra_precise': "Ультраточный режим (Чанкинг)",
        'tt_ultra_precise': "Может значительно улучшить точность транскрипции даже на слабых\nмоделях — дословно улавливая каждое заикание, запинку и повторение.\nОсобенно рекомендуется для длинных записей,\nгде стандартный режим имеет тенденцию пропускать или объединять мелкие детали.\n\nПримечание: Обработка с этой опцией может занять заметно больше времени,\nв зависимости от вашего оборудования и платформы\n(Ускорение GPU доступно только на NVIDIA)",
    },
}
for _lang_code, _upd_keys in _UPDATE_I18N.items():
    TRANS.setdefault(_lang_code, {}).update(_upd_keys)
del _UPDATE_I18N, _lang_code, _upd_keys
