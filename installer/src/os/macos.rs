...
pub fn create_macos_app_bundle(install_dir: &Path, version: &str, create_desktop: bool) -> std::io::Result<()> {
    #[cfg(target_os = "macos")]
    {
        if let Some(home) = dirs::home_dir() {
            ...
            let disk_launcher_c = install_dir.join("setupfiles").join("macos").join("launcher.c");
            let disk_src = std::fs::read_to_string(&disk_launcher_c).ok();
            let launcher_c_src = disk_src.as_deref().unwrap_or(include_str!("../../../setupfiles/macos/launcher.c"));
